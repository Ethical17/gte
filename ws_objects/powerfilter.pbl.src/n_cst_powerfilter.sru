...
.============================================================
.	08/03/2010	1.0		Reesejr	Initial Version
.	07/09/2013	2.0		Reesejr	Added Color Constants, Color choice, font choice
.============================================================*/
...
String		BackgroundColor = '16053492' //String that matches constant values in n_cst_powerfilter, or a string containing the appropriate long value for the preferred color
String		FontName='Tahoma'
String		FontColor='windowtext' //String that matches constant values in n_cst_powerfilter, or a string containing the appropriate long value for the preferred color
datawindow	idw_dw 
Window	iw_parent
...
PRIVATEWRITE Boolean	ib_Visible //= true 
...
Constant Long Silver						=	12632256 
Constant Long ButtonFace				=	67108864
Constant Long ScrollBar					=	134217728
Constant Long Desktop					=	134217729
Constant Long ActiveTitleBar			=	134217730
Constant Long InactiveTitleBar			=	134217731
Constant Long MenuBar					=	134217732
Constant Long WindowFrame			=	134217734
Constant Long ActiveBorder				=	134217738
Constant Long InactiveBorder			=	134217739
Constant Long Highlight					=	134217741
Constant Long ButtonShadow			=	134217744
Constant Long ButtonHighlight			=	134217748
Constant Long ButtonDarkShadow		=	134217749
Constant Long ButtonLightShadow		=	134217750
Constant Long Tooltip						=	134217752
Constant Long Link							=	134217856
Constant Long LinkHover					=	134217857
Constant Long LinkActive					=	134217858
Constant Long LinkVisited				=	134217859
Constant Long ApplicationWorkspace	=	268435456
Constant Long WindowBackground		=	1073741824
Constant Long WindowText				=	33554432
Constant Long MenuText					=	134217735
Constant Long ActiveTitleBarText		=	134217737
Constant Long HighlightText				=	134217742
Constant Long DisabledText				=	134217745
Constant Long ButtonText				=	134217746
Constant Long InactiveTitleBarText	=	134217747
Constant Long TooltipText				=	134217751
...
.	01/09/2012	2.0		Reesejr	Use Long vars instead of Integer for horizontal positioning
.============================================================*/
IF not this.checked then RETURN 0 //buttons not painted, so no need to do anything

/////////////////////////////////////////////////////////////////////////////////////////////////////////////
String	ls_mod,ls_colname,ls_visible
Integer	li_ColNum,li_Visible
Long		ll_x,ll_xpos,ll_colwidth,ll_width
...
FOR li_colnum = 1 to ii_maxcol
	...
	ll_x=long(idw_dw.describe(ls_colname+".x"))
	ll_colwidth=long(idw_dw.describe(ls_colname+".width"))
	ll_x = ll_x + buttonXoffset * idec_unitXfactor

	IF ll_colwidth = 0 THEN
	...
	/*Button positioning*/
	IF ib_freeform THEN
		ll_xpos = ll_x + ll_colwidth
		ll_width = buttonwidth
	ELSE
		IF ll_colwidth >= buttonwidth * idec_unitXfactor THEN
			ll_xpos = ll_x +ll_colwidth - buttonwidth * idec_unitXfactor
			ll_width = buttonwidth * idec_unitXfactor
		ELSE
			ll_xpos = ll_x
			ll_width = ll_colwidth
		END IF
	END IF
	...
	END IF
	IF long(idw_dw.describe("b_powerfilter"+string(li_ColNum)+".width")) <> ll_width THEN
		//only modify if width has changed
		ls_mod = ls_mod + " " + "b_powerfilter"+string(li_ColNum)+".width='"+String(ll_width)+"'"
	END IF
NEXT
...
IF NOT IsValid(iw_parent) THEN
	...
	IF iw_parent.workspaceheight( ) < 1372 THEN
		CHOOSE CASE iw_parent.windowtype
			CASE popup!,response!
			CASE ELSE
...
IF NOT this.checked and PromptUserToRestore THEN
	...
	ELSE
		...
		ls_openparms.parm14 = ii_language
		ls_openparms.parm15 = long(backgroundcolor)
		ls_openparms.parm16 = FontName
		ls_openparms.parm17 = fontcolor
		Openwithparm(w_powerfilter_prompt_to_restore,ls_openparms,iw_parent)
...
IF this.checked THEN
	this.of_setbackgroundcolor(BackgroundColor)
	this.of_setfontcolor( FontColor )

	iw_parent.dynamic OpenUserObject ( iu_powerfilter_dropdown, 'u_powerfilter_dropdown') 
	iu_powerfilter_dropdown.of_initialize(idw_dw,l_powerfilter,MaxItems,MaxTime,ii_maxcol,AllowQuickSort,DefaultTipText)
	iu_powerfilter_Dropdown.of_setlanguage(ii_language)
	iu_powerfilter_Dropdown.of_setcolor(BackgroundColor)
	iu_powerfilter_Dropdown.of_setfont(FontName,FontColor)
	...
	iu_powerfilter_predeffilters.of_setcolor(BackgroundColor)
	iu_powerfilter_predeffilters.of_setfont(FontName,FontColor)
	...
	iu_powerfilter_monthopts.of_setcolor(BackgroundColor)
	iu_powerfilter_monthopts.of_setfont(FontName,FontColor)
...
.Returns:			integer 
.
.Description:	Event to fire user-specified post-filter processing
.					
.PowerFilter Datawindow Filter Service
.Copyright © 2010 James R. Reese.  All Rights Reserved.
.============================================================
...
.	08/03/2010	1.00		Reesejr	Initial Version
.============================================================*/
if len(is_postfilterevent) > 0 then
	ipo_parent.triggerevent(is_postfilterevent)
end if
return 1
end event
...
//iw_parent.event dynamic ue_testmessage('of_buildfilter START')
...
//iw_parent.event dynamic ue_testmessage('of_buildfilter END')
Return 0
...
.============================================================
.	08/03/2010	1.00		Reesejr	Initial Version
.	05/20/2012	2.0		Reesejr	Use Long for horizontal positioning for extra-wide datawindows
.============================================================*/
Integer	li_ypos,li_colwidth,li_width,li_height
Long		ll_xpos,ll_x
...
ll_x=long(idw_dw.describe(as_colname+".x"))
...
IF li_colwidth = 0 then ls_visible = '0' //default visibility to OFF if column width is 0.  2.0

ll_x = ll_x + buttonXoffset * idec_unitXfactor
IF li_colwidth >= buttonwidth * idec_unitXfactor THEN
	ll_xpos = ll_x +li_colwidth - buttonwidth * idec_unitXfactor
...
//RETURN FALSE //??JRR FORCE TO NOT USE DISPLAY VALUE IF DW IS SECONDARY SHARED
...
//iw_parent.event dynamic ue_testmessage('of_filter START')

idw_dw.setfilter( as_filter)
idw_dw.filter()
//iw_parent.event dynamic ue_testmessage('of_filter filtered, starting sort...')
...
END IF
idw_dw.groupcalc()
//iw_parent.event dynamic ue_testmessage('of_filter END')
RETURN 1
end function
...
choose case lower(as_color)
	case 'windowtext'
		BackgroundColor = string(windowtext)
	case 'menutext' 
		BackgroundColor = string(menutext)
	case 'activetitlebartext'
		BackgroundColor = string(activetitlebartext)
	case 'highlighttext' 
		BackgroundColor = string(highlighttext)
	case 'disabledtext' 
		BackgroundColor = string(disabledtext)
	case 'buttontext'
		BackgroundColor = string(buttontext)
	case 'inactivetitlebartext'
		BackgroundColor = string(inactivetitlebartext)
	case 'tooltiptext' 
		BackgroundColor = string(tooltiptext)
	case 'silver'
		BackgroundColor = string(silver)
	case 'buttonface'
		BackgroundColor = string(buttonface)
	case 'scrollbar'
		BackgroundColor = string(scrollbar)
	case 'desktop'	
		BackgroundColor = string(desktop)
	case 'activetitlebar'	
		BackgroundColor = string(activetitlebar)
	case 'inactivetitlebar'		
		BackgroundColor = string(inactivetitlebar)
	case 'menubar'				
		BackgroundColor = string(menubar)
	case 'windowframe'		
		BackgroundColor = string(windowframe)
	case 'activeborder'			
		BackgroundColor = string(activeborder)
	case 'inactiveborder'			
		BackgroundColor = string(inactiveborder)
	case 'highlight'				
		BackgroundColor = string(highlight)
	case 'buttonshadow'			
		BackgroundColor = string(buttonshadow)
	case 'buttonhighlight'			
		BackgroundColor = string(buttonhighlight)
	case 'buttondarkshadow'		
		BackgroundColor = string(buttondarkshadow)
	case 'buttonlightshadow'	
		BackgroundColor = string(buttonlightshadow)
	case 'tooltip'					
		BackgroundColor = string(tooltip)
	case 'link'						
		BackgroundColor = string(link)
	case 'linkhover'				
		BackgroundColor = string(linkhover)
	case 'linkactive'				
		BackgroundColor = string(linkactive)
	case 'linkvisited'				
		BackgroundColor = string(linkvisited)
	case 'applicationworkspace'	
		BackgroundColor = string(applicationworkspace)
	case 'windowbackground'		
		BackgroundColor = string(windowbackground)
	case else
		if isnumber(as_color) then
		BackgroundColor = as_color
		else
			BackgroundColor = string(windowbackground) //default to windowbackground
			//see if an RGB function was passed in to parse
			
			IF upper(left(trim(as_color),3)) = 'RGB' THEN
				 Integer li_openparen,li_firstcomma,li_secondcomma,li_closeparen
				 Long ll_red,ll_green,ll_blue
				 //parse to get red/green/blue values out of string
				 li_openparen =		POS ( as_color, '(')
				 li_firstcomma =		POS ( as_color,',',li_openparen+1)
				 li_secondcomma =	POS ( as_color,',',li_firstcomma+1)
				 li_closeparen =		POS ( as_color,')',li_secondcomma+1)
				 ll_red =		long(mid( as_color,li_openparen+1,(li_firstcomma - li_openparen - 1)))
				 ll_green =	long(mid( as_color,li_firstcomma+1,(li_secondcomma - li_firstcomma - 1)))
				 ll_blue =	long(mid( as_color,li_secondcomma+1,(li_closeparen - li_secondcomma - 1)))
				 BackgroundColor = string(rgb(ll_red,ll_green,ll_blue))
			END IF
		
		end if
end choose

return long(BackgroundColor)
...
is_postfilterevent = as_eventname
return 0
end function
...
choose case lower(as_color)
	case 'windowtext'
		FontColor = string(windowtext)
	case 'menutext' 
		FontColor = string(menutext)
	case 'activetitlebartext'
		FontColor = string(activetitlebartext)
	case 'highlighttext' 
		FontColor = string(highlighttext)
	case 'disabledtext' 
		FontColor = string(disabledtext)
	case 'buttontext'
		FontColor = string(buttontext)
	case 'inactivetitlebartext'
		FontColor = string(inactivetitlebartext)
	case 'tooltiptext' 
		FontColor = string(tooltiptext)
	case 'silver'
		FontColor = string(silver)
	case 'buttonface'
		FontColor = string(buttonface)
	case 'scrollbar'
		FontColor = string(scrollbar)
	case 'desktop'	
		FontColor = string(desktop)
	case 'activetitlebar'	
		FontColor = string(activetitlebar)
	case 'inactivetitlebar'		
		FontColor = string(inactivetitlebar)
	case 'menubar'				
		FontColor = string(menubar)
	case 'windowframe'		
		FontColor = string(windowframe)
	case 'activeborder'			
		FontColor = string(activeborder)
	case 'inactiveborder'			
		FontColor = string(inactiveborder)
	case 'highlight'				
		FontColor = string(highlight)
	case 'buttonshadow'			
		FontColor = string(buttonshadow)
	case 'buttonhighlight'			
		FontColor = string(buttonhighlight)
	case 'buttondarkshadow'		
		FontColor = string(buttondarkshadow)
	case 'buttonlightshadow'	
		FontColor = string(buttonlightshadow)
	case 'tooltip'					
		FontColor = string(tooltip)
	case 'link'						
		FontColor = string(link)
	case 'linkhover'				
		FontColor = string(linkhover)
	case 'linkactive'				
		FontColor = string(linkactive)
	case 'linkvisited'				
		FontColor = string(linkvisited)
	case 'applicationworkspace'	
		FontColor = string(applicationworkspace)
	case 'windowbackground'		
		FontColor = string(windowbackground)
	case else
		if isnumber(as_color) then
		FontColor = as_color
		else
			FontColor = string(0) //default to black
			//see if an RGB function was passed in to parse
			...
			IF upper(left(trim(as_color),3)) = 'RGB' THEN
				 ...
				 ll_green =	long(mid( as_color,li_firstcomma+1,(li_secondcomma - li_firstcomma - 1)))
				 ll_blue =	long(mid( as_color,li_secondcomma+1,(li_closeparen - li_secondcomma - 1)))
				 FontColor = string(rgb(ll_red,ll_green,ll_blue))
			END IF
		
		end if
end choose

return long(FontColor)
