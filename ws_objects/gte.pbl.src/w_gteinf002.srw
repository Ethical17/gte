$PBExportHeader$w_gteinf002.srw
forward
global type w_gteinf002 from window
end type
type cb_6 from commandbutton within w_gteinf002
end type
type cbx_1 from checkbox within w_gteinf002
end type
type em_1 from editmask within w_gteinf002
end type
type st_1 from statictext within w_gteinf002
end type
type cb_5 from commandbutton within w_gteinf002
end type
type cb_7 from commandbutton within w_gteinf002
end type
type cb_8 from commandbutton within w_gteinf002
end type
type cb_9 from commandbutton within w_gteinf002
end type
type dw_2 from datawindow within w_gteinf002
end type
type cb_4 from commandbutton within w_gteinf002
end type
type cb_3 from commandbutton within w_gteinf002
end type
type cb_2 from commandbutton within w_gteinf002
end type
type cb_1 from commandbutton within w_gteinf002
end type
type dw_1 from datawindow within w_gteinf002
end type
end forward
...
string title = "(w_gteinf002) Store Issue"
...
global w_gteinf002 w_gteinf002
...
integer height = 1256
integer taborder = 40
string dataobject = "dw_gteinf002a"
...
//		end if;	
		...
		dw_2.setitem(row,'unitprice',100)
		ld_tot_val =0
		ld_efunit_price =dw_2.getitemnumber(row,'unitprice')
		ld_tot_val = ld_qnty * ld_efunit_price
...
IF MessageBox("Save  Alert", 'Do You Want To Save ....?' ,Exclamation!, YesNo!, 1) = 1 THEN
	...
	if lb_neworder = true then
		...
		ll_last = ll_last + 1
		ls_tmp_id = 'REQ'+string(ll_last,'0000000000') 
	...
	if dw_1.getitemstring(dw_1.getrow(),'PRIS_STOCKIND')='Y'  then
		if f_issue_fifo(dw_1.getitemstring(dw_1.getrow(),'pris_id'),dw_1.getitemstring(dw_1.getrow(),'pris_division')) = -1 then
			return 1
		else
			dw_2.reset()
		...
		for ll_ctr = 1 to dw_2.rowcount()		
			...
			if dw_1.getitemstatus(dw_1.getrow(),0,primary!) = DataModified! then
				if dw_1.getitemstring(dw_1.getrow(),'PRIS_STOCKIND')='Y'  then
					//new
					//New/Update DailyExpense
					if luo_fames.wf_upd_mes(string(dw_1.getitemdatetime(dw_1.getrow(),'pris_date'),'dd/mm/yyyy'),dw_2.getitemstring(ll_ctr,'eacsubhead_id'),dw_2.getitemnumber(ll_ctr,'pris_value'),'T','N')  = -1 then 
						rollback using sqlca;
						return 1;
					end if;
					///update Stock Quantity
					if luo_fames.wf_upd_dailystock(dw_1.getitemstring(dw_1.getrow(),'pris_id'),string(dw_1.getitemdatetime(dw_1.getrow(),'pris_date'),'dd/mm/yyyy'),dw_2.getitemstring(ll_ctr,'sp_id'),dw_2.getitemnumber(ll_ctr,'pris_issuequantity'),dw_2.getitemnumber(ll_ctr,'unitprice'),dw_2.getitemnumber(ll_ctr,'pris_value'),'Store Issue','I','N',dw_1.getitemstring(dw_1.getrow(),'pris_division')) = -1 then 						
						rollback using sqlca;
						return 1;
					end if;		
				end if
...
integer height = 488
integer taborder = 30
string dataobject = "dw_gteinf002"
boolean hscrollbar = true
end type
