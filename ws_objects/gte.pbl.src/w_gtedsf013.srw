$PBExportHeader$w_gtedsf013.srw
forward
global type w_gtedsf013 from window
end type
type cb_10 from commandbutton within w_gtedsf013
end type
type cb_6 from commandbutton within w_gtedsf013
end type
type cb_5 from commandbutton within w_gtedsf013
end type
type cb_7 from commandbutton within w_gtedsf013
end type
type cb_8 from commandbutton within w_gtedsf013
end type
type cb_9 from commandbutton within w_gtedsf013
end type
type cb_4 from commandbutton within w_gtedsf013
end type
type cb_3 from commandbutton within w_gtedsf013
end type
type cb_2 from commandbutton within w_gtedsf013
end type
type cb_1 from commandbutton within w_gtedsf013
end type
type dw_1 from datawindow within w_gtedsf013
end type
type dw_2 from datawindow within w_gtedsf013
end type
end forward
...
string title = "(w_gtedsf013) Miscellaneous Invoice"
...
global w_gtedsf013 w_gtedsf013
...
string ls_bname, ls_badd, ls_cname, ls_cadd, ls_gradelist, ls_cnno, ls_cust, ls_brok,ls_lotno, ls_iss_locn, ls_rec_locn,ls_hsn_cd,ls_wareid, ls_lutno,ls_fin_yr,ls_msi_id,ls_si_type,ls_uom
...
double ld_sgst_amt,ld_cgst_amt,ld_igst_amt
datetime ld_condt, ld_invdt
datawindowchild idw_item
string ls_cusid,ls_cusreg,ls_exempted
double ld_packingchrg,ld_othchgr,ld_delvchrg,ld_otsgst,ld_otcgst,ld_otigst,ld_packper,ld_othper,ld_delvper
string ls_decvhsn,ls_packhsn,ls_othhsn
end variables
...
event clicked;opensheetwithparm(w_gtedsr014,this.tag,w_mdi,0,layered!)
end event
...
event clicked;opensheetwithparm(w_gtedsr013,this.tag,w_mdi,0,layered!)
end event
...
//======
		if ls_iss_gstn <> ls_rec_gstn then		
			if ll_tino = 0 then
				...
				if dw_1.getitemstring(dw_1.getrow(),'si_type') = 'RUBBER' then
					select distinct 'x' into :ls_temp from FB_SERIAL_NO where SN_DOC_TYPE in ('RBTI') and SN_ACCT_YEAR=:ll_ac_year;
					
					if sqlca.sqlcode = 100 then
						INSERT INTO FB_SERIAL_NO ( SN_DOC_TYPE, SN_SRL_NO, SN_ACCT_YEAR ) VALUES ( 'RBTI', 0, :ll_ac_year); 
						commit using sqlca;
					end if
					
					ll_tino = f_get_lastno('RBTI',string(ll_ac_year))
					...
					if ll_tino >= 0 then
						...
						ls_tinvno =gs_garden_snm+"/RTI/"+ls_count+"/"+ls_fin_yr					
						dw_1.setitem(dw_1.getrow(),'SI_TAXINVNO',ls_tinvno)	
					end if 
				end if
			
				
				if dw_1.getitemstring(dw_1.getrow(),'si_type') = 'SCRAP' then
					select distinct 'x' into :ls_temp from FB_SERIAL_NO where SN_DOC_TYPE in ('SCTI') and SN_ACCT_YEAR=:ll_ac_year;
					
					if sqlca.sqlcode = 100 then
						INSERT INTO FB_SERIAL_NO ( SN_DOC_TYPE, SN_SRL_NO, SN_ACCT_YEAR ) VALUES ( 'SCTI', 0, :ll_ac_year); 
						commit using sqlca;
					end if
					
					ll_tino = f_get_lastno('SCTI',string(ll_ac_year))
				...
				if dw_1.getitemstring(dw_1.getrow(),'si_type') = 'MISC' then
					select distinct 'x' into :ls_temp from FB_SERIAL_NO where SN_DOC_TYPE in ('MSTI') and SN_ACCT_YEAR=:ll_ac_year;
					
					if sqlca.sqlcode = 100 then
						INSERT INTO FB_SERIAL_NO ( SN_DOC_TYPE, SN_SRL_NO, SN_ACCT_YEAR ) VALUES ( 'MSTI', 0, :ll_ac_year); 
						commit using sqlca;
					end if
					
					ll_tino = f_get_lastno('MSTI',string(ll_ac_year))
					...
					if ll_tino >= 0 then
						ls_count = string(ll_tino,'000')
						ls_tinvno =gs_garden_snm+"/MTI/"+ls_count+"/"+ls_fin_yr					
						dw_1.setitem(dw_1.getrow(),'SI_TAXINVNO',ls_tinvno)	
					end if 
				end if	
			else
			...
			if not isnull(ls_rec_gstn) and len(ls_rec_gstn) > 0 then	
				declare p2 procedure for up_post_e_invoicing ('MISC',:ls_tinvno, 'GTEDSF013');
...
//		elseif ls_iss_gstn = ls_rec_gstn then		
			if ll_dlcno = 0 then
				...
				if dw_1.getitemstring(dw_1.getrow(),'si_type') = 'RUBBER' then
					select distinct 'x' into :ls_temp from FB_SERIAL_NO where SN_DOC_TYPE in ('RBDC') and SN_ACCT_YEAR=:ll_ac_year;
					
					if sqlca.sqlcode = 100 then
						INSERT INTO FB_SERIAL_NO ( SN_DOC_TYPE, SN_SRL_NO, SN_ACCT_YEAR ) VALUES ( 'RBDC', 0, :ll_ac_year); 
						commit using sqlca;
					end if
					
					ll_dlcno = f_get_lastno('RBDC',string(ll_ac_year))
					...
					if ll_dlcno >= 0 then
						...
						ls_dlcno = gs_garden_snm+"/RDC/"+ls_count+"/"+ls_fin_yr					
				...
				if dw_1.getitemstring(dw_1.getrow(),'si_type') = 'SCRAP' then
					select distinct 'x' into :ls_temp from FB_SERIAL_NO where SN_DOC_TYPE in ('SCDC') and SN_ACCT_YEAR=:ll_ac_year;
					
					if sqlca.sqlcode = 100 then
						INSERT INTO FB_SERIAL_NO ( SN_DOC_TYPE, SN_SRL_NO, SN_ACCT_YEAR ) VALUES ( 'SCDC', 0, :ll_ac_year); 
						commit using sqlca;
					end if
					
					ll_dlcno = f_get_lastno('SCDC',string(ll_ac_year))
					...
					if ll_dlcno >= 0 then
						ls_count = string(ll_dlcno,'000')
						ls_dlcno =gs_garden_snm+"/SDC/"+ls_count+"/"+ls_fin_yr					
						dw_1.setitem(dw_1.getrow(),'SI_DELVCHNO',ls_dlcno)	
					end if 
				end if
				...
				if dw_1.getitemstring(dw_1.getrow(),'si_type') = 'MISC' then
					...
					ll_dlcno = f_get_lastno('MSTI',string(ll_ac_year))
					...
					if ll_dlcno >= 0 then
						ls_count = string(ll_dlcno,'000')
						ls_dlcno =gs_garden_snm+"/MDC/"+ls_count+"/"+ls_fin_yr					
						dw_1.setitem(dw_1.getrow(),'SI_DELVCHNO',ls_dlcno)	
					end if 
				end if		
			else 
		...
		if ll_last=0 then
			select nvl(MAX(substr(SI_ID,4,10)),0) into :ll_last from fb_misc_saleinvoice;
		end if
		ll_last = ll_last + 1
		ls_msi_id = 'MSI'+string(ll_last,'0000000000')
	...
	if dw_1.update(true,false) = 1 and dw_2.update(true,false) = 1 then
		if dw_1.getitemstatus(dw_1.getrow(),0,primary!) = NewModified! or dw_1.getitemstatus(dw_1.getrow(),0,primary!) = New! then		
			if ll_tino > 0 then
				...
				if ls_iss_gstn <> ls_rec_gstn then	
					if dw_1.getitemstring(dw_1.getrow(),'si_type') = 'RUBBER' then 
						if f_upd_lastno('RBTI',string(ll_ac_year),ll_tino) = -1 then
							rollback using sqlca;			
							return 1
						end if	
					end if
					if dw_1.getitemstring(dw_1.getrow(),'si_type') = 'SCRAP' then 
						if f_upd_lastno('SCTI',string(ll_ac_year),ll_tino) = -1 then
							rollback using sqlca;			
							return 1
						end if	
					end if
					if dw_1.getitemstring(dw_1.getrow(),'si_type') = 'MISC' then 
						if f_upd_lastno('MSTI',string(ll_ac_year),ll_tino) = -1 then
							rollback using sqlca;			
							return 1
						end if	
					end if
			...
			if ll_dlcno > 0 then
				if dw_1.getitemstring(dw_1.getrow(),'si_type') = 'RUBBER' then 
					if f_upd_lastno('RBDC',string(ll_ac_year),ll_tino) = -1 then
						rollback using sqlca;			
						return 1
					end if	
				end if
				if dw_1.getitemstring(dw_1.getrow(),'si_type') = 'SCRAP' then 
					if f_upd_lastno('SCDC',string(ll_ac_year),ll_tino) = -1 then
						rollback using sqlca;			
						return 1
					end if	
				end if
				if dw_1.getitemstring(dw_1.getrow(),'si_type') = 'MISC' then 
					if f_upd_lastno('MSDC',string(ll_ac_year),ll_tino) = -1 then
						rollback using sqlca;			
						return 1
					end if	
				end if						
...
integer height = 1276
integer taborder = 30
string dataobject = "dw_gtedsf013"
end type
...
//		if f_check_string_sp(data) = -1 then return 1
	...
	if dwo.name='si_packchrg' or dwo.name='si_othchrg' or  dwo.name='si_delvchrg' or dwo.name='si_packchrg_per' or dwo.name='si_othchrg_per' or dwo.name='si_delvchrg_per'   then
		ls_iss_locn = dw_1.getitemstring(1,'si_iss_locn')
		ls_rec_locn = dw_1.getitemstring(1,'si_rec_locn')	
		ls_iss_gstn = dw_1.getitemstring(1,'si_iss_gstnno')
		ls_rec_gstn = dw_1.getitemstring(1,'si_rec_gstnno')	
		ld_invdt = dw_1.getitemdatetime(1,'si_date')
	
		if dwo.name='si_packchrg' or dwo.name='si_packchrg_per' then
			if  dwo.name='si_packchrg' then
				ld_packingchrg=double(data)
				ld_packper=dw_1.getitemnumber(1,'si_packchrg_per')
			elseif dwo.name='si_packchrg_per' then
				ld_packper=double(data)
				ld_packingchrg=dw_1.getitemnumber(1,'si_packchrg')
			end if		
			if (ls_iss_locn = ls_rec_locn) and ( ls_iss_gstn <> ls_rec_gstn) then 
				ld_igst_per = 0
				ld_cgst_per = ld_packper/2
				ld_sgst_per = ld_packper/2
				dw_1.setitem(row,'si_packchrg_cgst',(ld_packingchrg *(ld_cgst_per/100)))
				dw_1.setitem(row,'si_packchrg_sgst',(ld_packingchrg *(ld_sgst_per/100)))
				dw_1.setitem(row,'si_packchrg_igst',0)
			elseif (ls_iss_locn <> ls_rec_locn) and ( ls_iss_gstn <> ls_rec_gstn) then
				ld_igst_per = ld_packper
				ld_cgst_per =0
				ld_sgst_per =0
				dw_1.setitem(row,'si_packchrg_cgst',0)
				dw_1.setitem(row,'si_packchrg_sgst',0)
				dw_1.setitem(row,'si_packchrg_igst',(ld_packingchrg *(ld_igst_per/100)))
			end if	
		elseif  dwo.name='si_othchrg' or dwo.name='si_othchrg_per'  then
			if  dwo.name='si_othchrg' then
				ld_othchgr=double(data)
				ld_othper=dw_1.getitemnumber(1,'si_othchrg_per')
			elseif dwo.name='si_othchrg_per' then
				ld_othper=double(data)
				ld_othchgr=dw_1.getitemnumber(1,'si_othchrg')
			end if	
			if (ls_iss_locn = ls_rec_locn) and ( ls_iss_gstn <> ls_rec_gstn) then 
				ld_igst_per = 0
				ld_cgst_per = ld_othper/2
				ld_sgst_per = ld_othper/2
				dw_1.setitem(row,'si_ocgst',(ld_othchgr *(ld_cgst_per/100)))
				dw_1.setitem(row,'si_osgst',(ld_othchgr *(ld_sgst_per/100)))
				dw_1.setitem(row,'si_oigst',0)
			elseif (ls_iss_locn <> ls_rec_locn) and ( ls_iss_gstn <> ls_rec_gstn) then
				ld_igst_per = ld_othper
				ld_cgst_per =0
				ld_sgst_per =0
				dw_1.setitem(row,'si_ocgst',0)
				dw_1.setitem(row,'si_osgst',0)
				dw_1.setitem(row,'si_oigst',(ld_othchgr *(ld_igst_per/100)))
			end if			
		elseif  dwo.name='si_delvchrg'  or dwo.name='si_delvchrg_per' then		
			if  dwo.name='si_delvchrg' then
				ld_delvchrg=double(data)
				ld_delvper=dw_1.getitemnumber(1,'si_delvchrg_per')
			elseif dwo.name='si_delvchrg_per' then
				ld_delvper=double(data)
				ld_delvchrg=dw_1.getitemnumber(1,'si_delvchrg')
			end if	
			if (ls_iss_locn = ls_rec_locn) and ( ls_iss_gstn <> ls_rec_gstn) then 
				ld_igst_per = 0
				ld_cgst_per =ld_delvper /2
				ld_sgst_per = ld_delvper/2
				dw_1.setitem(row,'si_delvchrg_cgst',(ld_delvchrg *(ld_cgst_per/100)))
				dw_1.setitem(row,'si_delvchrg_sgst',(ld_delvchrg *(ld_sgst_per/100)))
				dw_1.setitem(row,'si_delvchrg_igst',0)
			elseif (ls_iss_locn <> ls_rec_locn) and ( ls_iss_gstn <> ls_rec_gstn) then
				ld_igst_per = ld_delvper
				ld_cgst_per =0
				ld_sgst_per =0
				dw_1.setitem(row,'si_delvchrg_cgst',0)
				dw_1.setitem(row,'si_delvchrg_sgst',0)
				dw_1.setitem(row,'si_delvchrg_igst',(ld_delvchrg *(ld_igst_per/100)))
			end if	
		end if
end if
...
integer height = 1200
integer taborder = 40
string dataobject = "dw_gtedsf013a"
...
event itemchanged;ls_exempted=dw_1.getitemstring(row,'si_exempted')
IF isnull(ls_exempted) then ls_exempted = 'N'
if dwo.name = 'item_id' then
	ls_item_id = data
	select ITEM_UOM into :ls_uom from fb_misc_item where item_id = :ls_item_id and item_active_ind = 'Y';
	if sqlca.sqlcode = -1 then
		messagebox('Error','Error occured while fetching Unit Of Measure!!')
		rollback using sqlca;
		return 1;
	end if 
	select ITEM_HSN_NO into :ls_hsn_cd from fb_misc_item where item_id =  :ls_item_id;
	if sqlca.sqlcode = -1 then 
	...
	dw_2.setitem(dw_2.getrow(),'item_uom',ls_uom)
	ld_quantity = 0; 
	ld_rate = 0;
	ld_tot_amt = 0;
	ld_net_amt= 0;
...
if dwo.name = 'sid_quantity' then
	ld_quantity = double(data)
	if isnull(dw_2.getitemstring(dw_2.getrow(),'item_id')) or len(dw_2.getitemstring(dw_2.getrow(),'item_id')) = 0 then
		messagebox('Warning','Kindly select item first')
		return 1;
	end if 
	ld_rate = double(dw_2.getitemnumber(dw_2.getrow(),'sid_rate'))	
	...
	if ls_iss_gstn <> ls_rec_gstn then
		...
		ls_cusid=dw_1.getitemstring(1,'cus_id')		
		select cus_registered into :ls_cusreg from fb_customer where cus_id=:ls_cusid;		
		if sqlca.sqlcode = -1 then 
			messagebox('Sql Error','Error During Getting Customer Registeration : '+sqlca.sqlerrtext)
			return 1
		elseif sqlca.sqlcode = 100 then 
			messagebox('Error','Customer Registeration Is Missing, Please Check !!!')
			return 1
		end if;			
		if ls_cusreg='1' then
			select nvl(HM_CGST_RATE,0), nvl(HM_SGST_RATE,0), nvl(HM_IGST_RATE,0) into :ld_cgst_per, :ld_sgst_per, :ld_igst_per
