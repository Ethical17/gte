$PBExportHeader$w_gtelaf036.srw
forward
global type w_gtelaf036 from window
end type
type cb_4 from commandbutton within w_gtelaf036
end type
type cb_3 from commandbutton within w_gtelaf036
end type
type cb_2 from commandbutton within w_gtelaf036
end type
type cb_1 from commandbutton within w_gtelaf036
end type
type dw_1 from datawindow within w_gtelaf036
end type
end forward
...
string title = "(w_gtelaf036) Ration Parameters"
...
global w_gtelaf036 w_gtelaf036

type variables
long ll_ctr, ll_cnt, ll_year, ll_last,ll_user_level
string ls_temp,ls_name,ls_last,ls_type
...
public function integer wf_check_fillcol (integer fl_row);if dw_1.rowcount() > 0 and fl_row > 0 then
	if (isnull(dw_1.getitemdatetime(fl_row,'rp_period_from')) or &
		 isnull(dw_1.getitemstring(fl_row,'rp_emptype')) or  len(dw_1.getitemstring(fl_row,'rp_emptype'))= 0 ) then
	    messagebox('Warning: One Of The Following Fields Are Blank', 'Emp Type, Period From, Please Check !!!')
		 return -1
	end if
end if
return 1
...
IF (isnull(dw_1.getitemstring(dw_1.rowcount(),'rp_emptype')) or len(dw_1.getitemstring(dw_1.rowcount(),'rp_emptype'))=0) THEN
...
IF MessageBox("Save  Alert", 'Do You Want To Save ....?' ,Exclamation!, YesNo!, 1) = 1 THEN
	...
	for ll_ctr = 1 to dw_1.rowcount( ) 
			ld_frdt = dw_1.getitemdatetime(ll_ctr,'rp_period_from')
			ls_type = dw_1.getitemstring(ll_ctr,'rp_emptype')
			
			select distinct 'x' into :ls_temp from fb_rationparam
			 where RP_EMPTYPE = :ls_type and  RP_PERIOD_FROM >= :ld_frdt and RP_PERIOD_TO is null;
			...
			elseif sqlca.sqlcode  = 0 then
				...
				return 1
				  ...
				  update fb_rationparam set RP_PERIOD_TO = (:ld_frdt - 1)
					where RP_EMPTYPE = :ls_type and  RP_PERIOD_TO is null;
				 
				 if sqlca.sqlcode = -1 then
					messagebox('Error : While Updating Record',sqlca.sqlerrtext)
					rollback using sqlca;
					return 1	
				 end if	
			end if
...
if dw_1.rowcount() = 0 then
	...
	dw_1.setfocus()
	dw_1.setitem(dw_1.getrow(),'rp_entry_by',gs_user)
	dw_1.setitem(dw_1.getrow(),'rp_entry_dt',datetime(today()))
	dw_1.setcolumn('rp_emptype')
else
	IF wf_check_fillcol(dw_1.getrow()) = -1 THEN
		return 1
...
integer height = 2052
integer taborder = 30
string dataobject = "dw_gtelaf036"
...
event key_down;if lb_query = false then
 if lb_neworder = true then	
	...
	if newrow = dw_1.rowcount() and dw_1.rowcount() > 1  then
		dw_1.setitem(newrow,'rp_entry_by',gs_user)
		dw_1.setitem(newrow,'rp_entry_dt',datetime(today()))
		dw_1.setcolumn('rp_emptype')
	end if
 end if
end if
...
if dwo.name = 'rp_period_from' then
	ls_type = dw_1.getitemstring(row,'rp_emptype')
	...
	dw_1.setitem(row,'rp_entry_by',gs_user)
	dw_1.setitem(row,'rp_entry_dt',datetime(today()))
...
SELECT COLUMN_ID,COLUMN_Name FROM USER_TAB_COLUMNS WHERE TABLE_NAME = upper('fb_rationparam');
			...
			ls_unique_id=dw_1.GetItemString(li_row, 1, Primary!, true)
					 ...
					 if ls_old_value <> ls_new_value then
						insert into fb_log(table_name, Unique_key, U_ACTION, U_FIELD, U_OLDVALUE, U_NEWVALUE, U_USERCODE, U_DATE, U_COMMENT) values
												('fb_rationparam',:ls_unique_id,'U',:ls_columnname,:ls_old_value,:ls_new_value,:gs_user,sysdate,'');
							if sqlca.sqlcode = -1 then
								messagebox("SQL Error : While Inserting Into Log Table ",SQLCA.SQLErrtext,Information!)
								rollback using sqlca;
								return -1 
