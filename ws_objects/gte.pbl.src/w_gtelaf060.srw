$PBExportHeader$w_gtelaf060.srw
forward
global type w_gtelaf060 from window
end type
type cb_4 from commandbutton within w_gtelaf060
end type
type cb_1 from commandbutton within w_gtelaf060
end type
type cb_3 from commandbutton within w_gtelaf060
end type
type cb_2 from commandbutton within w_gtelaf060
end type
type dw_1 from datawindow within w_gtelaf060
end type
end forward
...
string title = "(w_gtelaf060) PF Incoming/Outgoing Transfer"
...
global w_gtelaf060 w_gtelaf060
...
public function integer wf_check_duplicate_rec (string fs_lab_id, string fs_tag, long rownum)
end prototypes
...
public function integer wf_check_fillcol (integer fl_row);if dw_1.rowcount() > 0 and fl_row > 0 then
	if (isnull(dw_1.getitemstring(fl_row,'emp_id')) or  len(dw_1.getitemstring(fl_row,'emp_id'))=0 or &
		...
		isnull(dw_1.getitemstring(fl_row,'pfig_tag')) or  len(dw_1.getitemstring(fl_row,'pfig_tag'))=0 or &
		isnull(dw_1.getitemdatetime(fl_row,'pfig_date')) or isnull(dw_1.getitemdatetime(fl_row,'pfig_date'))) then
	    messagebox('Warning: One Of The Following Fields Are Blank','Emp ID, PF No., Incoming/Outgoing Tag, Incoming/Outgoing Date, Please Check !!!')
		 return -1
	end if
	
	if (isnull(dw_1.getitemnumber(fl_row,'pfig_amount')) or  dw_1.getitemnumber(fl_row,'pfig_amount') = 0) and dw_1.getitemstring(fl_row,'pfig_tag') = 'INCOMING' then
		 messagebox('Warning','Incoming Amount is blank, Please Check !!!')
		 return -1
	end if
end if
return 1
...
public function integer wf_check_duplicate_rec (string fs_lab_id, string fs_tag, long rownum);long fl_row
string ls_lab_id1, ls_tag1
...
if dw_1.rowcount() > 1 then
	for fl_row = 1 to (dw_1.rowcount())
		IF fl_row <> rownum then
			...
			ls_tag1 = dw_1.getitemstring(fl_row,'pfig_tag')
			if ls_lab_id1 = fs_lab_id and ls_tag1 = fs_tag then
...
if dw_1.rowcount() = 0 then
	...
	dw_1.setfocus()
	dw_1.setitem(dw_1.getrow(),'pfig_entry_by',gs_user)
	dw_1.setitem(dw_1.getrow(),'pfig_entry_dt',datetime(today()))
	dw_1.settaborder('emp_id',10)
	dw_1.settaborder('emp_id_1',20)
	dw_1.settaborder('pfig_tag',30)
	dw_1.settaborder('pfig_date',40)
	dw_1.settaborder('pfig_amount',40)
...
event clicked;string ls_temp_empid, ls_temp_tag
long ll_temp_yrmn
...
IF MessageBox("Save  Alert", 'Do You Want To Save ....?' ,Exclamation!, YesNo!, 1) = 1 THEN
		...
		ls_temp_tag =  dw_1.getitemstring(dw_1.getrow(),'pfig_tag')
		IF wf_check_fillcol(dw_1.getrow()) = -1 or wf_check_duplicate_rec(ls_temp_empid,ls_temp_tag,dw_1.getrow()) = -1 THEN
			return 1
		END IF
		select 'x' into :ls_temp from fb_pf_incomingoutgoing where emp_id = :ls_temp_empid and pfig_tag = :ls_temp_tag ;
...
event clicked;if cb_2.text = "&Query" then
	...
	dw_1.settaborder('pfig_tag',0)
	dw_1.settaborder('pfig_date',0)
	dw_1.settaborder('pfig_amount',0)
	dw_1.SetRedraw (TRUE)
	cb_2.text = "&Query"
	lb_query = false
	cb_1.enabled = true
...
integer height = 2052
integer taborder = 30
string dataobject = "dw_gtelaf060"
boolean hscrollbar = true
boolean vscrollbar = true
boolean resizable = true
end type

event key_down;string ls_temp_empid, ls_temp_tag
 ...
 if lb_neworder = true then	
	if currentrow <> dw_1.rowcount() and dw_1.rowcount() > 1 then	
		 ls_temp_empid = dw_1.getitemstring(currentrow,'emp_id')
    		ls_temp_tag =  dw_1.getitemstring(dw_1.getrow(),'pfig_tag')
		IF wf_check_fillcol(currentrow) = -1 or wf_check_duplicate_rec(ls_temp_empid,ls_temp_tag,currentrow) = -1 THEN
			return 1
		END IF
		select 'x' into :ls_temp from fb_pf_incomingoutgoing where emp_id = :ls_temp_empid and pfig_tag = :ls_temp_tag;
		if sqlca.sqlcode = -1 then
	...
	if newrow = dw_1.rowcount() and dw_1.rowcount() > 1  then
		dw_1.setitem(newrow,'pfig_entry_by',gs_user)
		dw_1.setitem(newrow,'pfig_entry_dt',datetime(today()))
...
event itemchanged;if lb_query = false then
	...
	if dwo.name = 'pfig_tag' and data = 'OUTGOING' then
		dw_1.setitem(row,'pfig_amount',0)
	end if
	dw_1.setitem(row,'pfig_entry_by',gs_user)
	dw_1.setitem(row,'pfig_entry_dt',datetime(today()))
	cb_3.enabled = true
end if	
...
SELECT COLUMN_ID,COLUMN_Name FROM USER_TAB_COLUMNS WHERE TABLE_NAME = upper('fb_pf_incomingoutgoing');
			...
			ls_unique_id=dw_1.GetItemString(li_row, 1, Primary!, true)
					 ...
					 if ls_old_value <> ls_new_value then
						insert into fb_log(table_name, Unique_key, U_ACTION, U_FIELD, U_OLDVALUE, U_NEWVALUE, U_USERCODE, U_DATE, U_COMMENT) values
												('fb_pf_incomingoutgoing',:ls_unique_id,'U',:ls_columnname,:ls_old_value,:ls_new_value,:gs_user,sysdate,'');
							if sqlca.sqlcode = -1 then
								messagebox("SQL Error : While Inserting Into Log Table ",SQLCA.SQLErrtext,Information!)
								rollback using sqlca;
								return -1 
