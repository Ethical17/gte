$PBExportHeader$w_gtelaf035.srw
forward
global type w_gtelaf035 from window
end type
type cb_4 from commandbutton within w_gtelaf035
end type
type cb_3 from commandbutton within w_gtelaf035
end type
type cb_2 from commandbutton within w_gtelaf035
end type
type cb_1 from commandbutton within w_gtelaf035
end type
type dw_1 from datawindow within w_gtelaf035
end type
end forward
...
string title = "(w_gtelaf035) Labour Wage Advance Week"
...
global w_gtelaf035 w_gtelaf035
...
public function integer wf_check_fillcol (integer fl_row);if dw_1.rowcount() > 0 and fl_row > 0 then
	if (isnull(dw_1.getitemdatetime(fl_row,'lwaw_startdate')) or isnull(dw_1.getitemdatetime(fl_row,'lwaw_enddate'))) then
...
if dw_1.rowcount() > 1 then
	for fl_row = 1 to (dw_1.rowcount() - 1)
		ld_fdt1 = dw_1.getitemdatetime(fl_row,'lwaw_startdate')
		ld_todt1 = dw_1.getitemdatetime(fl_row,'lwaw_enddate')
...
IF (isnull(dw_1.getitemdatetime(dw_1.rowcount(),'lwaw_startdate')) ) THEN
...
IF MessageBox("Save  Alert", 'Do You Want To Save ....?' ,Exclamation!, YesNo!, 1) = 1 THEN
	...
	if lb_neworder = true then
		select nvl(MAX(lwaw_ID),0) into :ls_last from fb_labourwagadvweek ;
	...
	if dw_1.rowcount() > 0 then
		for ll_ctr = 1 to dw_1.rowcount( ) 			
			ld_todt = dw_1.getitemdatetime(ll_ctr,'lwaw_enddate')
			ld_frdt = dw_1.getitemdatetime(ll_ctr,'lwaw_startdate')
			
			select distinct 'x' into :ls_temp from fb_labourwagadvweek  where trunc(:ld_frdt) between trunc(lwaw_startdate) and trunc(lwaw_enddate);
			...
			select distinct 'x' into :ls_temp from fb_labourwagadvweek  where trunc(lwaw_startdate) = trunc(:ld_frdt) and trunc(lwaw_enddate) = trunc(:ld_todt);
			...
			if sqlca.sqlcode = -1 then
				...
				return 1
			elseif sqlca.sqlcode  = 0 then
				messagebox('Warning!','Labour Wage Advance Week Already Exists, Please Check !!!')
				return 1
			end if
			...
			if lb_neworder = true then
				...
				ls_tmp_id = 'LWAW'+string(ll_cnt,'00000')
				dw_1.setitem(ll_ctr,'lwaw_id',ls_tmp_id)
			end if
...
select count(*) into :ll_temp from fb_labourwagadvweek where nvl(lwaw_PAIDFLAG,'0') = '0';

if sqlca.sqlcode = -1 then
	messagebox('Error : While Getting Open Labour Wage Advance Week',sqlca.sqlerrtext)
	rollback using sqlca;
	return 1
elseif sqlca.sqlcode  = 0 then
	if ll_temp > 1 then
		messagebox('Warning!','Two Labour Wage Advance Week Already Open, Please Check !!!')
		return 1
	end if
end if
...
integer height = 2052
integer taborder = 30
string dataobject = "dw_gtelaf035"
...
event key_down;if lb_query = false then
 if lb_neworder = true then	
	...
	if newrow = dw_1.rowcount() and dw_1.rowcount() > 1  then
		dw_1.setitem(newrow,'lwaw_entry_by',gs_user)
		dw_1.setitem(newrow,'lwaw_entry_dt',datetime(today()))
		dw_1.setcolumn('lwaw_startdate')
	end if
 end if
end if
...
event itemchanged;if lb_query = false then
	if dwo.name = 'lwaw_startdate'  then
		ld_frdt = datetime(data)
		ld_todt = dw_1.getitemdatetime(row,'lwaw_enddate')
...
//		end if
		...
		//dw_1.setitem(row,'lwaw_enddate',RelativeDate(date(ld_frdt), 6))
	end if
	
	if dwo.name = 'lwaw_enddate'  then
		ld_todt = datetime(data)
		ld_frdt = dw_1.getitemdatetime(row,'lwaw_startdate')
...
//		end if
	...
	dw_1.setitem(row,'lwaw_entry_by',gs_user)
	dw_1.setitem(row,'lwaw_entry_dt',datetime(today()))
cb_3.enabled = true
end if
...
SELECT COLUMN_ID,COLUMN_Name FROM USER_TAB_COLUMNS WHERE TABLE_NAME = upper('fb_labourwagadvweek');
			...
			ls_unique_id=dw_1.GetItemString(li_row, 1, Primary!, true)
					 ...
					 if ls_old_value <> ls_new_value then
						insert into fb_log(table_name, Unique_key, U_ACTION, U_FIELD, U_OLDVALUE, U_NEWVALUE, U_USERCODE, U_DATE, U_COMMENT) values
												('fb_labourwagadvweek',:ls_unique_id,'U',:ls_columnname,:ls_old_value,:ls_new_value,:gs_user,sysdate,'');
							if sqlca.sqlcode = -1 then
								messagebox("SQL Error : While Inserting Into Log Table ",SQLCA.SQLErrtext,Information!)
								rollback using sqlca;
								return -1 
