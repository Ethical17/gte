$PBExportHeader$w_gtelaf015_07102024.srw
forward
global type w_gtelaf015_07102024 from window
end type
type cb_9 from commandbutton within w_gtelaf015_07102024
end type
type cb_8 from commandbutton within w_gtelaf015_07102024
end type
type cb_7 from commandbutton within w_gtelaf015_07102024
end type
type cbx_4 from checkbox within w_gtelaf015_07102024
end type
type cb_6 from commandbutton within w_gtelaf015_07102024
end type
type cbx_3 from checkbox within w_gtelaf015_07102024
end type
type cbx_2 from checkbox within w_gtelaf015_07102024
end type
type cbx_1 from checkbox within w_gtelaf015_07102024
end type
type cb_5 from commandbutton within w_gtelaf015_07102024
end type
type rb_2 from radiobutton within w_gtelaf015_07102024
end type
type rb_1 from radiobutton within w_gtelaf015_07102024
end type
type st_3 from statictext within w_gtelaf015_07102024
end type
type st_2 from statictext within w_gtelaf015_07102024
end type
type ddlb_1 from dropdownlistbox within w_gtelaf015_07102024
end type
type cb_2 from commandbutton within w_gtelaf015_07102024
end type
type cb_1 from commandbutton within w_gtelaf015_07102024
end type
type st_1 from statictext within w_gtelaf015_07102024
end type
type cb_4 from commandbutton within w_gtelaf015_07102024
end type
type cb_3 from commandbutton within w_gtelaf015_07102024
end type
type gb_1 from groupbox within w_gtelaf015_07102024
end type
type dw_1 from datawindow within w_gtelaf015_07102024
end type
type em_1 from editmask within w_gtelaf015_07102024
end type
type dp_1 from datepicker within w_gtelaf015_07102024
end type
end forward
...
global w_gtelaf015_07102024 w_gtelaf015_07102024
...
datetime ld_rundt,ld_stdt,ldenddt,ld_dob, ld_pwlastday
...
//		ld_status1 = dw_1.getitemnumber(fl_row,'lda_status')
		...
		ld_dt = dw_1.getitemstring(fl_row,'lda_date')
		ls_JUNGLIFOOT_ind = dw_1.getitemstring(fl_row,'lda_mfj_junglifoot')
		
		select emp.emp_dob labdob, get_diff(to_date(:ld_dt,'dd/mm/yyyy'),emp_dob,'B'), emp_type, ls_id,field_id into :ld_dob, :ld_labage, :ls_emptype, :ll_paybook,:ls_div//, ((:ld_dt - emp_dob) /365)
		...
		from fb_param_detail 
		where pd_doc_type in ('CHILDAGE','ADOLEAGE') and
				 to_date(:ld_dt,'dd/mm/yyyy') between PD_PERIOD_FROM and nvl(PD_PERIOD_TO,trunc(sysdate));
		...
		if sqlca.sqlcode = -1 then
 		...
 		if gs_garden_snm = 'FB' or gs_garden_snm = 'MK' or gs_garden_snm = 'ME' or gs_garden_snm = 'KG' or gs_garden_snm = 'BE' or gs_garden_snm = 'MV' or gs_garden_snm = 'SP' then
			If ld_labage <= ll_child Then //(144 months=12 years)
				...
				where TASK_EMPTYPE = :ls_emptype and TASK_PAYBOOK = :ll_paybook and TASK_ID = :ls_kam_id and to_date(:ld_dt,'dd/mm/yyyy') between TASK_DT_FROM and TASK_DT_TO and TASK_TYPE = 'CHILD' ;
			...
			elseIf ld_labage > ll_child And ld_labage <= ll_adoleage Then
				...
				into :ld_rtcp1,:ld_rtcp2,:ld_rtcp3,:ld_rtlo1,:ld_rtlo2,:ld_rtlo3,:ld_rtup1,:ld_rtup2,:ld_rtup3
				from fb_task_fulbari 
				where TASK_EMPTYPE = :ls_emptype and TASK_PAYBOOK = :ll_paybook and TASK_ID = :ls_kam_id and to_date(:ld_dt,'dd/mm/yyyy') between TASK_DT_FROM and TASK_DT_TO and TASK_TYPE = 'ADOLE' ;
				if sqlca.sqlcode = -1 then
					messagebox('Error : While Getting Labour Task Rate Detail',sqlca.sqlerrtext)
					rollback using sqlca;
					return 1
			...
			elseIf ld_labage > ll_adoleage then
				...
				into :ld_rtcp1,:ld_rtcp2,:ld_rtcp3,:ld_rtlo1,:ld_rtlo2,:ld_rtlo3,:ld_rtup1,:ld_rtup2,:ld_rtup3
				from fb_task_fulbari 
				where TASK_EMPTYPE = :ls_emptype and TASK_PAYBOOK = :ll_paybook and TASK_ID = :ls_kam_id and to_date(:ld_dt,'dd/mm/yyyy') between TASK_DT_FROM and TASK_DT_TO and TASK_TYPE = 'ADULT'  ;
				if sqlca.sqlcode = -1 then
					messagebox('Error : While Getting Labour Task Rate Detail',sqlca.sqlerrtext)
					rollback using sqlca;
					return 1
			...
			If ld_labage <= ll_child Then //(144 months=12 years)
				...
				into :ld_rtcp1,:ld_rtcp2,:ld_rtcp3,:ld_rtlo1,:ld_rtlo2,:ld_rtlo3,:ld_rtup1,:ld_rtup2,:ld_rtup3
				from fb_task 
				where TASK_ID = :ls_kam_id and to_date(:ld_dt,'dd/mm/yyyy') between TASK_DT_FROM and TASK_DT_TO and TASK_TYPE = 'CHILD'; //and
			...
			elseIf ld_labage > ll_child And ld_labage <= ll_adoleage Then
				...
				into :ld_rtcp1,:ld_rtcp2,:ld_rtcp3,:ld_rtlo1,:ld_rtlo2,:ld_rtlo3,:ld_rtup1,:ld_rtup2,:ld_rtup3
				from fb_task 
				where TASK_ID = :ls_kam_id and to_date(:ld_dt,'dd/mm/yyyy') between TASK_DT_FROM and TASK_DT_TO and TASK_TYPE = 'ADOLE';// and
						//(:gs_garden_snm <> 'MT' or (:gs_garden_snm ='MT' and nvl(task_fieldid,'N') = nvl(:ls_div,'N') and nvl(task_pruntype,'N') = decode(:ls_JUNGLIFOOT_ind,'J','UNPRUN','F','PRUN','N'))) ;
				if sqlca.sqlcode = -1 then
					messagebox('Error : While Getting Labour Task Rate Detail',sqlca.sqlerrtext)
					rollback using sqlca;
			...
			elseIf ld_labage > ll_adoleage then
				...
				into :ld_rtcp1,:ld_rtcp2,:ld_rtcp3,:ld_rtlo1,:ld_rtlo2,:ld_rtlo3,:ld_rtup1,:ld_rtup2,:ld_rtup3
				from fb_task 
				where TASK_ID = :ls_kam_id and to_date(:ld_dt,'dd/mm/yyyy') between TASK_DT_FROM and TASK_DT_TO and TASK_TYPE = 'ADULT';// and
						//(:gs_garden_snm <> 'MT' or (:gs_garden_snm ='MT' and nvl(task_fieldid,'N') = nvl(:ls_div,'N') and nvl(task_pruntype,'N') = decode(:ls_JUNGLIFOOT_ind,'J','UNPRUN','F','PRUN','N'))) ;
				if sqlca.sqlcode = -1 then
					messagebox('Error : While Getting Labour Task Rate Detail',sqlca.sqlerrtext)
					rollback using sqlca;
		...
		ld_wagrtn = wf_kamjari_rate(datetime(ld_dt),ls_labour_id,ls_kam_id,ld_status1,fl_row)
...
if gs_garden_snm='MT' then		
	select PH_HOURS into :ld_prorata_hours from fb_prorata_hours where PH_DATE = to_date(:ld_dt,'dd/mm/yyyy');
...
if gs_garden_snm = 'FB' then
	...
	if not isnull(ls_sec1) or len(ls_sec1) > 0  then
		select max(nvl(SPR_ROUND,0)) into :ll_round from fb_sectionpluckinground where SECTION_ID = :ls_sec1 and trunc(SPR_DATE) = to_date(:ld_dt,'dd/mm/yyyy');
		if sqlca.sqlcode = -1 then
...
elseif ld_rtcp3 > 0 then	
	...
	elseif ld_measure < (ld_rtcp1 - (ld_rtcp2 - ld_rtcp1) - (ld_rtcp3 - ld_rtcp2)) then
		...
		elseif gs_garden_snm='FB' and cbx_4.checked = false and ls_daily_rate = 'Y' then
			...
			where trim(kam.kamsub_id) = :ls_kam_id and to_date(:ld_dt,'dd/mm/yyyy') between KAMSUB_FRDT and nvl(KAMSUB_TODT,sysdate) ;
			...
			where INC_REC_TYPE = 'K' and KAMSUB_ID = :ls_kam_id and to_date(:ld_dt,'dd/mm/yyyy') between trunc(INC_FRDT) and nvl(trunc(INC_TODT),trunc(sysdate));
			if sqlca.sqlcode = -1 then
...
on w_gtelaf015_07102024.create
...
on w_gtelaf015_07102024.destroy
destroy(this.cb_9)
destroy(this.cb_8)
destroy(this.cb_7)
destroy(this.cbx_4)
...
select initcap(LS_MANID)||' {'||lpad(to_char(ls_id),2,'0')||'}' from fb_laboursheet 
 where ls_id in (select distinct ls_id from fb_employee e where e.emp_active='1') AND 
		 ...
		 ls_id not in (select ls_id from fb_laboursheetattendance lsa,fb_labourwagesweek lww 
		  				 where lsa.lww_id=lww.lww_id  and trunc(lsa.lsa_date) = to_date(:ld_dt,'dd/mm/yyyy') and 
...
IF MessageBox("Absent ALL  Alert", 'Do You Want To Absent ALL ....?' ,Exclamation!, YesNo!, 2) = 1 THEN
	...
	select distinct 'X' into :ls_temp from fb_mobile_attendance where trunc(lma_date) = to_date(:ld_dt,'dd/mm/yyyy') and not exists (select distinct 'x' from fb_mobile_attendance where trunc(lma_date) = to_date(:ld_dt,'dd/mm/yyyy') and nvl(lma_pst_ind,'N') = 'Y' );
	if sqlca.sqlcode = 0 then
	...
	where to_date(:ld_dt,'dd/mm/yyyy') between trunc(lww_startdate) and trunc(lww_enddate) and lww_paidflag='0';
...
IF MessageBox(" Alert", 'Do You Want To Set LWW For ALL ....?' ,Exclamation!, YesNo!, 2) = 1 THEN
	...
	from fb_param_detail 
	where pd_doc_type in ('CHILDAGE','ADOLEAGE') and to_date(:ld_dt,'dd/mm/yyyy') between PD_PERIOD_FROM and nvl(PD_PERIOD_TO,trunc(sysdate));
	...
	where nvl(EMP_ACTIVE,'1') = '1' and emp_type in ('LP','LT','LO') and LS_ID = decode(nvl(:ll_paybook,0),0,LS_ID,:ll_paybook) and 
			not exists (select distinct labour_id from fb_labourdailyattendance b where trunc(lda_date) = to_date(:ld_dt,'dd/mm/yyyy') and b.labour_id = a.emp_id)
	order by emp_id;
...
//			ls_cam_ind,ls_emp_ty,ls_sex
			...
			select distinct 'x' into :ls_temp from fb_maternity	where mt_empid = :ls_labour_id and to_date(:ld_dt,'dd/mm/yyyy') between trunc(mt_fromdt) and trunc(mt_todt);
			if sqlca.sqlcode = -1 then
				messagebox('Error : While Getting Employee Details',sqlca.sqlerrtext)
				rollback using sqlca;
				return 1
			elseif sqlca.sqlcode = 0 then
				...
				ld_wages = wf_kamjari_rate(datetime(ld_dt),ls_labour_id,ls_kam_id,1,0)
...
//				end if		
	...
	select distinct 'x' into :ls_temp from fb_laboursheetattendance where LWW_ID = :ls_lwwid and trunc(LSA_DATE) = to_date(:ld_dt,'dd/mm/yyyy') and LS_ID = decode(nvl(:ll_paybook,0),0,LS_ID,:ll_paybook);
	if sqlca.sqlcode = -1 then
	...
	elseif sqlca.sqlcode = 0 then
		...
		where LWW_ID = :ls_lwwid and LSA_DATE = to_date(:ld_dt,'dd/mm/yyyy') and LS_ID = decode(nvl(:ll_paybook,0),0,LS_ID,:ll_paybook);
		if sqlca.sqlcode = -1 then
	...
	elseif sqlca.sqlcode = 100 then
		...
		select :ls_lwwid,LS_ID,'0',to_date(:ld_dt,'dd/mm/yyyy'),'0'  from fb_laboursheet
...
//	order by emp_id;
	...
	select EMP_ID, initcap(emp_name) emp_name,FIELD_ID, get_diff(to_date(:ld_dt,'dd/mm/yyyy'),emp_dob,'B') , emp_type, emp_sex
	  from fb_employee a 
	where nvl(EMP_ACTIVE,'1') = '1' and emp_type in ('LP','LT','LO') and LS_ID = decode(nvl(:ll_paybook,0),0,LS_ID,:ll_paybook) and 
			not exists (select distinct labour_id from fb_labourdailyattendance b where trunc(lda_date) = to_date(:ld_dt,'dd/mm/yyyy') and b.labour_id = a.emp_id)
	order by emp_id;
...
//
				...
				insert into fb_labourdailyattendance(LDA_DATE, LABOUR_ID, KAMSUB_ID, LDA_WAGES, LDA_STATUS, JOB_ID, 
																lda_sectionid1,LWW_ID, LDA_ELP, LDA_ENTRY_BY, LDA_ENTRY_DT)
				values(to_date(:ld_dt,'dd/mm/yyyy'),:ls_labour_id,:ls_kam_id,:ld_wages,1,:ls_tmp_id,:ls_sec1,:ls_lwwid,0,:gs_user,sysdate);
...
//				end if		
				...
				insert into fb_labourdailyattendance(LDA_DATE, LABOUR_ID, KAMSUB_ID, LDA_WAGES, LDA_STATUS, JOB_ID, 
																lda_sectionid1,LWW_ID, LDA_ELP, LDA_ENTRY_BY, LDA_ENTRY_DT)
				values(to_date(:ld_dt,'dd/mm/yyyy'),:ls_labour_id,:ls_kam_id,0,0,:ls_tmp_id,:ls_sec1,:ls_lwwid,0,:gs_user,sysdate);
...
ld_dt = string(dp_1.value,'dd/mm/yyyy')
...
//	end if	
	...
	if gs_garden_snm = 'FB' or gs_garden_snm = 'MK' or gs_garden_snm = 'ME' or gs_garden_snm = 'KG' or gs_garden_snm = 'BE' or gs_garden_snm = 'MV' or gs_garden_snm = 'SP' then
		DECLARE c1 CURSOR FOR  
		select EMP_ID, emp_name,task_ind from fb_employee a,(select distinct TASK_EMPTYPE, TASK_PAYBOOK, TASK_TYPE,'Y' task_ind from fb_task_fulbari where  to_date(:ld_dt,'dd/mm/yyyy') between TASK_DT_FROM and TASK_DT_TO) 
		where nvl(EMP_ACTIVE,'1') = '1' and emp_type in ('LP','LT','LO') and LS_ID = :ll_paybook and 
				not exists (select distinct labour_id from fb_labourdailyattendance b where trunc(lda_date) = to_date(:ld_dt,'dd/mm/yyyy') and b.labour_id = a.emp_id) and
				decode( sign((round(get_diff(to_date(:ld_dt,'dd/mm/yyyy'),emp_dob,'B') * 12)) - 144),1,decode( sign(get_diff(to_date(:ld_dt,'dd/mm/yyyy'),emp_dob,'B') - 216),1,'ADULT','ADOLE'),'CHILD') = TASK_TYPE(+) and emp_type = TASK_EMPTYPE (+) and ls_id = TASK_PAYBOOK(+)//decode( sign((round((( trunc(:ld_dt) - emp_dob) /365) * 12)) - 144),1,decode( sign(get_diff(trunc(:ld_dt),emp_dob,'B') - 216),1,'ADULT','ADOLE'),'CHILD') = TASK_TYPE(+) and emp_type = TASK_EMPTYPE (+) and ls_id = TASK_PAYBOOK(+)
		order by emp_id;
		...
		IF sqlca.sqlcode = 0 THEN
			...
			do while sqlca.sqlcode <> 100
				...
				dw_1.setitem(dw_1.getrow(),'lda_date',date(ld_dt))
	...
	elseif gs_garden_snm = 'MT' then
		DECLARE c2 CURSOR FOR  
		select EMP_ID, emp_name,task_ind, kam, jf, lda_nature from fb_employee a,(select distinct TASK_TYPE,'Y' task_ind from fb_task where to_date(:ld_dt,'dd/mm/yyyy') between TASK_DT_FROM and TASK_DT_TO),
		...
		where nvl(EMP_ACTIVE,'1') = '1' and emp_type in ('LP','LT','LO') and LS_ID = :ll_paybook and a.emp_id = c.labour_id (+) and
				not exists (select distinct b.labour_id from fb_labourdailyattendance b where trunc(lda_date) =to_date(:ld_dt,'dd/mm/yyyy') and b.labour_id = a.emp_id) //and
	...
	//			end if
		...
		select EMP_ID, emp_name,task_ind from fb_employee a,(select distinct TASK_TYPE,'Y' task_ind from fb_task where  to_date(:ld_dt,'dd/mm/yyyy') between TASK_DT_FROM and TASK_DT_TO) 
		where nvl(EMP_ACTIVE,'1') = '1' and emp_type in ('LP','LT','LO') and LS_ID = :ll_paybook and 
				not exists (select distinct labour_id from fb_labourdailyattendance b where trunc(lda_date) = to_date(:ld_dt,'dd/mm/yyyy') and b.labour_id = a.emp_id) and
				decode( sign((round(get_diff(to_date(:ld_dt,'dd/mm/yyyy'),emp_dob,'B') * 12)) - 144),1,decode( sign((round(get_diff(to_date(:ld_dt,'dd/mm/yyyy'),emp_dob,'B') * 12)) - 216),1,'ADULT','ADOLE'),'CHILD') = TASK_TYPE(+)
		order by emp_id;
		...
		IF sqlca.sqlcode = 0 THEN
			...
			do while sqlca.sqlcode <> 100
				...
				elseif sqlca.sqlcode = 0 then
					...
					ll_row = dw_1.getrow()
					dw_1.setitem(dw_1.getrow(),'kamsub_id',ls_kam_id)
					ld_wages = wf_kamjari_rate(datetime(ld_dt),ls_labour_id,ls_kam_id,1,ll_row)			
...
IF MessageBox("Save  Alert", 'Do You Want To Save ....?' ,Exclamation!, YesNo!, 1) = 1 THEN
	...
	if dw_1.rowcount() > 0 then
		for ll_ctr = 1 to dw_1.rowcount( ) 
			...
			ld_dt = dw_1.getitemstring(ll_ctr,'lda_date')
			ls_kam_id = dw_1.getitemstring(ll_ctr,'kamsub_id')
			ls_labour_id = dw_1.getitemstring(ll_ctr,'labour_id')
			
			select distinct 'x' into :ls_temp from fb_laboursheetattendance where LWW_ID = :ls_lwwid and trunc(LSA_DATE) = to_date(:ld_dt,'dd/mm/yyyy') and LS_ID = :ll_paybook;
			if sqlca.sqlcode = -1 then
				messagebox('Error : While getting detail',sqlca.sqlerrtext)
				rollback using sqlca;
				return 1
			elseif sqlca.sqlcode = 0 then
				update fb_laboursheetattendance set LSA_ATTENDANCEFLAG = '0', LSA_ATTENDANCECONFIRM = '0' 
				where LWW_ID = :ls_lwwid and LSA_DATE = to_date(:ld_dt,'dd/mm/yyyy') and LS_ID = :ll_paybook;
				if sqlca.sqlcode = -1 then
					messagebox('Error : While Updating table (fb_laboursheetattendance)',sqlca.sqlerrtext)
					rollback using sqlca;
					return 1
			...
			elseif sqlca.sqlcode = 100 then
				insert into fb_laboursheetattendance(LWW_ID, LS_ID, LSA_ATTENDANCEFLAG, LSA_DATE, LSA_ATTENDANCECONFIRM) 
				values(:ls_lwwid,:ll_paybook,'0',to_date(:ld_dt,'dd/mm/yyyy'),'0');
				if sqlca.sqlcode = -1 then
					messagebox('Error : While Inserting Record 2',sqlca.sqlerrtext)
					rollback using sqlca;
					return 1
			...
			if trim(ls_nkam_type) = 'OTHERS' then 
				...
				if ls_covid = 'Y' then
					update fb_employee set EMP_COVID_ACTIVE = 'N', EMP_COVID_ENDDT =  to_date(:ld_dt,'dd/mm/yyyy') where emp_id = :ls_labour_id and emp_covid_active = 'Y' AND EMP_COVID_DT <=  to_date(:ld_dt,'dd/mm/yyyy'); 
					if sqlca.sqlcode = -1 then
...
event itemchanged;if lb_query = false then
	if dwo.name = 'labour_id' then
		ls_labour_id = data
		ld_dt = dw_1.getitemstring(row,'lda_date')
		...
		select distinct 'x' into :ls_temp from fb_labourdailyattendance where lda_date =  to_date(:ld_dt,'dd/mm/yyyy') and labour_id = :ls_labour_id;
		if sqlca.sqlcode = -1 then
		...
		where mt_empid = :ls_labour_id and to_date(:ld_dt,'dd/mm/yyyy') between trunc(mt_fromdt) and trunc(mt_todt);
		if sqlca.sqlcode = -1 then
			messagebox('Error : While Getting Employee Details',sqlca.sqlerrtext)
			rollback using sqlca;
			return 1
		...
		elseif sqlca.sqlcode = 100 then	
			select nvl(INC_AMTPERDAY,0) into :ld_wages from fb_incentive 
			where INC_REC_TYPE = 'L' and KAMSUB_ID = :ls_labour_id and to_date(:ld_dt,'dd/mm/yyyy') between INC_FRDT and nvl(INC_TODT,trunc(sysdate));
			if sqlca.sqlcode = -1 then
				messagebox('Error : While Getting LWW Availed Details',sqlca.sqlerrtext)
				rollback using sqlca;
				return 1
			elseif sqlca.sqlcode = 100 then
				ld_wages = wf_kamjari_rate(datetime(ld_dt),ls_labour_id,ls_kam_id,ld_status,row)
	...
	if dwo.name = 'lda_taskind' then
		if data = 'N' then
			...
			elseif sqlca.sqlcode = 100 then	
				select nvl(INC_AMTPERDAY,0) into :ld_wages from fb_incentive 
				where INC_REC_TYPE = 'L' and KAMSUB_ID = :ls_labour_id and to_date(:ld_dt,'dd/mm/yyyy')  between trunc(INC_FRDT) and nvl(INC_TODT,trunc(sysdate));
				if sqlca.sqlcode = -1 then
					messagebox('Error : While Getting LWW Availed Details',sqlca.sqlerrtext)
					rollback using sqlca;
					return 1
	...
	if dwo.name = 'lda_mfj_junglifoot'  then
		...
		select FIELD_ID,get_diff(trunc(sysdate),trunc(EMP_DOB),'B'), ((to_date(:ld_dt,'dd/mm/yyyy')  - emp_dob) /365), emp_type, ls_id into :ls_div,:ll_year,:ld_labage, :ls_emptype, :ll_paybook  from fb_employee where emp_id = :ls_labour_id; //round((trunc(sysdate) - trunc(EMP_DOB))/365)
...
//		dwc_section3.retrieve()		
		ll_year = 0;
		ld_lwwopn = wf_lwwopening(ls_labour_id,ll_paybook,datetime(ld_dt))
	//	messagebox('opn',string(ld_lwwopn))
		...
		from fb_kamsubhead where KAMSUB_ID = :ls_kam_id and to_date(:ld_dt,'dd/mm/yyyy')  between trunc(KAMSUB_FRDT) and trunc(nvl(KAMSUB_TODT,sysdate));
		...
		if sqlca.sqlcode = -1 then
 		...
 		if gs_garden_snm = 'FB' or gs_garden_snm = 'MK' or gs_garden_snm = 'ME' or gs_garden_snm = 'KG' or gs_garden_snm = 'BE' or gs_garden_snm = 'MV' or gs_garden_snm = 'SP' then
			...
			elseIf ld_labage > ll_adoleage then
				select distinct 'x' into :ls_temp	from fb_task_fulbari 
				where TASK_EMPTYPE = :ls_emptype and TASK_PAYBOOK = :ll_paybook and TASK_ID = :ls_kam_id and to_date(:ld_dt,'dd/mm/yyyy')   between trunc(TASK_DT_FROM) and trunc(nvl(TASK_DT_TO,sysdate)) and TASK_TYPE = 'ADULT' ;
				if sqlca.sqlcode = -1 then
					messagebox('Error : While Getting Labour Task Rate Detail',sqlca.sqlerrtext)
					rollback using sqlca;
					return 1
			...
			If ld_labage <= ll_child Then //(144 months=12 years)
				select distinct 'x' into :ls_temp	from fb_task 
				where TASK_ID = :ls_kam_id and to_date(:ld_dt,'dd/mm/yyyy')  between trunc(TASK_DT_FROM) and trunc(nvl(TASK_DT_TO,sysdate)) and TASK_TYPE = 'CHILD'; //and
						//(:gs_garden_snm <> 'MT' or (:gs_garden_snm ='MT' and nvl(task_fieldid,'N') = nvl(:ls_div,'N') and nvl(task_pruntype,'N') = decode(:ls_JUNGLIFOOT_ind,'J','UNPRUN','F','PRUN','N')));
				if sqlca.sqlcode = -1 then
					messagebox('Error : While Getting Labour Task Rate Detail',sqlca.sqlerrtext)
					rollback using sqlca;
			...
			elseIf ld_labage > ll_child And ld_labage <= ll_adoleage Then
				select distinct 'x' into :ls_temp	from fb_task  
				where TASK_ID = :ls_kam_id and to_date(:ld_dt,'dd/mm/yyyy')   between trunc(TASK_DT_FROM) and trunc(nvl(TASK_DT_TO,sysdate)) and TASK_TYPE = 'ADOLE';// and
						//(:gs_garden_snm <> 'MT' or (:gs_garden_snm ='MT' and nvl(task_fieldid,'N') = nvl(:ls_div,'N') and nvl(task_pruntype,'N') = decode(:ls_JUNGLIFOOT_ind,'J','UNPRUN','F','PRUN','N'))) ;
				if sqlca.sqlcode = -1 then
					messagebox('Error : While Getting Labour Task Rate Detail',sqlca.sqlerrtext)
					rollback using sqlca;
			...
			elseIf ld_labage > ll_adoleage then
				select distinct 'x' into :ls_temp	from fb_task 
				where TASK_ID = :ls_kam_id and to_date(:ld_dt,'dd/mm/yyyy')   between trunc(TASK_DT_FROM) and trunc(nvl(TASK_DT_TO,sysdate)) and TASK_TYPE = 'ADULT';// and
						//(:gs_garden_snm <> 'MT' or (:gs_garden_snm ='MT' and nvl(task_fieldid,'N') = nvl(:ls_div,'N') and nvl(task_pruntype,'N') = decode(:ls_JUNGLIFOOT_ind,'J','UNPRUN','F','PRUN','N'))) ;
				if sqlca.sqlcode = -1 then
					messagebox('Error : While Getting Labour Task Rate Detail',sqlca.sqlerrtext)
					rollback using sqlca;
...
//			end if
			...
			if trim(ls_nkam_type) = 'SICKALLOWANCE' then
					select nvl(count(1),0) into :ld_sick from fb_labourdailyattendance where LABOUR_ID = :ls_labour_id and KAMSUB_ID = (select KAMSUB_ID from fb_kamsubhead where nvl(KAMSUB_ACTIVE_IND,'Y') = 'Y' and KAMSUB_NKAMTYPE = 'SICKALLOWANCE') and
											trunc(LDA_DATE) between to_date('01/01/'||to_char(sysdate,'YYYY'),'dd/mm/yyyy') and to_date(:ld_dt,'dd/mm/yyyy') ;
				if sqlca.sqlcode = -1 then
					messagebox('Error : While Getting LWW Availed Details',sqlca.sqlerrtext)
					rollback using sqlca;
					return 1
				...
				where pd_doc_type = 'SICKALLOWANCE' and to_date(:ld_dt,'dd/mm/yyyy')  between trunc(PD_PERIOD_FROM) and nvl(PD_PERIOD_TO,trunc(sysdate));
				...
				if ls_covid = 'Y' then
					...
					where pd_doc_type = 'COVIDSICKALLOW' and to_date(:ld_dt,'dd/mm/yyyy')  between trunc(PD_PERIOD_FROM) and nvl(PD_PERIOD_TO,trunc(sysdate));
					if sqlca.sqlcode = -1 then
	...
	if dwo.name = 'lda_daily_rateind' then
		...
		if (gs_garden_snm='FB' or gs_garden_snm='MT') and cbx_4.checked = false and data = 'Y' then
			...
			select kam.kamsub_afrate afrate into :ld_afrate from fb_kamsubhead kam
			where trim(kam.kamsub_id) = :ls_kam_id and to_date(:ld_dt,'dd/mm/yyyy')  between trunc(KAMSUB_FRDT) and trunc(nvl(KAMSUB_TODT,sysdate));
...
//		end if
	...
	if dwo.name = 'lda_mfj_count1' then		
		...
		ld_wagrtn = wf_kamjari_rate(datetime(ld_dt),ls_labour_id,ls_kam_id,ld_status,row)
		if gs_garden_snm <> 'MF' and gs_garden_snm <> 'GF' and (ls_kam_id = 'ESUB0163' or ls_kam_id = 'ESUB0434' or ls_kam_id = 'ESUB0435') then //= 'ESUB0163' then
			if len(ls_sec1) > 0  or not isnull(ls_sec1) then
				setnull(ls_temp)
				
				select distinct 'x' into :ls_temp	from fb_sectionpluckinground where trunc(SPR_DATE) = to_date(:ld_dt,'dd/mm/yyyy')  and section_id = :ls_sec1;
...
//		dw_1.setitem(row,'lda_sectionid3',ls_temp)
		...
		select FIELD_ID,get_diff(trunc(sysdate),trunc(EMP_DOB),'B'),get_diff(to_date(:ld_dt,'dd/mm/yyyy')  ,trunc(EMP_DOB),'B'), emp_type, ls_id into :ls_div,:ll_year,:ld_labage, :ls_emptype, :ll_paybook  from fb_employee where emp_id = :ls_labour_id; //round((trunc(sysdate) - trunc(EMP_DOB))/365), ((:ld_dt - emp_dob) /365)
...
//		  				 where trunc(lsa.lda_date) = to_date(:ls_dt,'dd/mm/yyyy'))
// order by to_number(ls_id);


//messagebox('Check','Date'+ ls_dt)

DECLARE c1 CURSOR FOR  
select distinct initcap(LS_MANID)||' {'||lpad(to_char(ls_id),2,'0')||'}' from fb_laboursheet ;
 

open c1;
messagebox('Check','sqlca.sqlcode '+ string(sqlca.sqlcode) + ' sqlca.sqlcode text' +string(sqlca.sqlerrtext))
IF sqlca.sqlcode = 0 THEN
	fetch c1 into :ls_paybook;
	
	do while sqlca.sqlcode <> 100
	//	messagebox('Check','paybook '+ string(ls_paybook) )
		ddlb_1.additem(string(ls_paybook))
		fetch c1 into :ls_paybook;
	loop
	close c1;
...
datetime value = DateTime(Date("2024-10-07"), Time("17:34:35.000000"))
integer textsize = -9
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = roman!
