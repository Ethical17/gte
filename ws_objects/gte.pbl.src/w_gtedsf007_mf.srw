$PBExportHeader$w_gtedsf007_mf.srw
forward
global type w_gtedsf007_mf from window
end type
type cb_10 from commandbutton within w_gtedsf007_mf
end type
type cb_6 from commandbutton within w_gtedsf007_mf
end type
type cb_5 from commandbutton within w_gtedsf007_mf
end type
type cb_7 from commandbutton within w_gtedsf007_mf
end type
type cb_8 from commandbutton within w_gtedsf007_mf
end type
type cb_9 from commandbutton within w_gtedsf007_mf
end type
type dw_2 from datawindow within w_gtedsf007_mf
end type
type cb_4 from commandbutton within w_gtedsf007_mf
end type
type cb_3 from commandbutton within w_gtedsf007_mf
end type
type cb_2 from commandbutton within w_gtedsf007_mf
end type
type cb_1 from commandbutton within w_gtedsf007_mf
end type
type dw_1 from datawindow within w_gtedsf007_mf
end type
end forward
...
global w_gtedsf007_mf w_gtedsf007_mf

type variables
long ll_ctr, ll_cnt,l_ctr,ix,ll_st_year, ll_end_year, ll_user_level,ll_ac_year,ll_gpno,ll_last,ll_stno,lledno,ll_nop,ll_season, ll_tmp,ll_pnop,ll_dnop,ll_fymm,ll_tino,ll_dlcno
string ls_temp,ls_del_ind,ls_tmp_id,ls_entry_user,ls_cons,ls_last,ls_count, ls_ref, ls_desc, ls_pinv, ls_dtpdid,ls_adv, ls_gpno,ls_waybill, ls_type,ls_iss_gstn,ls_rec_gstn, ls_tinvno,ls_wareid, ls_dlcno,ls_count1
string ls_bname, ls_badd, ls_cname, ls_cadd, ls_gradelist, ls_cnno, ls_cust, ls_brok,ls_lotno, ls_iss_locn, ls_rec_locn,ls_hsn_cd, ls_tcs_ind, ls_inc_gst, ls_pan, ls_dt1
boolean lb_neworder, lb_query
double ld_gwt,ld_trwt,ld_nwt,ld_tgwt,ld_tnwt, ld_insval, ld_insrate,ld_totalnwt,ld_advqty,ld_hesc,ld_cess,ld_edcess,ld_adex,ld_toll,ld_igst_per,ld_cgst_per,ld_sgst_per, ld_rate
datetime ld_condt, ld_invdt,ld_bill_dt
double ld_netamt, ld_tcgst, ld_tsgst, ld_tigst, ld_totfy_sale, ld_tcs_rate
...
public function integer wf_gradetotal (string fs_grade)
...
public function integer wf_check_fillcol (integer fl_row);if dw_2.rowcount() > 0 and fl_row > 0 then
		if (isnull(dw_2.getitemstring(fl_row,'dtp_lotno')) or  len(dw_2.getitemstring(fl_row,'dtp_lotno'))=0 or &
		    isnull(dw_2.getitemnumber(fl_row,'sid_rate')) or  dw_2.getitemnumber(fl_row,'sid_rate') =0) then
			 messagebox('Warning: One Of The Following Fields Are Blank','Packing Invoice No and Rate, Please Check !!!')
			 return -1
		end if
end if
return 1
...
public function integer wf_gradetotal (string fs_grade);long fl_row
string ls_grade1
...
integer height = 1432
integer taborder = 40
string dataobject = "dw_gtedsf007a_mf"
...
if dwo.name = 'dtp_lotno' then
	...
	if(gs_garden_snm='MV' and match(ls_pinv,'^[P]')=true) then
		messagebox('Warning!','You cannot Select Pouch !!!')
		return 1
	end if
	...
	if  wf_check_duplicate_rec(ls_pinv) = -1 then return 1
		...
		if dw_1.getitemstring(dw_1.getrow(),'si_type') = 'PRICONS' then
			...
			elseif sqlca.sqlcode = 0 then
				...
				if wf_gradetotal(ls_tmp_id) > (ld_advqty + ld_toll) then
...
if dwo.name = 'sid_srnostart' then
	ll_stno = long(data)
//	ls_adv = dw_1.getitemstring(dw_1.getrow(),'advice_id')
	ls_tmp_id = dw_2.getitemstring(row,'tmp_id')
	ls_pinv = dw_2.getitemstring(row,'dtp_lotno')
	lledno = dw_2.getitemnumber(row,'sid_srnoend')
	ld_gwt = dw_2.getitemnumber(row,'dtpd_indwt') 
	ld_trwt = dw_2.getitemnumber(row,'dtpd_chestwt') 
	ld_nwt = dw_2.getitemnumber(row,'netwt')
	
	select distinct 'x' into :ls_temp
	from fb_dailyteapacked dtp,fb_dtpdetails dtpd 
	where dtp.dtp_id=dtpd.dtp_id and dtp.dtp_lotno = :ls_pinv and 
			dtpd.tmp_id = :ls_tmp_id and (:ll_stno between dtpd.dtpd_srnostart and dtpd.dtpd_srnoend and 
			:lledno between dtpd.dtpd_srnostart and dtpd.dtpd_srnoend);
	...
	if sqlca.sqlcode = -1 then
		...
		return 1
	elseif sqlca.sqlcode = 100 then
		messagebox('Warning !','Mark On Package From Not Exists In Packing Master, Please Check !!!')
		rollback using sqlca;
		return 1
//	elseif sqlca.sqlcode = 0 then
//		select distinct 'x' into :ls_temp	from fb_sidetails 
//		where dtpd_id=(select dtpd_id from fb_dailyteapacked dtp,fb_dtpdetails dtpd 
//								where dtp.dtp_id=dtpd.dtp_id and dtp.dtp_lotno= :ls_pinv and dtpd.tmp_id = :ls_tmp_id) and ((:ll_stno between sid_srnostart and sid_srnoend) or 
//										(:lledno between sid_srnostart and sid_srnoend ) or (sid_srnostart between :ll_stno and :lledno) or (sid_srnoend between :ll_stno and :lledno ));
//		if sqlca.sqlcode = -1 then
//			messagebox('Error : While Getting Start No Details',sqlca.sqlerrtext)
//			rollback using sqlca;
//			return 1
//		elseif sqlca.sqlcode = 0 then
//			messagebox('Warning !','Mark On Package From Already Despatched, Please Check !!!')
//			rollback using sqlca;
//			return 1
//		End if
	end if
	ll_nop = (lledno - ll_stno) + 1
	ld_tgwt = ll_nop * ld_gwt
	ld_tnwt = ll_nop * ld_nwt 
...
if dwo.name = 'sid_srnoend' then
	lledno = long(data)
//	ls_adv = dw_1.getitemstring(dw_1.getrow(),'advice_id')
	ls_tmp_id = dw_2.getitemstring(row,'tmp_id')
	ls_pinv = dw_2.getitemstring(row,'dtp_lotno')
	ll_stno = dw_2.getitemnumber(row,'sid_srnostart')
	ld_gwt = dw_2.getitemnumber(row,'dtpd_indwt') 
	ld_trwt = dw_2.getitemnumber(row,'dtpd_chestwt') 
	ld_nwt = dw_2.getitemnumber(row,'netwt')
	...
	if sqlca.sqlcode = -1 then
		...
		return 1
	elseif sqlca.sqlcode = 100 then
		messagebox('Warning !','Mark On Package To Not Exists In Packing Master, Please Check !!!')
		rollback using sqlca;
		return 1
	elseif sqlca.sqlcode = 0 then
		select distinct 'x' into :ls_temp	from fb_sidetails 
		...
		if sqlca.sqlcode = -1 then
			...
			return 1
		elseif sqlca.sqlcode = 0 then
			messagebox('Warning !','Mark On Package To Already Despatched, Please Check !!!')
			rollback using sqlca;
			return 1
		End if
	end if
...
event clicked;if lb_query = false then
	
CHOOSE CASE dw_2.getclickedcolumn()
	CASE 15
		if not isnull(dw_2.getitemstring(row,'dtp_lotno')) or len(dw_2.getitemstring(row,'dtp_lotno')) > 0  then
		ls_dtpdid = dw_2.getitemstring(row,'dtpd_id')
		openwithparm(w_gtedsf007a_mf,ls_dtpdid)
		end if
END CHOOSE
end if
end event
...
//	end if
	...
	if dw_2.rowcount() > 0 then
		for ll_ctr = 1 to dw_2.rowcount( ) 
			...
			ls_dtpdid = dw_2.getitemstring(ll_ctr,'dtpd_id')
			ll_nop = dw_2.getitemnumber(ll_ctr,'nop')
			...
			select ((nvl(DTPD_SRNOEND,0) - nvl(DTPD_SRNOSTART,0)) + 1) into :ll_pnop
			from fb_dailyteapacked a,fb_dtpdetails b where a.DTP_ID = b.DTP_ID and DTP_LOTNO = :ls_lotno;
			if sqlca.sqlcode = -1 then
				messagebox('Error : While Getting Packing Detail',sqlca.sqlerrtext)
				return 1
			end if			
			
			select sum((nvl(SID_SRNOEND,0) - nvl(SID_SRNOSTART,0)) + 1) into :ll_dnop	from fb_sidetails  where  dtpd_id = :ls_dtpdid;		
			if sqlca.sqlcode = -1 then
				messagebox('Error : While Getting Dispatch Detail',sqlca.sqlerrtext)
				return 1
			end if			
			
			if isnull(ll_pnop) then ll_pnop = 0;
			if isnull(ll_dnop) then ll_dnop = 0;
			if isnull(ll_nop) then ll_nop = 0;
			
			if ll_pnop = (ll_nop + ll_dnop) then
...
//				end if
//					
//				ll_tino = f_get_lastno('DC',string(ll_fymm))
//				
//				if ll_tino < 0 then
//					messagebox('Error At Last No Getting',sqlca.sqlerrtext)
//					return 1
...
//			if ll_tino >= 0 then
//				ls_count = string(ll_tino,'0000')
//				ls_tinvno =gs_garden_snm+"/DC/"+string(ll_fymm)+"-"+ls_count					
//				dw_1.setitem(dw_1.getrow(),'SI_TAXINVNO',ls_tinvno)	
//			end if 
//			
//		end if
...
integer height = 1084
integer taborder = 30
string dataobject = "dw_gtedsf007_mf"
end type
...
//		end if
	...
	if dwo.name = 'cus_id'  then
		...
		select CUS_GSTN_STCD,CUS_GSTNNO, nvl(CUS_TCS_IND,'N'), CUS_PANNO  into :ls_rec_locn,:ls_rec_gstn, :ls_tcs_ind, :ls_pan from fb_customer  where CUS_ID = :ls_cust;
