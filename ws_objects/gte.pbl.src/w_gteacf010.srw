$PBExportHeader$w_gteacf010.srw
forward
global type w_gteacf010 from window
end type
type cb_6 from commandbutton within w_gteacf010
end type
type em_3 from editmask within w_gteacf010
end type
...
//if f_check_fin_yr(datetime(ls_ac_dt)) = -1 then;	return 1;end if;
	if isnull(ls_ac_dt) =  false then
		if luo_fames.wf_wages_to_account(ls_lwwid,ls_frdt,ls_todt,ls_ac_dt) = -1 then 
			rollback using sqlca;
			return 1;
		end if;
	end if
