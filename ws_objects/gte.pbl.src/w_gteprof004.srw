$PBExportHeader$w_gteprof004.srw
forward
global type w_gteprof004 from window
end type
type cb_4 from commandbutton within w_gteprof004
end type
type cb_3 from commandbutton within w_gteprof004
end type
type cb_2 from commandbutton within w_gteprof004
end type
type cb_1 from commandbutton within w_gteprof004
end type
type dw_1 from datawindow within w_gteprof004
end type
end forward
...
string title = "(w_gteprof004) Tea Category"
...
global w_gteprof004 w_gteprof004
...
public function integer wf_check_fillcol (integer fl_row);if dw_1.rowcount() > 0 and fl_row > 0 then
	if ( isnull(dw_1.getitemstring(fl_row,'tpc_manid')) or  len(dw_1.getitemstring(fl_row,'tpc_manid'))=0 or &
		 isnull(dw_1.getitemstring(fl_row,'tpc_name')) or  len(dw_1.getitemstring(fl_row,'tpc_name'))=0 or &
		 isnull(dw_1.getitemnumber(fl_row,'tpc_rank'))) then
	    messagebox('Warning: One Of The Following Fields Are Blank','Tea Made Category, Name, Rank, Please Check !!!')
		 return -1
	end if
end if
return 1
...
IF (isnull(dw_1.getitemstring(dw_1.rowcount(),'tpc_manid')) or len(dw_1.getitemstring(dw_1.rowcount(),'tpc_manid'))=0) THEN
...
IF MessageBox("Save  Alert", 'Do You Want To Save ....?' ,Exclamation!, YesNo!, 1) = 1 THEN
	...
	if lb_neworder = true then
		select nvl(MAX(TPC_ID),0) into :ls_last from fb_teamadeproductcategory;
	...
	if dw_1.rowcount() > 0 then
		for ll_ctr = 1 to dw_1.rowcount( ) 
			if lb_neworder = true then
				...
				ls_tmp_id = 'TPC'+ls_count
				dw_1.setitem(ll_ctr,'tpc_id',ls_tmp_id)
			end if
		next	
	end if
...
event clicked;if cb_2.text = "&Query" then
	...
	dw_1.reset()
	dw_1.settaborder('tpc_manid',10)
	lb_query = true
	dw_1.modify("datawindow.queryclear= yes")
	dw_1.Object.datawindow.querymode= 'yes'
	dw_1.SetFocus ()
	dw_1.setcolumn('tpc_manid')
	cb_2.text = "&Run"
	cb_1.enabled = false
	cb_3.enabled = false
else
...
if dw_1.rowcount() = 0 then
	...
	dw_1.setfocus()
	dw_1.setitem(dw_1.getrow(),'tpc_entry_by',gs_user)
	dw_1.setitem(dw_1.getrow(),'tpc_entry_dt',datetime(today()))
	dw_1.setcolumn('tpc_manid')
else
	IF wf_check_fillcol(dw_1.getrow()) = -1 THEN
		return 1
...
integer height = 2052
integer taborder = 30
string dataobject = "dw_gteprof004"
...
event key_down;if lb_query = false then
 if lb_neworder = true then	
	...
	if newrow = dw_1.rowcount() and dw_1.rowcount() > 1  then
		dw_1.setitem(newrow,'tpc_entry_by',gs_user)
		dw_1.setitem(newrow,'tpc_entry_dt',datetime(today()))
		dw_1.setcolumn('tpc_manid')
	end if
 end if
end if
...
event itemchanged;if lb_query = false then
	...
	if dwo.name = 'tpc_name'  then
		...
		if f_check_special_char(ls_name) = -1 then return 1
	end if
	
	if dwo.name = 'tpc_rank'  then
		ll_rank = long(data)
		if f_check_initial_space(string(ll_rank)) = -1 then return 1	
	
		select distinct 'x' into :ls_temp from fb_teamadeproductcategory 
		where tpc_rank = :ll_rank;
		
		if sqlca.sqlcode = -1 then
			messagebox('Error : While Rank Details',sqlca.sqlerrtext)
			rollback using sqlca;
			return 1
		elseif sqlca.sqlcode  = 0 then
	...
	dw_1.setitem(row,'tpc_entry_by',gs_user)
	dw_1.setitem(row,'tpc_entry_dt',datetime(today()))
...
SELECT COLUMN_ID,COLUMN_Name,DATA_TYPE FROM USER_TAB_COLUMNS WHERE TABLE_NAME = upper('fb_teamadeproductcategory');
close c1;
open c1;
if sqlca.sqlcode = -1 then
		...
		do while sqlca.sqlcode <> 100
			...
			if ls_old_value <> ls_new_value then
				insert into fb_log(table_name, Unique_key, U_ACTION, U_FIELD, U_OLDVALUE, U_NEWVALUE, U_USERCODE, U_DATE, U_COMMENT) values
										('fb_teamadeproductcategory',:ls_unique_id,'U',:ls_columnname,:ls_old_value,:ls_new_value,:gs_user,sysdate,'');
				if sqlca.sqlcode = -1 then
