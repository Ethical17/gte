$PBExportHeader$w_gtelaf018.srw
forward
global type w_gtelaf018 from window
end type
type cb_4 from commandbutton within w_gtelaf018
end type
type cb_3 from commandbutton within w_gtelaf018
end type
type cb_2 from commandbutton within w_gtelaf018
end type
type cb_1 from commandbutton within w_gtelaf018
end type
type dw_1 from datawindow within w_gtelaf018
end type
end forward
...
string title = "(w_gtelaf018) Eletricity Deduction"
...
global w_gtelaf018 w_gtelaf018

type variables
long ll_ctr, ll_cnt,l_ctr,ll_st_year, ll_end_year,ll_last,ll_user_level
...
IF (isnull(dw_1.getitemnumber(dw_1.rowcount(),'edg_id')) or dw_1.getitemnumber(dw_1.rowcount(),'edg_id') = 0 ) THEN
...
IF MessageBox("Save  Alert", 'Do You Want To Save ....?' ,Exclamation!, YesNo!, 1) = 1 THEN
	...
	if lb_neworder = true then
		select nvl(MAX(edg_ID),0) into :ll_last from fb_electricdedgroup;
	end if
	...
	if dw_1.rowcount() > 0 then
		for ll_ctr = 1 to dw_1.rowcount( ) 
			if lb_neworder = true then
				ll_last = ll_last + 1
				dw_1.setitem(ll_ctr,'edg_id',ll_last)
			end if
		next	
	end if
...
event clicked;if cb_2.text = "&Query" then
	...
	dw_1.SetFocus ()
	dw_1.setcolumn('edg_electricded')
	cb_2.text = "&Run"
	cb_1.enabled = false
	cb_3.enabled = false
else
...
if dw_1.rowcount() = 0 then
	...
	dw_1.setfocus()
	dw_1.setitem(dw_1.getrow(),'edg_entry_by',gs_user)
	dw_1.setitem(dw_1.getrow(),'edg_entry_dt',datetime(today()))
	dw_1.setcolumn('edg_electricded')
else
	IF wf_check_fillcol(dw_1.getrow()) = -1 THEN
		return 1
...
integer height = 2052
integer taborder = 30
string dataobject = "dw_gtelaf018"
...
event key_down;if lb_query = false then
 if lb_neworder = true then	
	...
	if newrow = dw_1.rowcount() and dw_1.rowcount() > 1  then
		dw_1.setitem(newrow,'edg_entry_by',gs_user)
		dw_1.setitem(newrow,'edg_entry_dt',datetime(today()))
		dw_1.setcolumn('edg_electricded')
	end if
 end if
end if
...
//		end if	
//	end if
	dw_1.setitem(row,'edg_entry_by',gs_user)
	dw_1.setitem(row,'edg_entry_dt',datetime(today()))
