$PBExportHeader$w_gtedsf018.srw
forward
global type w_gtedsf018 from window
end type
type ddlb_1 from dropdownlistbox within w_gtedsf018
end type
type st_2 from statictext within w_gtedsf018
end type
type cb_6 from commandbutton within w_gtedsf018
end type
type cb_5 from commandbutton within w_gtedsf018
end type
type cb_7 from commandbutton within w_gtedsf018
end type
type cb_8 from commandbutton within w_gtedsf018
end type
type cb_9 from commandbutton within w_gtedsf018
end type
type cb_4 from commandbutton within w_gtedsf018
end type
type cb_3 from commandbutton within w_gtedsf018
end type
type cb_2 from commandbutton within w_gtedsf018
end type
type cb_1 from commandbutton within w_gtedsf018
end type
type dw_1 from datawindow within w_gtedsf018
end type
type dw_2 from datawindow within w_gtedsf018
end type
end forward
...
string title = "(w_gtedsf018) Credit Note"
...
global w_gtedsf018 w_gtedsf018
...
string ls_bname, ls_badd, ls_cname, ls_cadd, ls_gradelist, ls_cnno, ls_cust, ls_brok,ls_lotno, ls_iss_locn, ls_rec_locn,ls_hsn_cd,ls_wareid, ls_lutno, ls_tcs_ind, ls_pan, ls_inc_gst, ls_dt1, ls_invno
...
double ld_SI_AMOCHARGED, ld_SI_EXCISEDUTY, ld_SI_CESS, ld_SI_INSURANCEVALUE,ld_SI_HSECESS, ld_SI_TCS_PER, ld_SI_TCS_AMT, ld_SI_EDUCATIONALCESS 
string ls_si_id, ls_TRANSP_ID, ls_CN,  ls_advice_id, ls_SI_DESC, ls_SI_ACTIVE, ls_SI_CONFIRMFLAG, ls_SI_TYPE, ls_SI_CENTRALEXCISENO,  ls_BROK_ID, ls_WARE_ID, ls_COMM_ID
string ls_CUS_ID, ls_SI_WAYBILLNO, ls_SI_ISS_LOCN, ls_SI_REC_LOCN, ls_SI_PONO, ld_SI_PODT, ls_SI_DCNO, ld_SI_DCDT
string ls_SI_SUPPLY_PLACE, ls_SI_TAXINVNO,ls_SI_DELVCHNO, ls_si_ship_to_add, ls_si_rev_chrg, ls_SI_LUT_NO, ls_si_einv_ind, ls_SI_ISS_GSTNNO, ls_SI_REC_GSTNNO
date ld_CNDATE,  ld_SI_PROMPTDT, ld_SI_SUPPLY_DT, ld_SI_DATE
string ls_dtp_lotno,ls_dtpd_id
long  ll_did_SRNOSTART, ll_did_SRNOEND
double ld_did_INDWT, ld_did_CHESTWT, ld_netwt, ld_GTOT, ld_ntot, ld_sid_cgst_per, ld_sid_cgst_amt, ld_sid_sgst_per,  ld_sid_sgst_amt, ld_sid_igst_per,  ld_sid_igst_amt, ld_SID_RATE, ld_NGAMT
end variables
...
event clicked;opensheetwithparm(w_gtedsr018g,this.tag,w_mdi,0,layered!)
end event
...
//	if ((dw_1.getitemstring(dw_1.getrow(),'cnh_type') = 'PRICONS' and (isnull(dw_1.getitemstring(dw_1.getrow(),'advice_id')) or len(dw_1.getitemstring(dw_1.getrow(),'advice_id')) = 0)) or &
//		isnull(dw_1.getitemstring(dw_1.getrow(),'transp_id')) or len(dw_1.getitemstring(dw_1.getrow(),'transp_id')) = 0 or &
//		isnull(dw_1.getitemstring(dw_1.getrow(),'cnh_cn')) or len(dw_1.getitemstring(dw_1.getrow(),'cnh_cn')) = 0 or &
//		isnull(dw_1.getitemdatetime(dw_1.getrow(),'cnh_cndate'))) then
//		messagebox('Warning:','One Of The Fields Are Blank : Advice ID, Transporter ID, Consignment No, Date !!!')
//		dw_1.setfocus()
//		dw_1.setcolumn('cnh_date')
//		return
//	elseif ((dw_1.getitemstring(dw_1.getrow(),'cnh_type') = 'AUCTION') and &
...
//			dw_1.setcolumn('cnh_date')
//			return
//	elseif ((dw_1.getitemstring(dw_1.getrow(),'cnh_type') = 'EXP' or dw_1.getitemstring(dw_1.getrow(),'cnh_type') = 'SEZ') and &
//			((isnull(dw_1.getitemstring(dw_1.getrow(),'cnh_lut_no')) or len(dw_1.getitemstring(dw_1.getrow(),'cnh_lut_no')) = 0)))  then
//			messagebox('Warning:','One Of The Fields Are Blank : LUT No (Incase Of Export / SEZ Sale) !!!')
//			dw_1.setfocus()
//			dw_1.setcolumn('cnh_date')
//			return	
//	end if
	//ld_invdt = dw_1.getitemdatetime(dw_1.getrow(),'cnh_date')
	//if f_check_fin_yr(dw_1.getitemdatetime(dw_1.getrow(),'cnh_date')) = -1 then return 1
...
//	end if
	ls_adv = dw_1.getitemstring(dw_1.getrow(),'advice_id')
	ls_waybill = dw_1.getitemstring(dw_1.getrow(),'cnh_waybillno')
	ld_invdt = dw_1.getitemdatetime(dw_1.getrow(),'cnh_date')
	ls_cnno = dw_1.getitemstring(dw_1.getrow(),'cnh_cn')
...
//			end if
			...
			update fb_dailyteapacked set dtp_dispind = null where trim(dtp_lotno) = :ls_lotno ;
			if sqlca.sqlcode = -1 then
				messagebox('Error : While UpdatingPacking Dispatch Indicator',sqlca.sqlerrtext)
				rollback using sqlca;
				return 1
			end if		
		next	
...
//
//			dw_1.setitem(dw_1.getrow(),'cnh_centralexciseno',string(ll_gpno))
//			if ll_gpno = -1 then 
//				rollback using sqlca;
//				return 1;
//			end if;	
...
//		end if
		...
		ls_iss_gstn = dw_1.getitemstring(dw_1.getrow(),'cnh_iss_gstnno')
		ls_rec_gstn = dw_1.getitemstring(dw_1.getrow(),'cnh_rec_gstnno')
...
//======
		if ls_iss_gstn <> ls_rec_gstn then		
			if ll_tino = 0 then
				ll_fymm  =  long(string(dw_1.getitemdatetime(dw_1.getrow(),'cnh_date'),'yyyymm'))
			...
			if ll_tino >= 0 then
				...
				dw_1.setitem(dw_1.getrow(),'cnh_TAXINVNO',ls_tinvno)	
			end if 
			
			if not isnull(ls_rec_gstn) and len(ls_rec_gstn) > 0 then
				declare p2 procedure for up_post_e_invoicing ('PSCRN',:ls_tinvno, 'GTEDSF018');
...
//			//	dw_1.setitem(dw_1.getrow(),'cnh_delvchno',ls_dlcno)	
//			end if 
...
//======
		...
		if ll_last=0 then
			select nvl(MAX(substr(cnh_ID,3,10)),0) into :ll_last from fb_creditnote_hdr;
		end if
		ll_last = ll_last + 1
		ls_tmp_id = 'CN'+string(ll_last,'0000000000')
		dw_1.setitem(dw_1.getrow(),'cnh_id',ls_tmp_id)
		for ix = 1 to dw_2.rowcount()
			dw_2.setitem(ix,'cnd_id',ls_tmp_id)
		next
	end if		
...
event clicked;if cb_2.text = "&Query" then
	...
	cb_9.enabled = false
	cb_1.enabled = false
	dw_1.settaborder('cnh_id',5)
	dw_1.modify("datawindow.queryclear= yes")
	dw_1.Object.datawindow.querymode= 'yes'
	dw_1.SetFocus ()
	dw_1.setcolumn('cnh_id')
	cb_2.text = "&Run"
	cb_3.enabled = false
else
	lb_query = false
...
ls_tinvno = ddlb_1.text
//
//if rb_1.checked = true then
//	ls_rebatetype = 'PRICE'
//	dw_2.settaborder('cnd_rebatepkgs', 0)
//elseif rb_2.checked then
//	ls_rebatetype = 'PACKAGES'
//	dw_2.settaborder('cnd_rebatepkgs', 10)
//END IF


		select  SI_ID, TRANSP_ID, SI_CN, SI_AMOCHARGED, SI_EXCISEDUTY, SI_CESS, SI_DATE, advice_id,
				SI_INSURANCEVALUE, SI_DESC, SI_ACTIVE, SI_CONFIRMFLAG, SI_TYPE, SI_EDUCATIONALCESS, 
				SI_CENTRALEXCISENO, SI_HSECESS, SI_CNDATE, BROK_ID, WARE_ID, COMM_ID, CUS_ID,SI_WAYBILLNO, 
				SI_ISS_LOCN, SI_REC_LOCN, SI_PONO, SI_PODT, SI_DCNO, SI_DCDT, SI_PROMPTDT, SI_SUPPLY_DT, 
				SI_SUPPLY_PLACE,SI_TAXINVNO,SI_DELVCHNO, si_ship_to_add,si_rev_chrg,SI_LUT_NO, SI_TCS_PER, SI_TCS_AMT, si_einv_ind, SI_ISS_GSTNNO, SI_REC_GSTNNO
		into :ls_si_id, :ls_TRANSP_ID, :ls_CN, :ld_SI_AMOCHARGED, :ld_SI_EXCISEDUTY, :ld_SI_CESS, :ld_SI_DATE, :ls_advice_id,
				:ld_SI_INSURANCEVALUE, :ls_SI_DESC, :ls_SI_ACTIVE, :ls_SI_CONFIRMFLAG, :ls_SI_TYPE, :ld_SI_EDUCATIONALCESS, 
...
//			end if
			...
			dw_1.setitem(dw_1.getrow(),'cnh_CN',ls_CN)
			dw_1.setitem(dw_1.getrow(),'cnh_CNDATE',ld_CNDATE)
			dw_1.setitem(dw_1.getrow(),'cnh_AMOCHARGED',ld_SI_AMOCHARGED)
			dw_1.setitem(dw_1.getrow(),'cnh_EXCISEDUTY',ld_SI_EXCISEDUTY)
			dw_1.setitem(dw_1.getrow(),'cnh_CESS',ld_SI_CESS)
			dw_1.setitem(dw_1.getrow(),'cnh_DATE',ld_SI_DATE)
			dw_1.setitem(dw_1.getrow(),'advice_id',ls_advice_id)
			dw_1.setitem(dw_1.getrow(),'cnh_INSURANCEVALUE',ld_SI_INSURANCEVALUE)
			dw_1.setitem(dw_1.getrow(),'cnh_DESC',ls_SI_DESC)
			dw_1.setitem(dw_1.getrow(),'cnh_ACTIVE',ls_SI_ACTIVE)
			dw_1.setitem(dw_1.getrow(),'cnh_CONFIRMFLAG',ls_SI_CONFIRMFLAG)
			dw_1.setitem(dw_1.getrow(),'cnh_TYPE',ls_SI_TYPE)
			dw_1.setitem(dw_1.getrow(),'cnh_EDUCATIONALCESS',ld_SI_EDUCATIONALCESS)			
			dw_1.setitem(dw_1.getrow(),'cnh_CENTRALEXCISENO',ls_SI_CENTRALEXCISENO)
			dw_1.setitem(dw_1.getrow(),'cnh_HSECESS',ld_SI_HSECESS)
			dw_1.setitem(dw_1.getrow(),'cnh_CNDATE',ld_CNDATE)
			dw_1.setitem(dw_1.getrow(),'BROK_ID',ls_BROK_ID)
			dw_1.setitem(dw_1.getrow(),'WARE_ID',ls_WARE_ID)
			dw_1.setitem(dw_1.getrow(),'COMM_ID',ls_COMM_ID)
			dw_1.setitem(dw_1.getrow(),'CUS_ID',ls_CUS_ID)
			dw_1.setitem(dw_1.getrow(),'cnh_WAYBILLNO',ls_SI_WAYBILLNO)
			dw_1.setitem(dw_1.getrow(),'cnh_ISS_LOCN',ls_SI_ISS_LOCN)
			dw_1.setitem(dw_1.getrow(),'cnh_REC_LOCN',ls_SI_REC_LOCN)
			dw_1.setitem(dw_1.getrow(),'cnh_PONO',ls_SI_PONO)
			dw_1.setitem(dw_1.getrow(),'cnh_PODT',ld_SI_PODT)
			dw_1.setitem(dw_1.getrow(),'cnh_DCNO',ls_SI_DCNO)
			dw_1.setitem(dw_1.getrow(),'cnh_DCDT',ld_SI_DCDT)
			dw_1.setitem(dw_1.getrow(),'cnh_PROMPTDT',ld_SI_PROMPTDT)
			dw_1.setitem(dw_1.getrow(),'cnh_SUPPLY_DT',ld_SI_SUPPLY_DT) 
			dw_1.setitem(dw_1.getrow(),'cnh_SUPPLY_PLACE',ls_SI_SUPPLY_PLACE)
			dw_1.setitem(dw_1.getrow(),'SI_TAXINVNO',ls_SI_TAXINVNO)	
			dw_1.setitem(dw_1.getrow(),'cnh_ship_to_add',ls_si_ship_to_add)
			dw_1.setitem(dw_1.getrow(),'cnh_rev_chrg',ls_si_rev_chrg)
			dw_1.setitem(dw_1.getrow(),'cnh_LUT_NO',ls_SI_LUT_NO)
			dw_1.setitem(dw_1.getrow(),'cnh_TCS_PER',ld_SI_TCS_PER)
			dw_1.setitem(dw_1.getrow(),'cnh_TCS_AMT',ld_SI_TCS_AMT)
			dw_1.setitem(dw_1.getrow(),'cnh_ISS_GSTNNO',ls_SI_ISS_GSTNNO)
			dw_1.setitem(dw_1.getrow(),'cnh_REC_GSTNNO',ls_SI_REC_GSTNNO)
			dw_1.setitem(dw_1.getrow(),'cnh_entry_by',gs_user)
			dw_1.setitem(dw_1.getrow(),'cnh_entry_dt',datetime(today()))
			dw_1.setitem(dw_1.getrow(),'cnh_date',datetime(today()))
			//dw_1.setitem(dw_1.getrow(),'cnh_rebatetype',ls_rebatetype)
			dw_1.setcolumn('cnh_date')			
				
	
			dw_2.reset()
			declare c1 cursor for
			select  DTP_LOTNO,SID_SEASON,a.DTPD_ID, SID_SRNOSTART, SID_SRNOEND, ((nvl(SID_SRNOEND,0) - nvl(SID_SRNOSTART,0)) +1) nop,
					TMP_ID,  (nvl(DTPD_INDWT,0) + nvl(DTPD_CHESTWT,0)) DTPD_INDWT, DTPD_CHESTWT, nvl(DTPD_INDWT,0) netwt,
					((nvl(DTPD_INDWT,0) + nvl(DTPD_CHESTWT,0)) * ((nvl(SID_SRNOEND,0) - nvl(SID_SRNOSTART,0)) +1)) gtot, 
					(nvl(DTPD_INDWT,0) * ((nvl(SID_SRNOEND,0) - nvl(SID_SRNOSTART,0)) +1)) ntot,
					SID_CGST_PER, SID_CGST_AMT, SID_SGST_PER, SID_SGST_AMT, SID_IGST_PER, SID_IGST_AMT, sid_rate,
					 (nvl(sid_rate,0) * nvl(DTPD_INDWT,0) * ((nvl(SID_SRNOEND,0) - nvl(SID_SRNOSTART,0)) +1)) ngamt
			from fb_sidetails a,fb_saleinvoice b, 
				 (select DTP_LOTNO,TMP_ID, DTPD_SRNOSTART, DTPD_SRNOEND, DTPD_INDWT, DTPD_CHESTWT,DTPD_ID 
				  from fb_dailyteapacked a,fb_dtpdetails b where a.DTP_ID = b.DTP_ID) c
			where a.SI_ID = b.SI_ID and a. DTPD_ID = c.DTPD_ID and b.si_taxinvno = :ls_tinvno 
					and a.DTPD_ID not in (select DTPD_ID from fb_creditnote_hdr b,fb_creditnote_det c where  b.SI_TAXINVNO =:ls_tinvno and c.CND_ID=b.CNH_ID);
			open c1;
			
			IF sqlca.sqlcode = 0 THEN
				setnull(ls_dtp_lotno);setnull(ls_tmp_id);setnull(ls_invno); setnull(ls_dtpd_id); ll_nop = 0; 
				ld_did_INDWT = 0; ld_did_CHESTWT = 0; ld_netwt = 0; ld_GTOT = 0; ld_ntot = 0; ll_did_SRNOSTART = 0; ll_did_SRNOEND = 0; ll_SEASON = 0;
				ld_sid_cgst_per= 0; ld_sid_cgst_amt = 0; ld_sid_sgst_per= 0; ld_sid_sgst_amt= 0; ld_sid_igst_per= 0; ld_sid_igst_amt= 0; ld_SID_RATE= 0; ld_NGAMT = 0;
				
				fetch c1 into :ls_dtp_lotno,:ll_SEASON, :ls_dtpd_id, :ll_did_SRNOSTART, :ll_did_SRNOEND,:ll_nop, :ls_tmp_id, :ld_did_INDWT, :ld_did_CHESTWT, :ld_netwt, :ld_GTOT, :ld_ntot, 
								:ld_sid_cgst_per, :ld_sid_cgst_amt, :ld_sid_sgst_per,  :ld_sid_sgst_amt, :ld_sid_igst_per,  :ld_sid_igst_amt, :ld_SID_RATE, :ld_NGAMT ;
				
				do while sqlca.sqlcode <> 100
				
					dw_2.scrolltorow(dw_2.insertrow(0))
					dw_2.setitem(dw_2.getrow(),'dtp_lotno',ls_dtp_lotno)
					dw_2.setitem(dw_2.getrow(),'CND_SEASON',ll_SEASON)
					dw_2.setitem(dw_2.getrow(),'DTPD_ID',ls_dtpd_id)
					dw_2.setitem(dw_2.getrow(),'CND_SRNOSTART',ll_did_SRNOSTART)
					dw_2.setitem(dw_2.getrow(),'CND_SRNOEND',ll_did_SRNOEND)
					dw_2.setitem(dw_2.getrow(),'NOP',ll_nop)		
					dw_2.setitem(dw_2.getrow(),'TMP_ID',ls_tmp_id)
					dw_2.setitem(dw_2.getrow(),'DTPD_INDWT',ld_did_INDWT)
					dw_2.setitem(dw_2.getrow(),'DTPD_CHESTWT',ld_did_CHESTWT)
					dw_2.setitem(dw_2.getrow(),'NETWT',ld_netwt)
					dw_2.setitem(dw_2.getrow(),'GTOT',ld_GTOT)
					dw_2.setitem(dw_2.getrow(),'NTOT',ld_ntot)
					dw_2.setitem(dw_2.getrow(),'CND_CGST_PER',ld_sid_cgst_per)				
					dw_2.setitem(dw_2.getrow(),'CND_CGST_AMT',ld_sid_cgst_amt)
					dw_2.setitem(dw_2.getrow(),'CND_SGST_PER',ld_sid_sgst_per)
					dw_2.setitem(dw_2.getrow(),'CND_SGST_AMT',ld_sid_sgst_amt)
					dw_2.setitem(dw_2.getrow(),'CND_IGST_PER',ld_sid_igst_per)
					dw_2.setitem(dw_2.getrow(),'CND_IGST_AMT',ld_sid_igst_amt)
					dw_2.setitem(dw_2.getrow(),'CND_RATE',ld_SID_RATE)
					dw_2.setitem(dw_2.getrow(),'NGAMT',ld_NGAMT)

//					if rb_1.checked = true then
//					//	dw_2.settaborder('cnd_rebatepkgs', 0)
//						dw_2.setitem(dw_2.getrow(),'cnd_rebatepkgs',ll_DID_NOPKGS)
//						dw_2.setitem(dw_2.getrow(),'cnd_rebatetotwt',ld_DID_INVNETWT)
//					END IF
...
integer height = 1084
integer taborder = 30
string dataobject = "dw_gtedsf018"
end type
...
event key_down;if lb_query = false then
 if lb_neworder = true then	
	...
	if newrow = dw_1.rowcount() and dw_1.rowcount() > 1  then
		dw_1.setitem(newrow,'cnh_entry_by',gs_user)
		dw_1.setitem(newrow,'cnh_entry_dt',datetime(today()))
		dw_1.setcolumn('cnh_type')
	end if
 end if
end if
...
//			dw_1.setitem(row,'ware_id',ls_temp)
//			dw_1.setitem(row,'cnh_lut_no',ls_lutno)
////			idw_invno.SetFilter("") 
////			idw_invno.filter( )	
//		else
//			dw_1.setitem(row,'ware_id',ls_temp)
...
////		end if
//	else 
//		ld_invdt = dw_1.getitemdatetime(row,'cnh_date')
//	end if
//	
...
//		
//		//if ls_tcs_ind = 'Y' then
//			ld_bill_dt = dw_1.getitemdatetime(row,'cnh_date')
...
//		
//		dw_1.setitem(row,'cnh_rec_locn',ls_rec_locn)	
//		dw_1.setitem(row,'cnh_rec_gstnno',ls_rec_gstn)
//		dw_1.setitem(row,'cnh_tcs_per',ld_tcs_rate)	
//		
//		ls_iss_locn = dw_1.getitemstring(row,'cnh_iss_locn')
//		ls_rec_locn = dw_1.getitemstring(row,'cnh_rec_locn')	
//		
...
//	end if	
//	
////	if dwo.name = 'cnh_centralexciseno'  then
////		ls_gpno = trim(data)
////
////		select distinct 'x' into :ls_temp  from fb_saleinvoice where cnh_centralexciseno = :ls_gpno and 
////		decode(sign(to_number(to_char(cnh_date,'MM')) - 4),-1,to_number(to_char(cnh_date,'YYYY')) - 1,to_number(to_char(cnh_date,'YYYY'))) = decode(sign(to_number(to_char(:ld_invdt,'MM')) - 4),-1,to_number(to_char(:ld_invdt,'YYYY')) - 1,to_number(to_char(:ld_invdt,'YYYY')));
////		
////		if sqlca.sqlcode = -1 then
...
////			select max(to_number(cnh_centralexciseno)) into :ls_temp  from fb_saleinvoice where 
////				decode(sign(to_number(to_char(cnh_date,'MM')) - 4),-1,to_number(to_char(cnh_date,'YYYY')) - 1,to_number(to_char(cnh_date,'YYYY'))) = decode(sign(to_number(to_char(sysdate,'MM')) - 4),-1,to_number(to_char(sysdate,'YYYY')) - 1,to_number(to_char(sysdate,'YYYY')));
////				
////				if sqlca.sqlcode = -1 then
...
//			return 1
//		elseif sqlca.sqlcode = 0 then	
//			dw_1.setitem(row,'cnh_type',ls_temp)
...
//	//
	...
	if not isnull(dw_1.getitemstring(row,'cnh_taxinvno')) and dwo.name = 'cnh_einv_ind' and data = 'Y' then
		if MessageBox("Alert", 'Do You Want To Confirm For E-invoice ....?' ,Exclamation!, YesNo!, 1) = 1 then
			ls_tinvno = dw_1.getitemstring(row,'cnh_taxinvno')
	...
	dw_1.setitem(row,'cnh_entry_by',gs_user)
	dw_1.setitem(row,'cnh_entry_dt',datetime(today()))
...
event rowfocuschanged;if lb_query = false then
	if lb_neworder = false  then
		if dw_1.rowcount() > 0 then
			ls_ref = dw_1.getitemstring(dw_1.getrow(),'cnh_id')
			dw_2.reset()
...
integer height = 1200
integer taborder = 40
string dataobject = "dw_gtedsf018a"
...
//	END If
	if dw_2.rowcount() > 1 then
		dw_2.setitem(newrow,'cnd_id',dw_2.getitemstring(currentrow,'cnd_id'))
	end if
 end if
end if
...
if dwo.name = 'dtp_lotno' then
	...
	if  wf_check_duplicate_rec(ls_pinv) = -1 then return 1
		...
		dw_2.setitem(row,'cnd_season',ll_season)
		dw_2.setitem(row,'tmp_id',ls_tmp_id)
		dw_2.setitem(row,'cnd_srnostart',ll_stno)
		dw_2.setitem(row,'cnd_srnoend',lledno)
		...
		if dw_1.getitemstring(dw_1.getrow(),'cnh_type') = 'PRICONS' then
			select nvl(sa_type,'FWD') into :ls_type from fb_advice where advice_id = :ls_adv;
			if sqlca.sqlcode = -1 then
				messagebox('Error : While Getting Advice Quantity',sqlca.sqlerrtext)
				return 1
			...
			if ls_type = 'FWD' then
				...
				elseif sqlca.sqlcode = 0 then
					...
					if wf_gradetotal_fw(ls_tmp_id) > (ld_advqty + ld_toll) then
						...
						dw_2.setitem(row,'cnd_season',ll_tmp)
						dw_2.setitem(row,'tmp_id',ll_tmp)
						dw_2.setitem(row,'cnd_srnostart',ll_tmp)
						dw_2.setitem(row,'cnd_srnoend',ll_tmp)
	...
	//					return 1
		...
		end if //if dw_1.getitemstring(dw_1.getrow(),'cnh_type') = 'PRICONS'
	...
	elseif sqlca.sqlcode = 0 then
		...
		dw_1.setitem(dw_1.getrow(),'cnh_insurancevalue',ld_insval)
	end if
	// Insurance calc
	
	ld_rate = dw_2.getitemnumber(row,'cnd_rate')
	ls_cust = dw_1.getitemstring(dw_1.getrow(),'cus_id')
	ls_type = dw_1.getitemstring(dw_1.getrow(),'cnh_type')
	ls_iss_locn = dw_1.getitemstring(1,'cnh_iss_locn')
	ls_rec_locn = dw_1.getitemstring(1,'cnh_rec_locn')
	ld_invdt = dw_1.getitemdatetime(1,'cnh_date')
...
//		end if 
	...
	if ls_iss_gstn <> ls_rec_gstn then
		...
		if ld_igst_per > 0 then
			ld_cgst_per = 0
			ld_sgst_per = 0
			dw_2.setitem(row,'cnd_cgst_per',0)
			dw_2.setitem(row,'cnd_sgst_per',0)
			dw_2.setitem(row,'cnd_igst_amt',((ld_tnwt * ld_rate) *(ld_igst_per/100)))
			dw_2.setitem(row,'ngamt',((ld_tnwt * ld_rate))) 
			dw_2.setitem(row,'cnd_igst_per',ld_igst_per)
			dw_2.setitem(row,'cnd_cgst_amt',0) 
			dw_2.setitem(row,'cnd_sgst_amt',0) 
			if dw_1.getitemstring(dw_1.getrow(),'cnh_type') = 'EXP' or dw_1.getitemstring(dw_1.getrow(),'cnh_type') = 'SEZ' then
				dw_2.setitem(row,'cnd_igst_amt',0) 
				dw_2.setitem(row,'cnd_igst_per',0)
			end if
		elseif ld_cgst_per > 0 or ld_sgst_per > 0 then
			ld_igst_per = 0
			dw_2.setitem(row,'cnd_igst_per',0)
			dw_2.setitem(row,'cnd_cgst_amt',((ld_tnwt * ld_rate) *(ld_cgst_per/100))) 
			dw_2.setitem(row,'cnd_sgst_amt',((ld_tnwt * ld_rate) *(ld_sgst_per/100))) 
			dw_2.setitem(row,'ngamt',((ld_tnwt * ld_rate))) 
			dw_2.setitem(row,'cnd_cgst_per',ld_cgst_per) 
			dw_2.setitem(row,'cnd_sgst_per',ld_sgst_per) 				
			dw_2.setitem(row,'cnd_igst_amt',0) 			
			if dw_1.getitemstring(dw_1.getrow(),'cnh_type') = 'EXP' or dw_1.getitemstring(dw_1.getrow(),'cnh_type') = 'SEZ' then
				dw_2.setitem(row,'cnd_cgst_amt',0) 
				dw_2.setitem(row,'cnd_cgst_per',0)
