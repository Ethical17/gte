$PBExportHeader$w_gtelaf006.srw
forward
global type w_gtelaf006 from window
end type
type cb_4 from commandbutton within w_gtelaf006
end type
type cb_3 from commandbutton within w_gtelaf006
end type
type cb_2 from commandbutton within w_gtelaf006
end type
type cb_1 from commandbutton within w_gtelaf006
end type
type dw_1 from datawindow within w_gtelaf006
end type
end forward
...
string title = "(w_gtelaf006) Payment Period"
...
global w_gtelaf006 w_gtelaf006
...
IF (isnull(dw_1.getitemdatetime(dw_1.rowcount(),'lww_startdate')) ) THEN
...
IF MessageBox("Save  Alert", 'Do You Want To Save ....?' ,Exclamation!, YesNo!, 1) = 1 THEN
	...
	if lb_neworder = true then
		select nvl(MAX(lww_ID),0) into :ls_last from fb_labourwagesweek ;
	...
	if dw_1.rowcount() > 0 then
		for ll_ctr = 1 to dw_1.rowcount( ) 			
			ld_todt = dw_1.getitemdatetime(ll_ctr,'lww_enddate')
			ld_frdt = dw_1.getitemdatetime(ll_ctr,'lww_startdate')
			
			select distinct 'x' into :ls_temp from fb_labourwagesweek  where trunc(:ld_frdt) between trunc(lww_startdate) and trunc(lww_enddate);
			...
			select distinct 'x' into :ls_temp from fb_labourwagesweek  where trunc(lww_startdate) = trunc(:ld_frdt) and trunc(lww_enddate) = trunc(:ld_todt);
			
			if sqlca.sqlcode = -1 then
				messagebox('Error : While Getting Payment Period Details',sqlca.sqlerrtext)
				rollback using sqlca;
				return 1
			elseif sqlca.sqlcode  = 0 then
				messagebox('Warning!','Payment Period Details Already Exists, Please Check !!!')
				return 1
			end if
			...
			if lb_neworder = true then
				...
				ls_tmp_id = 'LWW'+ls_count
				dw_1.setitem(ll_ctr,'lww_id',ls_tmp_id)
			end if
...
select count(*) into :ll_temp from fb_labourwagesweek where nvl(LWW_PAIDFLAG,'0') = '0';

if sqlca.sqlcode = -1 then
	messagebox('Error : While Getting Open Payment Weeks',sqlca.sqlerrtext)
	rollback using sqlca;
	return 1
elseif sqlca.sqlcode  = 0 then
	if ll_temp > 1 then
		messagebox('Warning!','Two Weeks Payment Period Already Open, Please Check !!!')
		return 1
	end if
end if
...
integer height = 2052
integer taborder = 30
string dataobject = "dw_gtelaf006"
...
event key_down;if lb_query = false then
 if lb_neworder = true then	
	...
	if newrow = dw_1.rowcount() and dw_1.rowcount() > 1  then
		dw_1.setitem(newrow,'lww_entry_by',gs_user)
		dw_1.setitem(newrow,'lww_entry_dt',datetime(today()))
		dw_1.setcolumn('lww_startdate')
	end if
 end if
end if
...
event itemchanged;if lb_query = false then
	if dwo.name = 'lww_startdate'  then
		ld_frdt = datetime(data)
		ld_todt = dw_1.getitemdatetime(row,'lww_enddate')
...
//		end if
		...
		if gs_garden_snm <> 'MT' then
			dw_1.setitem(row,'lww_enddate',RelativeDate(date(ld_frdt), 6))
		end if
		
		if gs_garden_snm = 'KG' then
			//select (:ld_frdt) + 13 into :ld_todt from dual;
			dw_1.setitem(row,'lww_enddate',RelativeDate(date(ld_frdt), 13))
		end if		
		
	else
		ld_frdt = dw_1.getitemdatetime(row,'lww_startdate')
	end if
	
	if dwo.name = 'lww_enddate'  then
		...
		if gs_garden_snm = 'KG' then
			...
			if ll_cnt <> 13 then
				messagebox('Warning!','Difference Between Start Date & End Date Of Week Should Be 14, Please Check !!!')
				return 1
			end if
		end if		
...
//		end if
	...
	dw_1.setitem(row,'lww_entry_by',gs_user)
	dw_1.setitem(row,'lww_entry_dt',datetime(today()))
cb_3.enabled = true
end if
...
SELECT COLUMN_ID,COLUMN_Name FROM USER_TAB_COLUMNS WHERE TABLE_NAME = upper('fb_labourwagesweek');
...
elseIF sqlca.sqlcode = 0 THEN
	...
	fetch c1 into :ll_coumnid,:ls_columnname;
		...
		do while sqlca.sqlcode <> 100
			...
			ls_unique_id=dw_1.GetItemString(li_row, 3, Primary!, true)
					 
					 
					 if ls_old_value <> ls_new_value then
						insert into fb_log(table_name, Unique_key, U_ACTION, U_FIELD, U_OLDVALUE, U_NEWVALUE, U_USERCODE, U_DATE, U_COMMENT) values
												('fb_labourwagesweek',:ls_unique_id,'U',:ls_columnname,:ls_old_value,:ls_new_value,:gs_user,sysdate,'');
							if sqlca.sqlcode = -1 then
								messagebox("SQL Error : While Inserting Into Log Table ",SQLCA.SQLErrtext,Information!)
								rollback using sqlca;
								return -1 
