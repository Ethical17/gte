$PBExportHeader$w_gtelaf038.srw
forward
global type w_gtelaf038 from window
end type
type cb_4 from commandbutton within w_gtelaf038
end type
type cb_3 from commandbutton within w_gtelaf038
end type
type cb_2 from commandbutton within w_gtelaf038
end type
type cb_1 from commandbutton within w_gtelaf038
end type
type dw_1 from datawindow within w_gtelaf038
end type
end forward
...
string title = "(w_gtelaf038) Outsider Wage Rate"
...
global w_gtelaf038 w_gtelaf038
...
public function integer wf_check_duplicate_rec (string fs_id, datetime fd_dt)
end prototypes
...
public function integer wf_check_fillcol (integer fl_row);if dw_1.rowcount() > 0 and fl_row > 0 then
	if (isnull(dw_1.getitemstring(fl_row,'kamsub_id')) or  len(dw_1.getitemstring(fl_row,'kamsub_id'))=0 or &
		isnull(dw_1.getitemdatetime(fl_row,'kamsub_frdt')) or &
		 isnull(dw_1.getitemnumber(fl_row,'kamsub_afrate')) or  dw_1.getitemnumber(fl_row,'kamsub_afrate')=0 or &
		 isnull(dw_1.getitemnumber(fl_row,'kamsub_ahrate')) or  dw_1.getitemnumber(fl_row,'kamsub_ahrate')=0  ) then
	    messagebox('Warning: One Of The Following Fields Are Blank', 'Kamjari, From Date, Full & Half Rate, Please Check !!!')
		 return -1
	end if
end if
return 1
...
public function integer wf_check_duplicate_rec (string fs_id, datetime fd_dt);long fl_row
...
if dw_1.rowcount() > 1 then
	for fl_row = 1 to (dw_1.rowcount() - 1)
		...
		if ls_id1 = fs_id and  ld_dt1 = fd_dt then
...
IF MessageBox("Save  Alert", 'Do You Want To Save ....?' ,Exclamation!, YesNo!, 1) = 1 THEN
	...
	for ll_ctr = 1 to dw_1.rowcount( ) 
		...
		select distinct 'x' into :ls_temp from fb_fboutsiderwgrate
		 where kamsub_id = :ls_id and kamsub_frdt >= :ld_frdt and kamsub_todt is null;
		...
		elseif sqlca.sqlcode  = 0 then
			...
			return 1
			  ...
			  update fb_fboutsiderwgrate set kamsub_todt = (:ld_frdt - 1), kamsub_active_ind = 'N' 
				where kamsub_id = :ls_id and  kamsub_todt is null and kamsub_frdt < :ld_frdt;
...
integer height = 2052
integer taborder = 30
string dataobject = "dw_gtelaf038"
...
event itemchanged;if lb_query = false then
	if dwo.name = 'kamsub_id'  then
		...
		select distinct KAMSUB_AFRATE,  KAMSUB_AHRATE into :ld_afrt,:ld_ahrt from fb_fboutsiderwgrate 
		 where kamsub_id = :ls_id and  kamsub_todt is null;
	...
	if dwo.name = 'kamsub_frdt' then
		...
		if  wf_check_duplicate_rec(ls_id,ld_frdt) = -1 then return 1
...
SELECT COLUMN_ID,COLUMN_Name FROM USER_TAB_COLUMNS WHERE TABLE_NAME = upper('fb_fboutsiderwgrate');
			...
			ls_unique_id=dw_1.GetItemString(li_row, 1, Primary!, true)
					 ...
					 if ls_old_value <> ls_new_value then
						insert into fb_log(table_name, Unique_key, U_ACTION, U_FIELD, U_OLDVALUE, U_NEWVALUE, U_USERCODE, U_DATE, U_COMMENT) values
												('fb_fboutsiderwgrate',:ls_unique_id,'U',:ls_columnname,:ls_old_value,:ls_new_value,:gs_user,sysdate,'');
							if sqlca.sqlcode = -1 then
								messagebox("SQL Error : While Inserting Into Log Table ",SQLCA.SQLErrtext,Information!)
								rollback using sqlca;
								return -1 
