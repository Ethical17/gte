$PBExportHeader$w_gteflf020.srw
forward
global type w_gteflf020 from window
end type
type cb_4 from commandbutton within w_gteflf020
end type
type cb_1 from commandbutton within w_gteflf020
end type
type cb_3 from commandbutton within w_gteflf020
end type
type cb_2 from commandbutton within w_gteflf020
end type
type dw_1 from datawindow within w_gteflf020
end type
end forward
...
string title = "(w_gteflf020) Species Master"
...
global w_gteflf020 w_gteflf020
...
public function integer wf_check_duplicate_rec (string fs_desc, long rownum)
end prototypes
...
public function integer wf_check_fillcol (integer fl_row);if dw_1.rowcount() > 0 and fl_row > 0 then
	if (isnull(dw_1.getitemstring(fl_row,'sm_code')) or  len(dw_1.getitemstring(fl_row,'sm_code'))=0 or &
		isnull(dw_1.getitemstring(fl_row,'sm_description')) or  len(dw_1.getitemstring(fl_row,'sm_description'))=0 ) then
	    messagebox('Warning: One Of The Following Fields Are Blank','Code, Description, Please Check !!!')
		 return -1
	end if
end if
return 1
...
public function integer wf_check_duplicate_rec (string fs_desc, long rownum);long fl_row
string ls_desc1
...
if dw_1.rowcount() > 1 then
	for fl_row = 1 to (dw_1.rowcount())
		IF fl_row <> rownum then
			ls_desc1 = dw_1.getitemstring(fl_row,'sm_description')
			if upper(ls_desc1) = upper(fs_desc) then
				dw_1.SelectRow(fl_row, TRUE)
				messagebox("Warning ","Duplicate Record At Row : "+string(fl_row))
				return -1
			end if
		end if
	next
...
IF MessageBox("Save  Alert", 'Do You Want To Save ....?' ,Exclamation!, YesNo!, 1) = 1 THEN
	...
	ls_temp_desc = dw_1.getitemstring(dw_1.getrow(),'sm_description')
	IF wf_check_fillcol(dw_1.getrow()) = -1 or wf_check_duplicate_rec(ls_temp_desc,dw_1.getrow()) = -1 THEN
		return 1
	END IF

	
	if lb_neworder = true then
		select 'x' into :ls_temp from fb_species_master where upper(sm_description) = upper(:ls_temp_desc);
		if sqlca.sqlcode = -1 then
			messagebox('Error occured while checking existing records',sqlca.sqlerrtext)
			return 1
		elseif sqlca.sqlcode = 0 then
			messagebox('Warning','A record for this Description already exists in system')
			return 1
		end if
		
		select nvl(MAX(sm_id),0) into :ls_last from fb_species_master;
		ls_last = right(ls_last,4)
		ll_cnt = long(ls_last)
		for ll_ctr = 1 to dw_1.rowcount() 
			ll_cnt = ll_cnt + 1
			select lpad(:ll_cnt,4,'0') into :ls_count from dual;
			ls_sm_id = 'SM'+ls_count
			dw_1.setitem(ll_ctr,'sm_id',ls_sm_id)
		next
	end if
...
integer height = 2052
integer taborder = 30
string dataobject = "dw_gteflf020"
...
if lb_query = false then
 if lb_neworder = true then	
	if currentrow <> dw_1.rowcount() and dw_1.rowcount() > 1 then	
		 ls_temp_desc = dw_1.getitemstring(currentrow,'sm_description')
		IF wf_check_fillcol(currentrow) = -1 or wf_check_duplicate_rec(ls_temp_desc,currentrow) = -1 THEN
			return 1
		END IF
		select 'x' into :ls_temp from fb_species_master where upper(sm_description) = upper(:ls_temp_desc) ;
		if sqlca.sqlcode = -1 then
...
event itemchanged;if lb_query = false then
	if dwo.name = 'sm_desc' then
		select 'x' into :ls_temp from fb_species_master where upper(sm_description) = upper(:data) ;
		if sqlca.sqlcode = -1 then
			messagebox('Error occured while checking existing records',sqlca.sqlerrtext)
			return 1
		elseif sqlca.sqlcode = 0 then
...
SELECT COLUMN_ID,COLUMN_Name FROM USER_TAB_COLUMNS WHERE TABLE_NAME = upper('fb_species_master');
			...
			ls_unique_id=dw_1.GetItemString(li_row, 1, Primary!, true)
					 ...
					 if ls_old_value <> ls_new_value then
						insert into fb_log(table_name, Unique_key, U_ACTION, U_FIELD, U_OLDVALUE, U_NEWVALUE, U_USERCODE, U_DATE, U_COMMENT) values
												('fb_species_master',:ls_unique_id,'U',:ls_columnname,:ls_old_value,:ls_new_value,:gs_user,sysdate,'');
							if sqlca.sqlcode = -1 then
								messagebox("SQL Error : While Inserting Into Log Table ",SQLCA.SQLErrtext,Information!)
								rollback using sqlca;
								return -1 
