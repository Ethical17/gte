$PBExportHeader$w_gtelicf002.srw
forward
global type w_gtelicf002 from window
end type
type cb_4 from commandbutton within w_gtelicf002
end type
type cb_3 from commandbutton within w_gtelicf002
end type
type cb_2 from commandbutton within w_gtelicf002
end type
type cb_1 from commandbutton within w_gtelicf002
end type
type dw_1 from datawindow within w_gtelicf002
end type
end forward
...
string title = "(w_gtelicf001) License and Certificate Details"
...
global w_gtelicf002 w_gtelicf002
...
public function integer wf_check_fillcol (integer fl_row);if dw_1.rowcount() > 0 and fl_row > 0 then
	if (isnull(dw_1.getitemstring(fl_row,'ld_vehicle_no')) or  len(dw_1.getitemstring(fl_row,'ld_vehicle_no'))=0)  then
...
IF (isnull(dw_1.getitemstring(dw_1.rowcount(),'ld_vehicle_no'))) THEN
...
if cb_2.text = "&Query" then
	...
	//dw_1.settaborder('del_flag',50)
 	dw_1.TriggerEvent(rowfocuschanged!)
	dw_1.SetRedraw (TRUE)
	dw_1.settaborder('ld_vehicle_no',5)
	cb_2.text = "&Query"
	cb_1.enabled = true
end if
...
integer height = 2052
integer taborder = 30
string dataobject = "dw_gtelicf002"
...
event key_down;if lb_query = false then
 if lb_neworder = true then	
	...
	if newrow = dw_1.rowcount() and dw_1.rowcount() > 1  then
		...
		dw_1.setitem(newrow,'ld_issue_dt',datetime(today()))
		dw_1.setcolumn('ld_particular')
	end if
 end if
end if
...
event itemchanged;if lb_query = false then
	...
	if dwo.name = 'ld_licenseno'  then
		setnull(ls_licenseno)
		ls_licenseno = data
		select ld_licenseno into: ls_licensenotmp from fb_license_details where ld_licenseno = :ls_licenseno ;
		if sqlca.sqlcode = 0 then
			messagebox('Error', 'The license number you are entering already exists')
			return 1
		end if
	end if
	
	if  dwo.name = 'ld_particular'  then
		if f_check_string_sp_NUM(data) = -1 then
			return 1
		end if
	end if
	
	if  dwo.name = 'ld_issue_dt'  then
		ld_issue_tmp = date(data)
		ld_exp_tmp = date(dw_1.getitemdatetime(row, 'ld_exp_dt'))
		if ld_issue_tmp = ld_exp_tmp then
			messagebox('Warning', 'Issue date can not be equal to Expiry date')
			return 1
		end if
		if isnull(data) then
			messagebox('Warning', 'Invalid Issue Date')
			return 1
		end if
		if(date(data)>today()) then
			messagebox('Warning', 'Issue date can not be greater than today~'s date')
			return 1
		end if
	end if


	if  dwo.name = 'ld_exp_dt'  then
		ld_exp_tmp = date(data)
		ld_issue_tmp = date(dw_1.getitemdatetime(row, 'ld_issue_dt'))
		if ld_exp_tmp = ld_issue_tmp then
			messagebox('Warning', 'Expiry date can not be equal to issue date')
			return 1
			
		elseif  ld_exp_tmp < ld_issue_tmp then
			messagebox('Warning', 'Expiry date can not be earlier than issue date')
			return 1
		end if
	end if
...
//'Expiry date can not equal to issue date!!'
end event
...
SELECT COLUMN_ID,COLUMN_Name FROM USER_TAB_COLUMNS WHERE TABLE_NAME = upper('fb_license_details');
			...
			ls_unique_id=dw_1.GetItemString(li_row, 1, Primary!, true)
					 ...
					 if ls_old_value <> ls_new_value then
						insert into fb_log(table_name, Unique_key, U_ACTION, U_FIELD, U_OLDVALUE, U_NEWVALUE, U_USERCODE, U_DATE, U_COMMENT) values
												('fb_license_details',:ls_unique_id,'U',:ls_columnname,:ls_old_value,:ls_new_value,:gs_user,sysdate,'');
							if sqlca.sqlcode = -1 then
								messagebox("SQL Error : While Inserting Into Log Table ",SQLCA.SQLErrtext,Information!)
								rollback using sqlca;
								return -1 
