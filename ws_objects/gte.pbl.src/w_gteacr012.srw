$PBExportHeader$w_gteacr012.srw
forward
global type w_gteacr012 from window
end type
type em_1 from editmask within w_gteacr012
end type
type st_1 from statictext within w_gteacr012
end type
type cb_2 from commandbutton within w_gteacr012
end type
type cb_1 from commandbutton within w_gteacr012
end type
type dw_1 from datawindow within w_gteacr012
end type
end forward
...
string title = "(W_gteacr012) Projected Expenditure Summary"
...
global w_gteacr012 w_gteacr012
...
event clicked;string ls_todt,ls_ysdt,ls_ly_ysdt,ls_ly_yedt,ls_yedt

setpointer(hourglass!)
if len(em_1.text) <> 7 or isnull(em_1.text) then 
	messagebox('Error At Date','Year/Month Should Be Entered')
	return 1
else
	  ls_todt = left(em_1.text,4)+right(em_1.text,2)
end if;
 ...
 select to_char(to_date((decode(sign(to_number(to_char(to_date(:ls_todt,'yyyymm'),'mm')) - 4),-1,to_number(to_char(to_date(:ls_todt,'yyyymm'),'yyyy'))-1,to_number(to_char(to_date(:ls_todt,'yyyymm'),'yyyy')))||'0401'),'yyyymmdd'),'yyyymm'),
		 to_char(to_date((decode(sign(to_number(to_char(to_date(:ls_todt,'yyyymm'),'mm')) - 4),-1,to_number(to_char(to_date(:ls_todt,'yyyymm'),'yyyy')),(to_number(to_char(to_date(:ls_todt,'yyyymm'),'yyyy'))+1))||'0331'),'yyyymmdd'),'yyyymm'),
		...
		to_char(to_date((decode(sign(to_number(to_char(to_date(:ls_todt,'yyyymm'),'mm')) - 4),-1,to_number(to_char(to_date(:ls_todt,'yyyymm'),'yyyy'))-1,to_number(to_char(to_date(:ls_todt,'yyyymm'),'yyyy')))||'0331'),'yyyymmdd'),'yyyymm')
  into :ls_ysdt,:ls_yedt,:ls_ly_ysdt,:ls_ly_yedt from dual;
  
	dw_1.show()
	dw_1.settransobject(sqlca)
	dw_1.retrieve(ls_todt,ls_ysdt,ls_ly_ysdt,ls_ly_yedt,ls_yedt)
setpointer(arrow!)
end event
...
integer width = 4183
integer height = 2100
string dataobject = "dw_gteacr012"
boolean hscrollbar = true
boolean vscrollbar = true
