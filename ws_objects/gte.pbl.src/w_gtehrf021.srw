$PBExportHeader$w_gtehrf021.srw
forward
global type w_gtehrf021 from window
end type
type cbx_1 from checkbox within w_gtehrf021
end type
type cb_11 from commandbutton within w_gtehrf021
end type
type st_3 from statictext within w_gtehrf021
end type
type ddlb_2 from dropdownlistbox within w_gtehrf021
end type
type st_2 from statictext within w_gtehrf021
end type
type em_1 from editmask within w_gtehrf021
end type
type cb_7 from commandbutton within w_gtehrf021
end type
type cb_6 from commandbutton within w_gtehrf021
end type
type st_1 from statictext within w_gtehrf021
end type
type ddlb_1 from dropdownlistbox within w_gtehrf021
end type
type cb_4 from commandbutton within w_gtehrf021
end type
type cb_3 from commandbutton within w_gtehrf021
end type
type cb_2 from commandbutton within w_gtehrf021
end type
type dw_1 from datawindow within w_gtehrf021
end type
end forward
...
global w_gtehrf021 w_gtehrf021

type variables
double ld_basic,ld_hra,ld_da,ld_vda,ld_basic_old,ld_inc_rt,ld_inc_rt_old,ld_last_inc_rt,ld_stbasic,ld_electric,ld_crvfpf,ld_fuel,ld_last_incper, ld_incper
...
ddlb_1.additem('ALL');
	...
	select distinct EMP_GRADE desg_name from fb_employee where emp_type not in ('LP','LT','LO') and nvl(trim(EMP_INACTIVETYPE),'Regular') = 'Regular' and EMP_GRADE is not null and nvl(emp_active, 0) = 1	
	order by 1;	
	open c1;
...
declare c2 cursor for 
	select distinct emp_id,(emp_name||'['||emp_id||']') name 
	  from fb_employee
	  where  emp_type not in ('LP','LT','LO') and nvl(trim(EMP_INACTIVETYPE),'Regular') = 'Regular'  and nvl(emp_active, 0) = 1	
	order by 1;
...
ld_new_hr = 0;ld_new_electric = 0;ld_new_fuel = 0;ld_new_basic = 0;ld_last_incper = 0;ld_adhoc = 0;ld_incper = 0; ld_last_inc_rt = 0;
...
dw_1.settransobject(sqlca)
	...
	DECLARE c1 CURSOR FOR  
	  SELECT to_date(:ls_inc_dt,'dd/mm/yyyy'),:gs_garden_snm,EMP_ID,emp_name, EMP_GRADE,EAD_DA, round((EBS_BASICAMOUNT * (EAD_DA * 0.01)),0) , 
			 ...
			 0, last_inc_percent,last_inc_rate,0,'N',:gs_user,sysdate,EMP_PROMOTION_DT,'N'
		 FROM (select * from fb_employee where  
				...
				(select ei_emp_id emp_no,nvl(EI_INC_PERCENT,0) last_inc_percent, nvl(EI_INC_RATE,0) last_inc_rate  from fb_emp_inc where (ei_emp_id,ei_date) in (select ei_emp_id,max(ei_date) from fb_emp_inc group by ei_emp_id))
	...
	IF sqlca.sqlcode = 0 THEN
		fetch c1 into :ld_date,:ls_gaden_sname,:ls_emp_id,:ls_emp_nm,:ls_old_grade,:ld_old_da_rt,:ld_old_da,:ld_old_vda_rt,:ld_old_vda,:ld_old_cvrforpf,
					   ...
					   :ld_incper, :ld_last_incper, :ld_last_inc_rt, :ld_adhoc,:ls_letter_type,:ls_entry_by,:ld_entry_dt,:ld_last_prom,:ls_pst_ind;
		...
		do while sqlca.sqlcode <> 100			
			...
			dw_1.setitem(dw_1.getrow(),'ei_inc_percent',ld_incper)
			dw_1.setitem(dw_1.getrow(),'ei_oldinc_percent',ld_last_incper)
			dw_1.setitem(dw_1.getrow(),'ei_last_inc_rate',ld_last_inc_rt)			
			...
			dw_1.setitem(dw_1.getrow(),'ei_logic_ind','PERCENT')
...
//
select distinct 'x' into :ls_temp FROM fb_emp_inc WHERE nvl(EI_PST_IND,'N') = 'N' and ei_logic_ind = 'PERCENT' ;
if sqlca.sqlcode = -1 then 
...
elseif sqlca.sqlcode = 100 then 
	...
	setpointer(hourglass!)
	 ...
	 DECLARE c1 CURSOR FOR  
		select EI_EMP_ID, EI_NEW_BASIC,EI_DATE, EI_NEW_GRADE, EI_OLD_GRADE from fb_emp_inc where nvl(EI_PST_IND,'N') = 'N' and ei_logic_ind = 'PERCENT' ;
...
integer width = 4530
integer height = 2116
string dataobject = "dw_gtehrf021"
...
event itemchanged;if lb_query = false then 
	...
	if dwo.name = 'ei_inc_percent' then
		ld_incper = double(data)
	else
		ld_incper = dw_1.getitemnumber(row,'ei_inc_percent')
	end if;
	
	ld_basic_old = dw_1.getitemnumber(row,'ei_old_basic')
	
	ld_basic = round(ld_basic_old + (ld_basic_old * (ld_incper / 100)),0)
