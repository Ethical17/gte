$PBExportHeader$w_gtelaf010.srw
forward
global type w_gtelaf010 from window
end type
type cb_4 from commandbutton within w_gtelaf010
end type
type cb_3 from commandbutton within w_gtelaf010
end type
type cb_2 from commandbutton within w_gtelaf010
end type
type cb_1 from commandbutton within w_gtelaf010
end type
type dw_1 from datawindow within w_gtelaf010
end type
end forward
...
string title = "(w_gtelaf010) Cash Plucking Rate"
...
global w_gtelaf010 w_gtelaf010
...
public function integer wf_check_duplicate_rec (string fs_kam, datetime fd_dt)
end prototypes
...
public function integer wf_check_fillcol (integer fl_row);if dw_1.rowcount() > 0 and fl_row > 0 then
	if (isnull(dw_1.getitemstring(fl_row,'kamsub_id')) or  len(dw_1.getitemstring(fl_row,'kamsub_id'))=0 or &
		 isnull(dw_1.getitemdatetime(fl_row,'cp_dt_from')) or &
		 isnull(dw_1.getitemnumber(fl_row,'rate')) or  dw_1.getitemnumber(fl_row,'rate')=0 or &
		 isnull(dw_1.getitemnumber(fl_row,'dailyrate')) or  dw_1.getitemnumber(fl_row,'dailyrate')=0 ) then
	    messagebox('Warning: One Of The Following Fields Are Blank', 'Kamjari ID, From date, Rate Daily Rate, Please Check !!!')
		 return -1
	end if
end if
return 1
...
public function integer wf_check_duplicate_rec (string fs_kam, datetime fd_dt);long fl_row
string ls_kam1
datetime ld_dt1
...
if dw_1.rowcount() > 1 then
	for fl_row = 1 to (dw_1.rowcount() - 1)
		ls_kam1 = dw_1.getitemstring(fl_row,'kamsub_id')
		ld_dt1 = dw_1.getitemdatetime(fl_row,'cp_dt_from')
		
		if ls_kam1 = fs_kam and ld_dt1 = fd_dt then
...
IF MessageBox("Save  Alert", 'Do You Want To Save ....?' ,Exclamation!, YesNo!, 1) = 1 THEN
	...
	if dw_1.rowcount() > 0 then
		for ll_ctr = 1 to dw_1.rowcount( ) 
			ld_frdt = dw_1.getitemdatetime(ll_ctr,'cp_dt_from')
			ls_kam = dw_1.getitemstring(ll_ctr,'kamsub_id')
					
			select distinct 'x' into :ls_temp from fb_cashpluckingrate
			where KAMSUB_ID = :ls_kam and cp_dt_from >= :ld_frdt and cp_dt_to is null;
		   ...
		   update fb_cashpluckingrate set cp_dt_to = (:ld_frdt - 1), cp_active_ind = 'N'
		   where KAMSUB_ID = :ls_kam and cp_dt_to is null and cp_dt_from < :ld_frdt;
			if sqlca.sqlcode = -1 then
			...
			update fb_cashpluckingrate set cp_dt_to = (:ld_frdt), cp_active_ind = 'N'
			where KAMSUB_ID = :ls_kam and cp_dt_to is null and cp_dt_from = :ld_frdt;
			if sqlca.sqlcode = -1 then
...
if dw_1.rowcount() = 0 then
	...
	dw_1.setfocus()
	dw_1.setitem(dw_1.getrow(),'cp_entry_by',gs_user)
	dw_1.setitem(dw_1.getrow(),'cp_entry_dt',datetime(today()))
	dw_1.setitem(dw_1.getrow(),'kamsub_id','ESUB0163')
	dw_1.setcolumn('cp_dt_from')
else
	IF wf_check_fillcol(dw_1.getrow()) = -1 THEN
		return 1
	END IF
...
integer height = 2052
integer taborder = 30
string dataobject = "dw_gtelaf010"
...
event key_down;if lb_query = false then
 if lb_neworder = true then	
	...
	if newrow = dw_1.rowcount() and dw_1.rowcount() > 1  then
		dw_1.setitem(newrow,'cp_entry_by',gs_user)
		dw_1.setitem(newrow,'cp_entry_dt',datetime(today()))
		dw_1.setcolumn('kamsub_id')
	end if
 end if
end if
...
event itemchanged;if lb_query = false then
	if dwo.name = 'cp_dt_from'  then
		ld_frdt = datetime(data)
		ls_kam = dw_1.getitemstring(row,'kamsub_id')
				
		if  wf_check_duplicate_rec(ls_kam,ld_frdt) = -1 then return 1
	...
	dw_1.setitem(row,'cp_entry_by',gs_user)
	dw_1.setitem(row,'cp_entry_dt',datetime(today()))
...
SELECT COLUMN_ID,COLUMN_Name FROM USER_TAB_COLUMNS WHERE TABLE_NAME = upper('fb_cashpluckingrate');
			...
			ls_unique_id=dw_1.GetItemString(li_row, 1, Primary!, true)
					 ...
					 if ls_old_value <> ls_new_value then
						insert into fb_log(table_name, Unique_key, U_ACTION, U_FIELD, U_OLDVALUE, U_NEWVALUE, U_USERCODE, U_DATE, U_COMMENT) values
												('fb_cashpluckingrate',:ls_unique_id,'U',:ls_columnname,:ls_old_value,:ls_new_value,:gs_user,sysdate,'');
							if sqlca.sqlcode = -1 then
								messagebox("SQL Error : While Inserting Into Log Table ",SQLCA.SQLErrtext,Information!)
								rollback using sqlca;
								return -1 
