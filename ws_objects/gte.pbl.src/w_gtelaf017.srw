$PBExportHeader$w_gtelaf017.srw
forward
global type w_gtelaf017 from window
end type
type cb_4 from commandbutton within w_gtelaf017
end type
type cb_3 from commandbutton within w_gtelaf017
end type
type cb_2 from commandbutton within w_gtelaf017
end type
type cb_1 from commandbutton within w_gtelaf017
end type
type dw_1 from datawindow within w_gtelaf017
end type
end forward
...
string title = "(w_gtelaf017) Labour Advance"
...
global w_gtelaf017 w_gtelaf017
...
public function integer wf_check_fillcol (integer fl_row);if dw_1.rowcount() > 0 and fl_row > 0 then
	if (isnull(dw_1.getitemstring(fl_row,'labour_id')) or  len(dw_1.getitemstring(fl_row,'labour_id'))=0 or &
		 ...
		 ((isnull(dw_1.getitemnumber(fl_row,'la_amount')) or dw_1.getitemnumber(fl_row,'la_amount')=0) and &
		  (isnull(dw_1.getitemnumber(fl_row,'la_pfadvance')) or dw_1.getitemnumber(fl_row,'la_pfadvance')=0) and &
		  (isnull(dw_1.getitemnumber(fl_row,'la_pfinterest')) or dw_1.getitemnumber(fl_row,'la_pfinterest')=0))) then
	    messagebox('Warning: One Of The Following Fields Are Blank','Labour ID, Date, Advance Amount / PF Advance Amount/ PF Interest Amount, Please Check !!!')
		 return -1
	end if
end if
return 1
...
//		select nvl(MAX(la_ID),0) into :ls_last from fb_gltransaction;
...
event clicked;if cb_2.text = "&Query" then
	...
	dw_1.settaborder('labour_id_1',15)
	dw_1.settaborder('la_date',20)
	...
	dw_1.settaborder('labour_id_1',0)
	dw_1.settaborder('la_date',0)
	dw_1.SetRedraw (TRUE)
	cb_2.text = "&Query"
	lb_query = false
	cb_1.enabled = true
...
integer height = 2052
integer taborder = 30
string dataobject = "dw_gtelaf017"
...
//		
//		select distinct 'x' into :ls_temp from fb_labouradvance where labour_id = :ls_labour_id and trunc(la_date) = :ld_dt;
//	
...
//	cb_3.enabled = true
//end if
//	//if dwo.name = 'la_unitprice' then
//	//	ld_price = double(data)
//	//	ld_qnty = dw_1.getitemnumber(row,'la_quantity')
//	//	ld_tax = dw_1.getitemnumber(row,'tax_amt')
//	//	ld_otchrg = dw_1.getitemnumber(row,'la_otheramo')
//	//	
...
//	//	dw_1.setitem(row,'la_netamount',ld_net)
//	//end if
//	//
//	//if dwo.name = 'la_quantity' then
//	//	ld_qnty = double(data)
//	//	ld_price = dw_1.getitemnumber(row,'la_unitprice')
//	//	ld_tax = dw_1.getitemnumber(row,'tax_amt')
//	//	ld_otchrg = dw_1.getitemnumber(row,'la_otheramo')
//	//	
//	//	if isnull(ld_price) then ld_price = 0
...
//	//	if isnull(ld_otchrg) then ld_otchrg = 0
//	//	
//	//	ld_tax = ld_price * ld_qnty * ld_stax * 0.01
...
//	//	if isnull(ld_otchrg) then ld_otchrg = 0
//	//	
//	//	ld_stax = ld_tax  * 100 / (ld_price * ld_qnty)
//	//	dw_1.setitem(row,'la_saletax',ld_stax)
//	//
//	//	
//	//	ld_net = wf_cal_netval(ld_price,ld_qnty,ld_tax,ld_otchrg)
//	//	dw_1.setitem(row,'la_netamount',ld_net)
