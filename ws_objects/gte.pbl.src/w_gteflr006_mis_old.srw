$PBExportHeader$w_gteflr006_mis_old.srw
forward
global type w_gteflr006_mis_old from window
end type
type st_2 from statictext within w_gteflr006_mis_old
end type
type dp_2 from datepicker within w_gteflr006_mis_old
end type
type dp_1 from datepicker within w_gteflr006_mis_old
end type
type rb_1 from radiobutton within w_gteflr006_mis_old
end type
type st_1 from statictext within w_gteflr006_mis_old
end type
type cb_1 from commandbutton within w_gteflr006_mis_old
end type
type cb_2 from commandbutton within w_gteflr006_mis_old
end type
type rb_4 from radiobutton within w_gteflr006_mis_old
end type
type rb_3 from radiobutton within w_gteflr006_mis_old
end type
type rb_2 from radiobutton within w_gteflr006_mis_old
end type
type gb_1 from groupbox within w_gteflr006_mis_old
end type
type gb_2 from groupbox within w_gteflr006_mis_old
end type
type dw_1 from datawindow within w_gteflr006_mis_old
end type
type dw_2 from datawindow within w_gteflr006_mis_old
end type
end forward
...
string title = "(Gteflr006_old) - Division/Section Wise Plucking Performance"
...
global w_gteflr006_mis_old w_gteflr006_mis_old
...
double ld_ar_ly, ld_arcov_ly, ld_arcov_todt_ly, ld_mdays_ly, ld_totalgl_ly, ld_mdays_todt_ly, ld_totgl_todt_ly
double ld_ar_py, ld_arcov_py, ld_arcov_todt_py, ld_mdays_py, ld_totalgl_py, ld_mdays_todt_py, ld_totgl_todt_py
...
on w_gteflr006_mis_old.destroy
destroy(this.st_2)
destroy(this.dp_2)
destroy(this.dp_1)
destroy(this.rb_1)
...
datetime value = DateTime(Date("2014-12-03"), Time("15:43:12.000000"))
integer textsize = -9
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = roman!
...
SELECT  sum(nvl(b.SECTION_AREA,0)) sectionarea, sum(nvl(areacovered,0)) areacovered,sum(nvl(areacovered_todate,0)) areacovered_todate,
           sum(nvl(mandays,0)) mandays, sum(nvl(totalgl,0)) totalgl, sum(nvl(mandays_todate,0)) mandays_todate, sum(nvl(totalgl_todate,0)) totalgl_todate
into :ld_ar_ly,:ld_arcov_ly, :ld_arcov_todt_ly, :ld_mdays_ly, :ld_totalgl_ly, :ld_mdays_todt_ly, :ld_totgl_todt_ly           
...
SELECT  sum(nvl(b.SECTION_AREA,0)) sectionarea, sum(nvl(areacovered,0)) areacovered,sum(nvl(areacovered_todate,0)) areacovered_todate,
           sum(nvl(mandays,0)) mandays, sum(nvl(totalgl,0)) totalgl, sum(nvl(mandays_todate,0)) mandays_todate, sum(nvl(totalgl_todate,0)) totalgl_todate
into :ld_ar_py,:ld_arcov_py, :ld_arcov_todt_py, :ld_mdays_py, :ld_totalgl_py, :ld_mdays_todt_py, :ld_totgl_todt_py           
FROM FB_FIELD a, FB_SECTION b, 
...
//	dw_1.show()
//	dw_2.hide()	
	dw_1.retrieve(ls_frdt,ls_todt,ls_ysdt,ld_recper_td,ld_recper_tdt, ld_ar_ly, ld_arcov_ly, ld_arcov_todt_ly, ld_mdays_ly, ld_totalgl_ly, ld_mdays_todt_ly, ld_totgl_todt_ly, ld_recper_td_ly,ld_recper_tdt_ly, ld_ar_py, ld_arcov_py, ld_arcov_todt_py, ld_mdays_py, ld_totalgl_py, ld_mdays_todt_py, ld_totgl_todt_py, ld_recper_td_py,ld_recper_tdt_py)
//end if
...
integer width = 4416
integer height = 2056
string dataobject = "dw_gteflr006_mis_old"
