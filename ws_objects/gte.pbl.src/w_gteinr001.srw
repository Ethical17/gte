$PBExportHeader$w_gteinr001.srw
forward
global type w_gteinr001 from window
end type
type cbx_1 from checkbox within w_gteinr001
end type
type ddlb_3 from dropdownlistbox within w_gteinr001
end type
type st_5 from statictext within w_gteinr001
end type
type st_4 from statictext within w_gteinr001
end type
type ddlb_2 from dropdownlistbox within w_gteinr001
end type
type st_3 from statictext within w_gteinr001
end type
type ddlb_1 from dropdownlistbox within w_gteinr001
end type
type dp_2 from datepicker within w_gteinr001
end type
type dp_1 from datepicker within w_gteinr001
end type
type st_2 from statictext within w_gteinr001
end type
type st_1 from statictext within w_gteinr001
end type
type cb_2 from commandbutton within w_gteinr001
end type
type cb_4 from commandbutton within w_gteinr001
end type
type dw_1 from datawindow within w_gteinr001
end type
end forward
...
string title = "(W_gteinr001) Issue Detail"
...
global w_gteinr001 w_gteinr001

type variables
long ll_ctr,net, ll_cnt, ll_year,ll_unitprice,ll_qnty,ll_price
string ls_temp,ls_eacsubhead_id,ls_eachead_id,ls_spc_id,ls_div,ls_sp_id,ls_nppc
string ls_srep
double ld_area
datetime ld_date
...
SELECT initcap(e.SP_NAME)||'('||e.SP_ID||')'  FROM FB_STOREPRODUCT E
 WHERE e.SP_ACTIVE_IND='Y' and  e.SPC_ID = decode(:ls_spc_id,'ALL',e.SPC_ID,:ls_spc_id)
  ORDER BY 1;	
...
//	where nvl(SPC_ACTIVE_IND,'N')='Y'order by 1;	
   ...
   select distinct c.SPC_NAME||' ('||c.SPC_ID||')'
     from fb_productissue a,fb_productissuedetails b,FB_STOREPRODUCTCATEGORY c,
        fb_storeproduct d
  where a.PRIS_ID=b.PRIS_ID and b.SP_ID=d.SP_ID(+) and d.SPC_ID=c.SPC_ID(+) and
        a.PRIS_DIVISION=decode(:ls_div,'ALL',PRIS_DIVISION,:ls_div);
...
datetime value = DateTime(Date("2015-06-24"), Time("09:18:47.000000"))
integer textsize = -9
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = roman!
...
if  isnull(ddlb_1.text) or ddlb_1.text = "" then 
	messagebox('Error At Division','Please Select Division')
	return 1
end if;
...
if  isnull(ddlb_3.text) or ddlb_3.text = "" then 
	messagebox('Error At Product','Please Select Product')
	return 1
end if;
...
dw_1.retrieve(ls_frym,ls_toym,ls_div,ls_spc_id,ls_sp_id, gs_user,ls_nppc)
...
integer height = 2144
integer taborder = 80
string dataobject = "dw_gteinr001"
boolean vscrollbar = true
end type
