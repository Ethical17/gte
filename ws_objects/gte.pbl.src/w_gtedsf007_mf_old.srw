$PBExportHeader$w_gtedsf007_mf_old.srw
forward
global type w_gtedsf007_mf_old from window
end type
type cb_10 from commandbutton within w_gtedsf007_mf_old
end type
type cb_6 from commandbutton within w_gtedsf007_mf_old
end type
type cb_5 from commandbutton within w_gtedsf007_mf_old
end type
type cb_7 from commandbutton within w_gtedsf007_mf_old
end type
type cb_8 from commandbutton within w_gtedsf007_mf_old
end type
type cb_9 from commandbutton within w_gtedsf007_mf_old
end type
type dw_2 from datawindow within w_gtedsf007_mf_old
end type
type cb_4 from commandbutton within w_gtedsf007_mf_old
end type
type cb_3 from commandbutton within w_gtedsf007_mf_old
end type
type cb_2 from commandbutton within w_gtedsf007_mf_old
end type
type cb_1 from commandbutton within w_gtedsf007_mf_old
end type
type dw_1 from datawindow within w_gtedsf007_mf_old
end type
end forward
...
global w_gtedsf007_mf_old w_gtedsf007_mf_old

type variables
long ll_ctr, ll_cnt,l_ctr,ix,ll_st_year, ll_end_year, ll_user_level,ll_ac_year,ll_gpno,ll_last,ll_stno,lledno,ll_nop,ll_season, ll_tmp,ll_pnop,ll_dnop,ll_fymm,ll_tino
string ls_temp,ls_del_ind,ls_tmp_id,ls_entry_user,ls_cons,ls_last,ls_count, ls_ref, ls_desc, ls_pinv, ls_dtpdid,ls_adv, ls_gpno,ls_waybill, ls_type,ls_iss_gstn,ls_rec_gstn, ls_tinvno,ls_wareid
string ls_bname, ls_badd, ls_cname, ls_cadd, ls_gradelist, ls_cnno, ls_cust, ls_brok,ls_lotno, ls_iss_locn, ls_rec_locn,ls_hsn_cd
		...
		if dw_1.getitemstatus(dw_1.getrow(),0,primary!) = NewModified! or dw_1.getitemstatus(dw_1.getrow(),0,primary!) = New! then
			...
			if ll_tino > 0 then
				...
				if ls_iss_gstn <> ls_rec_gstn then	
					...
					if f_upd_lastno('DC',string(ll_fymm),ll_tino) = -1 then
						rollback using sqlca;			
						return 1
					end if						
				end if
...
integer height = 1084
integer taborder = 30
string dataobject = "dw_gtedsf007_mf_130717"
end type
...
//		end if
	...
	if dwo.name = 'cus_id'  then
		...
		select CUS_GSTN_STCD  into :ls_rec_locn from fb_customer  where CUS_ID = :ls_cust;
...
//		end if
	...
	if dwo.name = 'ware_id'  then
		...
		select ware_GSTN_STCD  into :ls_rec_locn from fb_warehouse  where ware_ID = :ls_wareid;
