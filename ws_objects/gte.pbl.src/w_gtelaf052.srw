$PBExportHeader$w_gtelaf052.srw
forward
global type w_gtelaf052 from window
end type
type dw_1 from datawindow within w_gtelaf052
end type
type cb_10 from commandbutton within w_gtelaf052
end type
type cb_8 from commandbutton within w_gtelaf052
end type
type cb_7 from commandbutton within w_gtelaf052
end type
type cb_9 from commandbutton within w_gtelaf052
end type
type cb_4 from commandbutton within w_gtelaf052
end type
type cb_3 from commandbutton within w_gtelaf052
end type
type cb_2 from commandbutton within w_gtelaf052
end type
type cb_1 from commandbutton within w_gtelaf052
end type
end forward
...
string title = "(w_gtelaf052) House Master"
...
global w_gtelaf052 w_gtelaf052

type variables
long ll_ctr, ll_cnt,ll_year,ll_user_level,ll_count,ll_pfno,ll_paybook,ll_retage,ll_agediff, ll_temp
...
datetime ld_dob, ld_doj, ld_dt, ld_dol,ld_dobpre,ld_labdob,ld_childdob, ld_temp
...
public function integer wf_check_fillcol (integer fl_row);if dw_1.rowcount() > 0 and fl_row > 0 and dw_1.getitemstring(fl_row,'unauth_constr') = 'Y' then
	if (isnull(dw_1.getitemstring(fl_row,'house_no')) or len(dw_1.getitemstring(fl_row,'house_no')) = 0  or & 
	isnull(dw_1.getitemstring(fl_row,'house_type')) or len(dw_1.getitemstring(fl_row,'house_type')) = 0  or & 
	...
	isnull(dw_1.getitemstring(fl_row,'fl_madeof')) or len(dw_1.getitemstring(fl_row,'fl_madeof')) = 0  or &
	isnull(dw_1.getitemstring(fl_row,'ceil_madeof')) or len(dw_1.getitemstring(fl_row,'ceil_madeof')) = 0  or &
	isnull(dw_1.getitemstring(fl_row,'rf_madeof')) or len(dw_1.getitemstring(fl_row,'rf_madeof')) = 0  or &
	isnull(dw_1.getitemstring(fl_row,'water_source')) or len(dw_1.getitemstring(fl_row,'water_source')) = 0  or &
	isnull(dw_1.getitemstring(fl_row,'fuel_cook')) or len(dw_1.getitemstring(fl_row,'fuel_cook')) = 0  or &
	isnull(dw_1.getitemstring(fl_row,'unauth_details')) or len(dw_1.getitemstring(fl_row,'unauth_details')) = 0  or &
	isnull(dw_1.getitemstring(fl_row,'house_cons_dt')) ) then
	    messagebox('Warning: One Of The Following Fields Are Blank', 'House No., House_Type, Line No., Divison , House Established Dt., Floor Made Of, Ceiling Made Of, Roof Made Of, Water Source, Cooking Fuel, Unauthorized Contruction Details, Please Check !!!')
		 return -1
	end if
elseif  dw_1.rowcount() > 0 and fl_row > 0 and dw_1.getitemstring(fl_row,'unauth_constr') = 'N' then
	if (isnull(dw_1.getitemstring(fl_row,'house_no')) or len(dw_1.getitemstring(fl_row,'house_no')) = 0  or & 
	isnull(dw_1.getitemstring(fl_row,'house_type')) or len(dw_1.getitemstring(fl_row,'house_type')) = 0  or & 
	isnull(dw_1.getitemstring(fl_row,'line_no')) or len(dw_1.getitemstring(fl_row,'line_no')) = 0  or & 
	isnull(dw_1.getitemstring(fl_row,'divison')) or len(dw_1.getitemstring(fl_row,'divison')) = 0  or & 
	...
	isnull(dw_1.getitemstring(fl_row,'fuel_cook')) or len(dw_1.getitemstring(fl_row,'fuel_cook')) = 0  or &
	isnull(dw_1.getitemstring(fl_row,'house_cons_dt'))) then
	    messagebox('Warning: One Of The Following Fields Are Blank', 'House No., House_Type, Line No., Divison , House Established Dt., Floor Made Of, Ceiling Made Of, Roof Made Of, Water Source, Cooking Fuel, Please Check !!!')
		 return -1
	end if	
end if
return 1
...
integer taborder = 90
string title = "none"
string dataobject = "dw_gtelaf052"
...
event itemchanged;if lb_query = false then
	if dwo.name = 'cow' and long(data) < 0 then
		messagebox('Warning','Value should be greater than or equal to 0.')
		return 1
	end if
	if dwo.name = 'goat' and long(data) < 0 then
		messagebox('Warning','Value should be greater than or equal to 0.')
		return 1
	end if
	if dwo.name = 'pig' and long(data) < 0 then
		messagebox('Warning','Value should be greater than or equal to 0.')
		return 1
	end if
	if dwo.name = 'tv' and long(data) < 0 then
		messagebox('Warning','Value should be greater than or equal to 0.')
		return 1
	end if
	if dwo.name = 'mcycle' and long(data) < 0 then
		messagebox('Warning','Value should be greater than or equal to 0.')
		return 1
	end if
	if dwo.name = 'cycle' and long(data) < 0 then
		messagebox('Warning','Value should be greater than or equal to 0.')
		return 1
	end if
	if dwo.name = 'car' and long(data) < 0 then
		messagebox('Warning','Value should be greater than or equal to 0.')
		return 1
	end if
	if dwo.name = 'no_of_mobile' and long(data) < 0 then
		messagebox('Warning','Value should be greater than or equal to 0.')
		return 1
	end if
	if dwo.name = 'no_of_bank_ac' and long(data) < 0 then
		messagebox('Warning','Value should be greater than or equal to 0.')
		return 1
	end if
	if dwo.name = 'gas' and long(data) < 0 then
		messagebox('Warning','Value should be greater than or equal to 0.')
		return 1
	end if
	if dwo.name = 'no_of_grads' and long(data) < 0 then
		messagebox('Warning','Value should be greater than or equal to 0.')
		return 1
	end if
	
	if dwo.name = 'latrine' then
		if data = 'N' then
			setnull(ld_temp)
			setnull(ll_temp)
			dw_1.setitem(row,'cons_dt_latrine',ld_temp)
			dw_1.setitem(row,'latrine_last_rep',ld_temp)
			dw_1.setitem(row,'cost_rep_latrine',ll_temp)
		end if
	end if
	
	if dwo.name = 'unauth_constr' then
		if data = 'N' then
			setnull(ls_temp)
			dw_1.setitem(row,'unauth_details',ls_temp)
		end if
	end if
...
SELECT COLUMN_ID,COLUMN_Name FROM USER_TAB_COLUMNS WHERE TABLE_NAME = upper('fb_house_master');
			...
			ls_unique_id=dw_1.GetItemString(li_row, 1, Primary!, true)
					 ...
					 if ls_old_value <> ls_new_value then
						insert into fb_log(table_name, Unique_key, U_ACTION, U_FIELD, U_OLDVALUE, U_NEWVALUE, U_USERCODE, U_DATE, U_COMMENT) values
												('fb_house_master',:ls_unique_id,'U',:ls_columnname,:ls_old_value,:ls_new_value,:gs_user,sysdate,'');
							if sqlca.sqlcode = -1 then
								messagebox("SQL Error : While Inserting Into Log Table ",SQLCA.SQLErrtext,Information!)
								rollback using sqlca;
								return -1 
...
IF MessageBox("Save  Alert", 'Do You Want To Save ....?' ,Exclamation!, YesNo!, 1) = 1 THEN
	...
	if lb_neworder = true then
		select nvl(MAX(house_id),0) into :ls_last from fb_house_master;
	...
	if dw_1.rowcount() > 0 then
		for ll_ctr = 1 to dw_1.rowcount( ) 
			...
			if lb_neworder = true then
				...
				ls_tmp_id = 'HN'+ls_count
				dw_1.setitem(ll_ctr,'house_id',ls_tmp_id)
			end if
