$PBExportHeader$w_gteprof021.srw
forward
global type w_gteprof021 from window
end type
type cb_5 from commandbutton within w_gteprof021
end type
type st_6 from statictext within w_gteprof021
end type
type sle_1 from singlelineedit within w_gteprof021
end type
type em_2 from editmask within w_gteprof021
end type
type st_5 from statictext within w_gteprof021
end type
type st_4 from statictext within w_gteprof021
end type
type ddlb_2 from dropdownlistbox within w_gteprof021
end type
type st_3 from statictext within w_gteprof021
end type
type ddlb_1 from dropdownlistbox within w_gteprof021
end type
type st_2 from statictext within w_gteprof021
end type
type em_1 from editmask within w_gteprof021
end type
type st_1 from statictext within w_gteprof021
end type
type dp_1 from datepicker within w_gteprof021
end type
type cb_4 from commandbutton within w_gteprof021
end type
type cb_3 from commandbutton within w_gteprof021
end type
type cb_2 from commandbutton within w_gteprof021
end type
type cb_1 from commandbutton within w_gteprof021
end type
type dw_1 from datawindow within w_gteprof021
end type
end forward
...
global w_gteprof021 w_gteprof021
...
event clicked;string ls_remarks,ls_ddp_pk_id
datetime ld_exceedate


IF MessageBox("Delete  Alert", 'Do You Want To Cancel The Selected Date Excess Tea ....?' ,Exclamation!, YesNo!, 1) = 1 THEN


	if isnull(sle_1.text) or len(sle_1.text)=0 then
		messagebox("Warning :  Remarks Cannot be Blank ","Remarks Cannot be blank",Information!)
		return
	else 
		ls_remarks=sle_1.text
	end if
	ld_exceedate= datetime(ddlb_2.text)
	

	insert into fb_excess_tea_cncl(ET_STOCK_DATE, ET_ESTATE, TMP_ID, ET_GL_QTY, ET_EXCESS_TEA, ET_ENTRY_BY, ET_ENTRY_DT, ET_ROSEND_DT, TPC_ID, GLFP_PK, ET_SEASON, ET_REMARKS, ET_CNCL_DATE, ET_USERID) 
 	select ET_STOCK_DATE, ET_ESTATE, TMP_ID, ET_GL_QTY, ET_EXCESS_TEA, ET_ENTRY_BY, ET_ENTRY_DT, ET_ROSEND_DT, TPC_ID, GLFP_PK, ET_SEASON,:ls_remarks,sysdate,:gs_user from fb_excess_tea where ET_STOCK_DATE =trunc(:ld_exceedate);
					
	if sqlca.sqlcode = -1 then
			messagebox("SQL Error : While Inserting Into Excess Tea Cancel Detail Table ",SQLCA.SQLErrtext,Information!)
			rollback using sqlca;
			return -1 
	end if
	
	
	insert into fb_dailydryerproduct_cncl(DRYER_ID, DDP_RUNHOURS, DDP_STARTRUNTIME, DDP_ENDRUNTIME, DDP_PLUCKINGDATE, DDP_CONFIRMFLAG, DDP_PK, DDP_TYPE, DDP_ENTRY_BY, DDP_ENTRY_DT, DDP_ROSEND_DT, PRIS_ID, DDP_REMARKS, DDP_CNCL_DATE, DDP_USERID)
	select DRYER_ID, DDP_RUNHOURS, DDP_STARTRUNTIME, DDP_ENDRUNTIME, DDP_PLUCKINGDATE, DDP_CONFIRMFLAG, DDP_PK, DDP_TYPE, DDP_ENTRY_BY, DDP_ENTRY_DT, DDP_ROSEND_DT, PRIS_ID,:ls_remarks, sysdate, :gs_user from fb_dailydryerproduct where DDP_PLUCKINGDATE = trunc(:ld_exceedate) and DRYER_ID = 'EXCESS';
	
	if sqlca.sqlcode = -1 then
			messagebox("SQL Error : While Inserting Into Daily Dryer Product Cancel Detail Table ",SQLCA.SQLErrtext,Information!)
			rollback using sqlca;
			return -1 
	end if
	...
	select DDP_PK from  fb_dailydryerproduct where DDP_PLUCKINGDATE = trunc(:ld_exceedate) and  DRYER_ID = 'EXCESS';
		 ...
		 open c1;
					 ...
					 if sqlca.sqlcode = -1 then 
						...
						setnull(ls_ddp_pk_id);				
						fetch c1 into :ls_ddp_pk_id;
							do while sqlca.sqlcode <> 100    
						
						 insert into fb_dailydryerunsorted_cncl(DDP_PK, TPC_ID, DDU_QUANTITY, DDP_DT, DDP_SEASON, DDP_REMARKS, DDP_CNCL_DATE, DDP_USERID)
 						 select DDP_PK, TPC_ID, DDU_QUANTITY, DDP_DT, DDP_SEASON,:ls_remarks, sysdate, :gs_user from fb_dailydryerunsorted where DDP_PK =:ls_ddp_pk_id;
						if sqlca.sqlcode = -1 then
							messagebox("SQL Error : While Inserting Into Daily Dryer Unsorted Cancel Detail Table ",SQLCA.SQLErrtext,Information!)
							rollback using sqlca;
							return -1 
						end if		
						
//						delete from fb_dailydryerunsorted where DDP_PK =:ls_ddp_pk_id;
//						if sqlca.sqlcode = -1 then
//							messagebox("SQL Error : While Deleting Daily Dryer Unsorted Table ",SQLCA.SQLErrtext,Information!)
//							rollback using sqlca;
//							return -1 
//						end if		
		...
		insert into fb_glforproduction_cncl(GLFP_PLUCKINGDATE, GLFP_GLFORPRODUCTION, SUP_ID, GLFP_PK, GLFP_GL_QTY, GLFP_GL_TQTY, GLFP_GL_SQTY, GLFP_REMARKS, GLFP_CNCL_DATE, GLFP_USERID) 
   		select GLFP_PLUCKINGDATE, GLFP_GLFORPRODUCTION, SUP_ID, GLFP_PK, GLFP_GL_QTY, GLFP_GL_TQTY, GLFP_GL_SQTY, :ls_remarks, sysdate, :gs_user 
    		from fb_glforproduction  where glfp_pk  in (select GLFP_PK from fb_excess_tea where ET_STOCK_DATE =trunc(:ld_exceedate));
		if sqlca.sqlcode = -1 then
				messagebox("SQL Error : While Inserting Into GL For Production Cancel Detail Table ",SQLCA.SQLErrtext,Information!)
				rollback using sqlca;
				return -1 
		end if		
		
		insert into fb_gardenwiseteamade_cncl(GWTM_TEAMADE, GWTM_TYPE, GLFP_PK, GWTM_SEASON, GWTM_REMARKS, GWTM_CNCL_DATE, GWTM_USERID)
    		select GWTM_TEAMADE, GWTM_TYPE, GLFP_PK, GWTM_SEASON,  :ls_remarks, sysdate, :gs_user  from fb_gardenwiseteamade where GLFP_PK in(select GLFP_PK from fb_excess_tea where ET_STOCK_DATE =trunc(:ld_exceedate));
		if sqlca.sqlcode = -1 then
				messagebox("SQL Error : While Inserting Into Garden Wise Tea Made Cancel Detail Table ",SQLCA.SQLErrtext,Information!)
				rollback using sqlca;
				return -1 
		end if
		
		delete from fb_glforproduction  where glfp_pk  in (select GLFP_PK from fb_excess_tea where ET_STOCK_DATE =trunc(:ld_exceedate));
		if sqlca.sqlcode = -1 then
				messagebox("SQL Error : While Deleting Into GL For Production Table ",SQLCA.SQLErrtext,Information!)
				rollback using sqlca;
				return -1 
		end if	
		
		delete from fb_gardenwiseteamade where GLFP_PK in(select GLFP_PK from fb_excess_tea where ET_STOCK_DATE =trunc(:ld_exceedate));
		if sqlca.sqlcode = -1 then
				messagebox("SQL Error : While Deleting Into  Garden Wise Tea Made Table ",SQLCA.SQLErrtext,Information!)
				rollback using sqlca;
				return -1 
		end if	
		
		
		delete  from fb_dailydryerproduct where DDP_PLUCKINGDATE = trunc(:ld_exceedate) and DRYER_ID = 'EXCESS';
		if sqlca.sqlcode = -1 then
				messagebox("SQL Error : While Deleting Daily Dryer Product Table ",SQLCA.SQLErrtext,Information!)
				rollback using sqlca;
				return -1 
		end if	
		
		delete from fb_excess_tea where ET_STOCK_DATE =trunc(:ld_exceedate);
		if sqlca.sqlcode = -1 then
				messagebox("SQL Error : While Deleting Excess Tea Table ",SQLCA.SQLErrtext,Information!)
				rollback using sqlca;
				return -1 
		end if	
			
	commit;
	dw_1.Retrieve(gs_user,date(ld_exceedate))
	//dw_1.retrive()
...
datetime value = DateTime(Date("2023-05-08"), Time("09:39:24.000000"))
integer textsize = -9
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = roman!
...
from fb_excess_tea where ET_STOCK_DATE < to_date(:ld_rundt,'dd/mm/yyyy');
...
IF sqlca.sqlcode = 0 THEN
	...
	do while sqlca.sqlcode <> 100
		...
		dw_1.setitem(dw_1.getrow(),'et_stock_date',date(dp_1.text))
...
integer width = 3506
integer height = 1020
string dataobject = "dw_gteprof021"
...
event itemchanged;string ls_date,ls_qty,ls_gfppk

if dwo.name= 'del_flag' then
	if data="1" then
			if	isnull(dw_1.getitemstring(row,'remarks')) or len(dw_1.getitemstring(row,'remarks'))=0  then
				messagebox("Warning: Remarks cannot be blank ",SQLCA.SQLErrtext,Information!)
				rollback using sqlca;
				return  
			else
					insert into fb_excess_tea_cncl(ET_STOCK_DATE, ET_ESTATE, TMP_ID, ET_GL_QTY, ET_EXCESS_TEA, ET_ENTRY_BY, ET_ENTRY_DT, ET_ROSEND_DT, TPC_ID, GLFP_PK, ET_SEASON, ET_REMARKS, ET_CNCL_DATE, ET_USERID) 
 					select ET_STOCK_DATE, ET_ESTATE, TMP_ID, ET_GL_QTY, ET_EXCESS_TEA, ET_ENTRY_BY, ET_ENTRY_DT, ET_ROSEND_DT, TPC_ID, GLFP_PK, ET_SEASON,'Wrong Entry',sysdate,'ADMIN' from fb_excess_tea where ET_STOCK_DATE =to_date('25/12/2021','dd/mm/yyyy');
					...
					declare c1 cursor for 
					 ...
					 select ET_STOCK_DATE, ET_EXCESS_TEA, GLFP_PK  from fb_excess_tea where ET_STOCK_DATE =to_date('25/12/2021','dd/mm/yyyy');
					 ...
					 if sqlca.sqlcode = -1 then 
						...
						setnull(ls_date);setnull(ls_qty);setnull(ls_gfppk);						
						fetch c1 into :ls_date,:ls_qty,:ls_gfppk;
