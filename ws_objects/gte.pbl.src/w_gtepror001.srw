$PBExportHeader$w_gtepror001.srw
forward
global type w_gtepror001 from window
end type
type st_2 from statictext within w_gtepror001
end type
type dp_2 from datepicker within w_gtepror001
end type
type dp_1 from datepicker within w_gtepror001
end type
type rb_2 from radiobutton within w_gtepror001
end type
type rb_1 from radiobutton within w_gtepror001
end type
type st_1 from statictext within w_gtepror001
end type
type cb_1 from commandbutton within w_gtepror001
end type
type cb_2 from commandbutton within w_gtepror001
end type
type dw_1 from datawindow within w_gtepror001
end type
type gb_1 from groupbox within w_gtepror001
end type
end forward
...
string title = "(Gtepror001) - Manufacturing Report"
...
global w_gtepror001 w_gtepror001
...
string ls_text, ls_text1

select nvl(pfa_permission,1) into :ll_user_level from fb_pf_access where pfa_user_id=:gs_user and nvl(pfa_active,'N')='Y';
...
if ll_user_level=1 or ll_user_level=0 then

ls_text = "CREATE OR REPLACE VIEW FBVW_DMR (PARTICULARS, GTTYPE, NAME, PLUCKINGDATE, QUANTITY) AS   "&
...
+"  WHERE tpc.tpc_id = tmp.tpc_id AND dtpd.tmp_id = tmp.tmp_id AND dtp.dtp_id = dtpd.dtp_id  and  not exists  (select *"&
...
+"  union all"&
+"  SELECT   'PACKED TEA MADE', 'ALL', tpc.tpc_manid, dtp.dtp_date, NVL(SUM ((dtpd.dtpd_srnoend - dtpd.dtpd_srnostart + 1) * dtpd.dtpd_indwt ), 0 ) "&
+"  FROM fb_dailyteapacked_new dtp, fb_dtpdetails_new dtpd, fb_teamadeproductcategory tpc, fb_teamadeproduct tmp  "&
...
ls_text1 = "CREATE OR REPLACE VIEW FBVW_DMR (PARTICULARS, GTTYPE, NAME, PLUCKINGDATE, QUANTITY) AS  "&
		 + " SELECT   'GREEN LEAF', DECODE (glt.gt_type, 'OWNATTE', 'OWN', 'OWNUSER', 'OWN',glt.gt_type), "&
		 + "           sup.sup_name, glt.pluckingdate, SUM(NVL(glt.gt_quantity,0) * decode(glt.gt_type,'TRANSFER',-1,'SALE',-1,1)) "&
		 + "   FROM fb_gltransaction glt,fb_supplier sup         "&
		 + "  WHERE glt.sup_id = sup.sup_id      "&
		 ...
		 + " GROUP BY (wr.wr_date) HAVING NVL (SUM (wr.wr_rainfall), 0) <> 0  "&
		 + " UNION ALL  "&
		+ " SELECT 'MANDAYS', 'ALL', 'MANUFACTURE MANDAYS' , tad.lda_date,	(1) md  "&
		+ " FROM fb_labourdailyattendance tad, fb_kamsubhead kam, fb_expenseacsubhead esub,  "&
		+ "     (select LABOUR_ID, x.LWW_ID  from FB_LABOURWEEKLYWAGES x, fb_labourwagesweek y where nvl(LABOUR_PF, 0) > 0 and x.LWW_ID = y.LWW_ID)  w  "&
		+ " WHERE tad.KAMSUB_ID = kam.kamsub_id AND nvl(KAMSUB_ACTIVE_IND,'N') = 'Y' and kam.kamsub_id = esub.eacsubhead_id AND (esub.eachead_id IN ('SLEG0016')) "& 
		+ "     and tad.LABOUR_ID = w.LABOUR_ID and tad.LWW_ID = w.LWW_ID "& 
		 + " UNION ALL  "&
		 + " SELECT 'FUEL CONSUMPTION', 'ALL', UPPER (ct.ct_manid), ddp.DDP_STARTRUNTIME, NVL(SUM(ddc.ddc_unitconsumed), 0) "&
		 + "   FROM fb_dailydryerproduct ddp, fb_dailydryerconsumption ddc, fb_consumptiontype ct, fb_machine mac  "&
		 + " WHERE ddp.ddp_pk = ddc.ddp_pk AND ddp.dryer_id = mac.machine_id AND TRIM (mac.machine_type) in ('DRY','SORT','TROUGH','CTC','BOILER') AND ddc.ct_id = ct.ct_id  "&
...
execute immediate :ls_text1 using sqlca;
if sqlca.sqlcode = -1 then
	messagebox('SQL ERROR: During DMR View Creation',sqlca.sqlerrtext)
	return
end if
else
	messagebox('Warnning:','No Access')
	return
end if
end event
...
dw_1.retrieve(ls_frdt,ls_todt,ls_ysdt,ls_ly_frdt,ls_ly_todt,ls_ly_ysdt,ls_year_ind) 
...
integer width = 3387
integer height = 2028
string dataobject = "dw_gtepror001"
