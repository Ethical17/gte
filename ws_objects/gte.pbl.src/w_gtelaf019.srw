$PBExportHeader$w_gtelaf019.srw
forward
global type w_gtelaf019 from window
end type
type cb_4 from commandbutton within w_gtelaf019
end type
type cb_3 from commandbutton within w_gtelaf019
end type
type cb_2 from commandbutton within w_gtelaf019
end type
type cb_1 from commandbutton within w_gtelaf019
end type
type dw_1 from datawindow within w_gtelaf019
end type
end forward
...
string title = "(w_gtelaf019) Labour Spouse"
...
global w_gtelaf019 w_gtelaf019
...
public function integer wf_check_fillcol (integer fl_row);if dw_1.rowcount() > 0 and fl_row > 0 then
	 if (isnull(dw_1.getitemstring(fl_row,'labour_id')) or  len(dw_1.getitemstring(fl_row,'labour_id'))=0 or &
	 	isnull(dw_1.getitemstring(fl_row,'spouse_type')) or  len(dw_1.getitemstring(fl_row,'spouse_type'))=0 or &
	    isnull(dw_1.getitemstring(fl_row,'spouse_id')) or  len(dw_1.getitemstring(fl_row,'spouse_id'))=0 or &
	    isnull(dw_1.getitemstring(fl_row,'spouse_name')) or  len(dw_1.getitemstring(fl_row,'spouse_name'))=0 or &
	    isnull(dw_1.getitemdatetime(fl_row,'spouse_dob')) ) then
	    messagebox('Warning: One Of The Following Fields Are Blank','Labour ID, Spouse Type, Spouse Name, Spouse DOB, Please Check !!!')
	    return -1
	end if
end if
return 1
...
if dw_1.rowcount() > 1 then
	for fl_row = 1 to (dw_1.rowcount() - 1)
		ls_emp1 = dw_1.getitemstring(fl_row,'labour_id')
		ls_spid1 = dw_1.getitemstring(fl_row,'spouse_id')
...
if dw_1.rowcount() > 1 then
	for fl_row = 1 to (dw_1.rowcount() - 1)
		ls_dpid1 = dw_1.getitemstring(fl_row,'spouse_id')
		ls_emp1 = dw_1.getitemstring(fl_row,'labour_id')
...
if dw_1.rowcount() > 1 then
	for fl_row = 1 to (dw_1.rowcount() - 1)
		ls_sp_nm = dw_1.getitemstring(fl_row,'spouse_name')
		ld_dob1 = dw_1.getitemdatetime(fl_row,'spouse_dob')
		ls_dpid1 = dw_1.getitemstring(fl_row,'spouse_id')
...
dw_1.GetChild ("spouse_id", idw_sptype)
...
if dw_1.rowcount() = 0 then
	...
	dw_1.setfocus()
	dw_1.setitem(dw_1.getrow(),'spouse_entry_by',gs_user)
	dw_1.setitem(dw_1.getrow(),'spouse_entry_dt',datetime(today()))
	dw_1.setcolumn('labour_id')
else
	IF wf_check_fillcol(dw_1.getrow()) = -1 THEN
		return 1
...
event key_down;if lb_query = false then
 if lb_neworder = true then	
	...
	if newrow = dw_1.rowcount() and dw_1.rowcount() > 1  then
		dw_1.setitem(newrow,'spouse_entry_by',gs_user)
		dw_1.setitem(newrow,'spouse_entry_dt',datetime(today()))
		dw_1.setcolumn('labour_id')
	end if
 end if
end if
...
event itemchanged;if  lb_query = false then
	if dwo.name = 'labour_id'  then
		...
		select distinct 'x'  into :ls_temp	from fb_labourspouse where labour_id = :ls_labour_id or spouse_id = :ls_labour_id;
	...
	if dwo.name = 'spouse_type' then
		if data = 'LABOUR' then
			...
			dw_1.setitem(row,'spouse_id',ls_temp)
			dw_1.setitem(row,'spouse_name',ls_temp)
			dw_1.setitem(row,'spouse_dob',ld_dt)
	...
	if dwo.name = 'spouse_id' then
		...
		select distinct 'x'  into :ls_temp	from fb_labourspouse where labour_id = :ls_labour_id or  nvl(spouse_id,'x') = nvl(:ls_sp_id,'x');
		...
		if sqlca.sqlcode = -1 then 
			...
			return 1
		elseif sqlca.sqlcode = 0 then
			dw_1.setitem(row,'spouse_name',ls_spouse_name)
			dw_1.setitem(row,'spouse_dob',ld_dt)
	...
	if dwo.name = 'spouse_dob'  then
		ls_dob= data
		ls_labour_id = dw_1.getitemstring(row,'labour_id')
		ls_sp_id = dw_1.getitemstring(row,'spouse_id')
		ls_spouse_name = dw_1.getitemstring(row,'spouse_name')
		...
		select distinct 'x'  into :ls_temp
			...
			where labour_id = :ls_labour_id or (nvl(spouse_id,'x') = nvl(:ls_sp_id,'x') and spouse_name=:ls_spouse_name and trunc(spouse_dob)=to_date(:ls_dob,'dd/mm/yyyy'));
	...
	dw_1.setitem(row,'spouse_entry_by',gs_user)
	dw_1.setitem(row,'spouse_entry_dt',datetime(today()))
