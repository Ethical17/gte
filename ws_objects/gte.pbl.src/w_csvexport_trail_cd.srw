$PBExportHeader$w_csvexport_trail_cd.srw
forward
global type w_csvexport_trail_cd from window
end type
type rb_4 from radiobutton within w_csvexport_trail_cd
end type
type rb_3 from radiobutton within w_csvexport_trail_cd
end type
type rb_2 from radiobutton within w_csvexport_trail_cd
end type
type st_2 from statictext within w_csvexport_trail_cd
end type
type dp_1 from datepicker within w_csvexport_trail_cd
end type
type ddlb_1 from dropdownlistbox within w_csvexport_trail_cd
end type
type st_1 from statictext within w_csvexport_trail_cd
end type
type rb_1 from radiobutton within w_csvexport_trail_cd
end type
type rb_113 from radiobutton within w_csvexport_trail_cd
end type
type rb_114 from radiobutton within w_csvexport_trail_cd
end type
type cb_4 from commandbutton within w_csvexport_trail_cd
end type
type gb_1 from groupbox within w_csvexport_trail_cd
end type
end forward
...
global w_csvexport_trail_cd w_csvexport_trail_cd
...
on w_csvexport_trail_cd.create
...
on w_csvexport_trail_cd.destroy
destroy(this.rb_4)
destroy(this.rb_3)
destroy(this.rb_2)
destroy(this.st_2)
...
datetime value = DateTime(Date("2025-03-07"), Time("16:27:20.000000"))
integer textsize = -10
fontcharset fontcharset = ansi!
fontpitch fontpitch = variable!
fontfamily fontfamily = roman!
...
event clicked;string ls_asondt, ls_status,ls_subject,ls_message,ls_body_text,ls_signature
...
declare p2 procedure for up_garden_monthly_trial_mes(:ls_asondt);
		if sqlca.sqlcode = -1 then
			messagebox('SQL Error: During Procedure Declare of up_garden_monthly_trial_mes',sqlca.sqlerrtext)					
			return 1
		end if

		execute p2;
		if sqlca.sqlcode = -1 then
			messagebox('SQL Error: During Procedure Execute of up_garden_monthly_trial_mes',sqlca.sqlerrtext)
			return 1
		end if
...
setpointer(arrow!) 
messagebox('Confirmation','Data Has been send to HO.')

select AP_STATUS into :ls_status from fb_acyear_period where AP_END_DT = to_date(:ls_asondt,'dd/mm/yyyy');
if sqlca.sqlcode = -1 then 
	messagebox('Sql Error During Getting A/c Year Status !!',sqlca.sqlerrtext)
	return 1
elseif sqlca.sqlcode = 0 then 
	if ls_status = 'O' then 
		update fb_acyear_period set AP_STATUS = 'C' where AP_END_DT = to_date(:ls_asondt,'dd/mm/yyyy');
		if sqlca.sqlcode = -1 then 
			messagebox('Sql Error During Updating A/c Year Status !!',sqlca.sqlerrtext)
			rollback using sqlca;
			return 1
		end if;
	end if;
	
	update fb_acyear_period set AP_STATUS = 'O' where AP_END_DT = add_months(to_date(:ls_asondt,'dd/mm/yyyy'),1);
	if sqlca.sqlcode = -1 then 
		messagebox('Sql Error During Updating A/c Year Status !!',sqlca.sqlerrtext)
		rollback using sqlca;
		return 1
...
ls_subject="Garden Trail And MES"

ls_subject 	 = ls_subject + string(today(),'dd/mm/yyyy')+" "+ gs_garden_nm

ls_message	= "Dear Sir/Madam, "+gs_lfcr+gs_lfcr+'Trail And Mes of Garden'+gs_garden_nm+ ' Has been ready for voucher entry.'+gs_lfcr+gs_lfcr
...
lnvo_mail.of_send_webmail_noattachment('erpsupport@Luxmigroup.in','sarbani@luxmigroup.in;',gs_garden_mail,ls_subject,ls_message,';prabhat.kumar@obeetee.com;piyush@luyxmigroup.in') 
...
SELECT sum(count(distinct VD_GL_CD)) into :li_ctr    
...
if sqlca.sqlcode = -1 then 
	messagebox('Sql Error During getting Count !!!',sqlca.sqlerrtext)
	return 1
end if;
  
insert into fb_garden_trailexp@LUXMI_ON(TE_YRMTH, TE_GSNM, TE_LEGERID, TE_LEDGERNM, TE_DRAMT, TE_CRAMT, TE_TRAILEXP_IND)
SELECT to_char(to_date(:ls_frym,'dd/mm/yyyy'),'YYYYMM'), :gs_garden_snm,VD_GL_CD gl_code,initcap(a.ACLEDGER_NAME) gl_desc,
    ...
    sum(decode(a.ACLEDGER_CUMLATIVE_IND,'C',decode(a.ACLEDGER_ACTYPE,'L',(decode(VD_DC_IND,'C',1,-1) * nvl(VD_AMOUNT,0)),'I',(decode(VD_DC_IND,'C',1,-1) * nvl(VD_AMOUNT,0)),0),
                                                  'H',decode(trunc(VH_VOU_DATE),to_date(:fy_end_dt,'dd/mm/yyyy'),0,decode(a.ACLEDGER_ACTYPE,'L',(decode(VD_DC_IND,'C',1,-1) * nvl(VD_AMOUNT,0)),'I',(decode(VD_DC_IND,'C',1,-1) * nvl(VD_AMOUNT,0)),0)),
                                                        decode(sign(trunc(VH_VOU_DATE) - to_date(:fy_st_dt,'dd/mm/yyyy')),-1,0,decode(a.ACLEDGER_ACTYPE,'L',(decode(VD_DC_IND,'C',1,-1) * nvl(VD_AMOUNT,0)),'I',(decode(VD_DC_IND,'C',1,-1) * nvl(VD_AMOUNT,0)),0)))) cr_cols, 'T' ind    
from FB_ACLEDGER a,fb_vou_head,Fb_vou_DET
where VH_DOC_SRL = vd_doc_srl and vd_gl_cd = ACLEDGER_ID AND 
   trunc(VH_VOU_DATE) <= to_date(:ls_frym,'dd/mm/yyyy') and VH_APPROVED_DT is not null
...
if sqlca.sqlcode = -1 then 
	messagebox('Sql Error During Inserting Data !!!',sqlca.sqlerrtext)
	return 1
end if;
...
select sum(count(distinct EACHEAD_ID)) into :li_ctr    
from fb_dailyexpense a, FB_EXPENSEACSUBHEAD b, fb_acsubledger c
...
insert into fb_garden_trailexp@LUXMI_ON(TE_YRMTH, TE_GSNM, TE_LEGERID, TE_LEDGERNM, TE_DRAMT, TE_CRAMT, TE_TRAILEXP_IND)
select  to_char(to_date(:ls_asondt,'dd/mm/yyyy'),'YYYYMM'), :gs_garden_snm,EACHEAD_ID, initcap(ACSUBLEDGER_NAME) ACSUBLEDGER_NAME, sum(nvl(DE_SALARYTODAYTY,0) + nvl(DE_WAGESTODAYTY,0) + nvl(DE_STORESTODAYTY,0) + nvl(DE_OTHERSTODAYTY,0)) amt, 0, 'E'
from fb_dailyexpense a, FB_EXPENSEACSUBHEAD b, fb_acsubledger c
where a.EACSUBHEAD_ID = b.EACSUBHEAD_ID and b.EACHEAD_ID = ACSUBLEDGER_ID and ACLEDGER_ID = 'LEG0015' and de_date between to_date((decode(sign(to_number(to_char(to_date(:ls_asondt,'dd/mm/yyyy'),'mm')) - 4),-1,to_number(to_char(to_date(:ls_asondt,'dd/mm/yyyy'),'yyyy'))-1,to_number(to_char(to_date(:ls_asondt,'dd/mm/yyyy'),'yyyy')))||'0401'),'yyyymmdd') AND to_date(:ls_asondt,'dd/mm/yyyy')
