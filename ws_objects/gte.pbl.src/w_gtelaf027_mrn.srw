$PBExportHeader$w_gtelaf027_mrn.srw
forward
global type w_gtelaf027_mrn from window
end type
type cb_3 from commandbutton within w_gtelaf027_mrn
end type
type st_1 from statictext within w_gtelaf027_mrn
end type
type cb_1 from commandbutton within w_gtelaf027_mrn
end type
type cb_2 from commandbutton within w_gtelaf027_mrn
end type
type dw_1 from datawindow within w_gtelaf027_mrn
end type
type ddlb_1 from dropdownlistbox within w_gtelaf027_mrn
end type
end forward
...
global w_gtelaf027_mrn w_gtelaf027_mrn

type variables
long ll_ctr,ll_user_level
string ls_temp, ls_dt, ls_lwwid, ls_labour,ls_exempt, ls_name
double ld_adv, ld_pf, ld_pfint,ld_elc, ld_net
datetime ld_dt
end variables

on w_gtelaf027_mrn.create
	...
	for ll_ctr = 1 to DW_1.ROWCOUNT()
		if dw_1.getitemstring(ll_ctr,'exempt_ind') ='Y' then
			...
			elseif sqlca.sqlcode  = 100 then
				insert into fb_labadvancededuction (LABOUR_ID, ADVANCEDED,pfadvanceded,pfinterestded,electricded, LWW_ID, LAD_DATE, LAD_ENTRY_BY)
				values(:ls_labour,:ld_adv,:ld_pf,:ld_pfint,:ld_elc,:ls_lwwid,trunc(sysdate),:gs_user);
...
integer width = 4283
integer height = 2036
string dataobject = "dw_gtelaf027_mrn"
