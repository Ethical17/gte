$PBExportHeader$w_gtelaf064.srw
forward
global type w_gtelaf064 from window
end type
type cb_4 from commandbutton within w_gtelaf064
end type
type cb_1 from commandbutton within w_gtelaf064
end type
type cb_3 from commandbutton within w_gtelaf064
end type
type cb_2 from commandbutton within w_gtelaf064
end type
type dw_1 from datawindow within w_gtelaf064
end type
end forward
...
string title = "(w_gtelaf064) LPG Entitlement Master"
...
global w_gtelaf064 w_gtelaf064
...
IF MessageBox("Save  Alert", 'Do You Want To Save ....?' ,Exclamation!, YesNo!, 1) = 1 THEN
		 ...
		 ls_temp_field_id = dw_1.getitemstring(dw_1.getrow(),'field_id')
		 ld_temp_frdt = dw_1.getitemdatetime(dw_1.getrow(),'fr_dt')
		 ld_temp_todt = dw_1.getitemdatetime(dw_1.getrow(),'to_dt')
		IF wf_check_fillcol(dw_1.getrow()) = -1 or wf_check_duplicate_rec(ls_temp_field_id,ld_temp_frdt,ld_temp_todt,dw_1.getrow()) = -1 THEN
			return 1
		END IF
...
event clicked;if cb_2.text = "&Query" then
	...
	dw_1.settaborder('field_id_1',0)	
	dw_1.settaborder('fr_dt',0)	
	dw_1.settaborder('to_dt',0)
	dw_1.settaborder('no_of_entitled',0)	
	dw_1.SetRedraw (TRUE)
	cb_2.text = "&Query"
	lb_query = false
	cb_1.enabled = true
...
integer height = 2052
integer taborder = 30
string dataobject = "dw_gtelaf064"
...
SELECT COLUMN_ID,COLUMN_Name FROM USER_TAB_COLUMNS WHERE TABLE_NAME = upper('fb_lpg_entitlement_mast');
			...
			ls_unique_id=dw_1.GetItemString(li_row, 1, Primary!, true)
					 ...
					 if ls_old_value <> ls_new_value then
						insert into fb_log(table_name, Unique_key, U_ACTION, U_FIELD, U_OLDVALUE, U_NEWVALUE, U_USERCODE, U_DATE, U_COMMENT) values
												('fb_lpg_entitlement_mast',:ls_unique_id,'U',:ls_columnname,:ls_old_value,:ls_new_value,:gs_user,sysdate,'');
							if sqlca.sqlcode = -1 then
								messagebox("SQL Error : While Inserting Into Log Table ",SQLCA.SQLErrtext,Information!)
								rollback using sqlca;
								return -1 
