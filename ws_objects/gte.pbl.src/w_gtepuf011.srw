$PBExportHeader$w_gtepuf011.srw
forward
global type w_gtepuf011 from window
end type
type cb_3 from commandbutton within w_gtepuf011
end type
type cb_1 from commandbutton within w_gtepuf011
end type
type cb_4 from commandbutton within w_gtepuf011
end type
type cb_2 from commandbutton within w_gtepuf011
end type
type dw_1 from datawindow within w_gtepuf011
end type
end forward
...
string title = "(W_ltcpuf010)Standard Store Items"
...
global w_gtepuf011 w_gtepuf011

type variables
long ll_ctr,net, ll_cnt,ll_last
string ls_temp,ls_del_ind,ls_sp_manid,ls_tmp_id, ls_sp_nm, ls_spc_id,ls_size,ls_tbcd
...
public function integer wf_check_fillcol (integer fl_row);if dw_1.rowcount() > 0 and fl_row > 0 then
	if (isnull(dw_1.getitemstring(fl_row,'ssp_pcid')) or  len(dw_1.getitemstring(fl_row,'ssp_pcid'))=0 or &
		 isnull(dw_1.getitemstring(fl_row,'ssp_name')) or  len(dw_1.getitemstring(fl_row,'ssp_name'))=0 or &
		 isnull(dw_1.getitemstring(fl_row,'ssp_unit')) or  len(dw_1.getitemstring(fl_row,'ssp_unit'))=0 or &
		 isnull(dw_1.getitemstring(fl_row,'ssp_pcsid')) or  len(dw_1.getitemstring(fl_row,'ssp_pcsid'))=0) then
	    messagebox('Warning: One Of The Following Fields Are Blank','Store Category,Sub category, Item Name, Unit,Please Check !!!')
		 return -1
	end if
end if
return 1
...
if dw_1.rowcount() > 1 then
	for fl_row = 1 to (dw_1.rowcount() - 1)
		ls_sp_name1 = dw_1.getitemstring(fl_row,'ssp_name')
...
IF (isnull(dw_1.getitemstring(dw_1.rowcount(),'ssp_pcid')) or len(dw_1.getitemstring(dw_1.rowcount(),'ssp_pcid'))=0) THEN
...
IF Net = 1 THEN
		if lb_neworder = true then	
			   if ll_last=0 then
				select nvl(MAX(substr(SSP_ID,5,10)),0) into :ll_last from fb_storeproduct_std ;
			end if
		end if
		...
		for ll_ctr = 1 to dw_1.rowcount( ) 
			   ll_last = ll_last + 1
				ls_tmp_id = 'STID'+string(ll_last,'000000') 
				dw_1.setitem(ll_ctr,'ssp_id',ls_tmp_id)
				
			IF wf_check_fillcol(ll_ctr) = -1 THEN 
				return 1
			end if
			if lb_neworder = true then
				ls_sp_nm = dw_1.getitemstring(ll_ctr,'ssp_name')
				
				select distinct 'x' into :ls_temp from fb_storeproduct_std where upper(ssp_name) = upper(:ls_sp_nm);
...
if dw_1.rowcount() = 0 then
	...
	dw_1.setfocus()
	dw_1.setitem(dw_1.getrow(),'ssp_entry_by',gs_user)
	dw_1.setitem(dw_1.getrow(),'ssp_entry_dt',datetime(today()))
	dw_1.setcolumn('ssp_pcid')
else
...
event clicked;if cb_2.text = "&Query" then
	...
	dw_1.reset()
	dw_1.settaborder('ssp_pcid',10)
	dw_1.settaborder('ssp_pcsid',15)
	dw_1.settaborder('ssp_name',20)
	dw_1.settaborder('ssp_name',25)
	dw_1.settaborder('ssp_unit',30) 
	lb_query = true
...
//	cb_3.enabled = false
	...
	dw_1.SetRedraw (TRUE)
	cb_2.text = "&Query"
	dw_1.settaborder('ssp_pcid',0)
	dw_1.settaborder('ssp_pcsid',0)
	dw_1.settaborder('ssp_name',0)
	dw_1.settaborder('ssp_name',0)
	dw_1.settaborder('ssp_unit',0) 
//	cb_1.enabled = true
end if
...
integer height = 2052
integer taborder = 30
string dataobject = "dw_gtepuf011"
...
event key_down;if lb_query = false then
 if lb_neworder = true then	
	...
	if newrow = dw_1.rowcount() and dw_1.rowcount() > 1  then
		dw_1.setitem(newrow,'ssp_entry_by',gs_user)
		dw_1.setitem(newrow,'ssp_entry_dt',datetime(today()))
		dw_1.setcolumn('ssp_pcid')
	end if
 end if
end if
...
event itemchanged;if lb_query = false then
	if dwo.name = 'ssp_name'  then
		ls_sp_nm = data
		//dw_1.setitem(dw_1.getrow(),'sp_manid',ls_sp_nm)
		
		ls_spc_id = dw_1.getitemstring(row,'ssp_pcid')
...
//	
//	if dwo.name = 'sp_tboard_code'  then 
//		ls_tbcd = data
//		
...
//	end if
	...
	dw_1.setitem(row,'ssp_entry_by',gs_user)
	dw_1.setitem(row,'ssp_entry_dt',datetime(today()))
	cb_3.enabled = true
end if
...
SELECT COLUMN_ID,COLUMN_Name FROM USER_TAB_COLUMNS WHERE TABLE_NAME = upper('fb_storeproduct_std');
			...
			ls_unique_id=dw_1.GetItemString(li_row, 1, Primary!, true)
					 ...
					 if ls_old_value <> ls_new_value then
						insert into fb_log(table_name, Unique_key, U_ACTION, U_FIELD, U_OLDVALUE, U_NEWVALUE, U_USERCODE, U_DATE, U_COMMENT) values
												('fb_storeproduct_std',:ls_unique_id,'U',:ls_columnname,:ls_old_value,:ls_new_value,:gs_user,sysdate,'');
							if sqlca.sqlcode = -1 then
								messagebox("SQL Error : While Inserting Into Log Table ",SQLCA.SQLErrtext,Information!)
								rollback using sqlca;
								return -1 
