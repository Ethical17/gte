$PBExportHeader$w_gtelaf065.srw
forward
global type w_gtelaf065 from window
end type
type cb_4 from commandbutton within w_gtelaf065
end type
type cb_1 from commandbutton within w_gtelaf065
end type
type cb_3 from commandbutton within w_gtelaf065
end type
type cb_2 from commandbutton within w_gtelaf065
end type
type dw_1 from datawindow within w_gtelaf065
end type
end forward
...
string title = "(w_gtelaf065) PF Unclaimed"
...
global w_gtelaf065 w_gtelaf065
...
public function integer wf_check_fillcol (integer fl_row);if dw_1.rowcount() > 0 and fl_row > 0 then
	if (isnull(dw_1.getitemstring(fl_row,'emp_id')) or  len(dw_1.getitemstring(fl_row,'emp_id'))=0 or &
		...
		isnull(dw_1.getitemdatetime(fl_row,'pfu_dt'))) then
	    messagebox('Warning: One Of The Following Fields Are Blank','Emp ID, PF No., Unclaimed declaration date, Please Check !!!')
		 return -1
	end if
end if
return 1
...
if dw_1.rowcount() = 0 then
	...
	dw_1.setfocus()
	dw_1.setitem(dw_1.getrow(),'pfu_entry_by',gs_user)
	dw_1.setitem(dw_1.getrow(),'pfu_entry_dt',datetime(today()))
	dw_1.settaborder('emp_id',10)
	dw_1.settaborder('emp_id_1',20)
	dw_1.settaborder('pfu_dt',30)
...
IF MessageBox("Save  Alert", 'Do You Want To Save ....?' ,Exclamation!, YesNo!, 1) = 1 THEN
		...
		IF wf_check_fillcol(dw_1.getrow()) = -1 or wf_check_duplicate_rec(ls_temp_empid,dw_1.getrow()) = -1 THEN
			return 1
		END IF
		select 'x' into :ls_temp from fb_pfunclaimed where emp_id = :ls_temp_empid ;
...
integer height = 2052
integer taborder = 30
string dataobject = "dw_gtelaf065"
...
if lb_query = false then
 if lb_neworder = true then	
	...
	if newrow = dw_1.rowcount() and dw_1.rowcount() > 1  then
		dw_1.setitem(newrow,'pfu_entry_by',gs_user)
		dw_1.setitem(newrow,'pfu_entry_dt',datetime(today()))
...
event itemchanged;if lb_query = false then
	...
	dw_1.setitem(row,'pfu_entry_by',gs_user)
	dw_1.setitem(row,'pfu_entry_dt',datetime(today()))
	cb_3.enabled = true
end if	
...
SELECT COLUMN_ID,COLUMN_Name FROM USER_TAB_COLUMNS WHERE TABLE_NAME = upper('fb_pfunclaimed');
			...
			ls_unique_id=dw_1.GetItemString(li_row, 1, Primary!, true)
					 ...
					 if ls_old_value <> ls_new_value then
						insert into fb_log(table_name, Unique_key, U_ACTION, U_FIELD, U_OLDVALUE, U_NEWVALUE, U_USERCODE, U_DATE, U_COMMENT) values
												('fb_pfunclaimed',:ls_unique_id,'U',:ls_columnname,:ls_old_value,:ls_new_value,:gs_user,sysdate,'');
							if sqlca.sqlcode = -1 then
								messagebox("SQL Error : While Inserting Into Log Table ",SQLCA.SQLErrtext,Information!)
								rollback using sqlca;
								return -1 
