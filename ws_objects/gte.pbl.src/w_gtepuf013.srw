$PBExportHeader$w_gtepuf013.srw
forward
...
type cb_6 from commandbutton within w_gtepuf013
end type
type cbx_1 from checkbox within w_gtepuf013
end type
type cb_5 from commandbutton within w_gtepuf013
end type
type cb_7 from commandbutton within w_gtepuf013
end type
type cb_8 from commandbutton within w_gtepuf013
end type
type cb_9 from commandbutton within w_gtepuf013
end type
type dw_2 from datawindow within w_gtepuf013
end type
type cb_4 from commandbutton within w_gtepuf013
end type
type cb_3 from commandbutton within w_gtepuf013
end type
type cb_2 from commandbutton within w_gtepuf013
end type
type cb_1 from commandbutton within w_gtepuf013
end type
type dw_1 from datawindow within w_gtepuf013
end type
end forward
...
string title = "(W_gtepuf013) Service Invoice"
...
global w_gtepuf013 w_gtepuf013

type variables
long ll_user_level,ll_ctr,ll_last,ll_count
string ls_temp,ls_sm_id,ls_last,ls_count,ls_tmp_id,ls_appr_ind,ls_lpo_id,ls_sup_id,ls_supname,ls_ac_dt,ls_spname,ls_iss_locn,ls_rec_locn,ls_hsn_cd, ls_rec_GSTNNO, ls_iss_GSTNNO
boolean lb_neworder, lb_query
double ld_tot_val,ld_freight,ld_insurance,ld_other,ld_net_amt,ld_discount,ld_qnty,ld_unit_price,ld_balqnty,ld_saletax,ld_efunit_price,ld_amount, ld_cgst_per, ld_sgst_per, ld_igst_per
...
if dw_2.rowcount() > 0 and fl_row > 0 then
	if (isnull(dw_2.getitemstring(fl_row,'sm_id')) or  len(dw_2.getitemstring(fl_row,'sm_id'))=0 or &
		...
		isnull(dw_2.getitemnumber(fl_row,'lpi_unitprice')) or dw_2.getitemnumber(fl_row,'lpi_unitprice') = 0 or & 
	    isnull(dw_2.getitemnumber(fl_row,'lpi_quantity')) or dw_2.getitemnumber(fl_row,'lpi_quantity') = 0) then
	    messagebox('Warning: One Of The Following Fields Are Blank','Service, HSN No, Quantity,Expense Head,Unit Price, Please Check !!!')
	    return -1
	end if
end if
return 1
...
if dw_2.rowcount() > 1 then
	for fl_row = 1 to (dw_2.rowcount() - 1)
		ls_con_id1 = dw_2.getitemstring(fl_row,'sm_id')
...
if f_check_fin_yr(datetime(ls_ac_dt)) = -1 then;	return 1;end if;
	...
	for ll_ctr = 1 to dw_1.rowcount() 
		if dw_1.getitemstring(ll_ctr,'appr_flag') = 'Y'  and isnull(dw_1.getitemstring(ll_ctr,'lpi_vou_no'))=true then
			...
			select rowid,SM_ID,EACSUBHEAD_ID,(LPI_QUANTITY * LPI_EFFECTIVEUNITPRICE)
			from fb_spidetails 
			where LPI_ID = :ls_lpi;
			...
			if sqlca.sqlcode = -1 then 
				...
				setnull(ls_rowid);setnull(ls_SM_ID);setnull(ls_EACSUBHEAD_ID);ld_value=0;
				fetch c1 into :ls_rowid,:ls_SM_ID,:ls_EACSUBHEAD_ID,:ld_value;
				
				do while sqlca.sqlcode <> 100
						//New/Update DailyExpense
					if luo_fames.wf_upd_mes(ls_ac_dt,ls_EACSUBHEAD_ID,ld_value,'C','N')  = -1 then 
						rollback using sqlca;
						return 1;
					end if;								
					setnull(ls_rowid);setnull(ls_SM_ID);setnull(ls_EACSUBHEAD_ID);ld_value=0;
				...
				// JV Creation
				if luo_fames.wf_servicepurchase_to_ac_gst(dw_1.getitemstring(ll_ctr,'lpi_id'),ls_ac_dt) = -1 then 
					rollback using sqlca;
					return 1;
				end if;
...
event itemchanged;if lb_query = false then
	if dwo.name = 'sm_id' then
		...
		if  wf_check_duplicate_rec(ls_sm_id) = -1 then return 1
		
		if isnull(ls_iss_locn) or len(ls_iss_locn) = 0 then
			messagebox('Warning!','Supplier State / From State Is Blank, Please Check !!!')
		end if
		
		select SM_HSN_NO into :ls_hsn_cd from fb_service_master where sm_id =  :ls_sm_id;
		if sqlca.sqlcode = -1 then 
	...
	if dwo.name = 'lpi_unitprice' then
		...
		ld_lpi_date = datetime(dw_1.getitemdatetime(dw_1.getrow(),'lpi_date'))
		if isnull(ld_lpi_date) then
			messagebox('Warning', 'Please Enter Invoice Date')
			return 1
		end if
		...
		if sqlca.sqlcode = -1 then 
			...
			return 1
		elseif sqlca.sqlcode = 100 then 
			messagebox('Error','Item HSN Code Missing in HSN Master, Please Check !!!')
			return 1
		end if;	

		select SUP_GSTN_STCD into :ls_iss_locn from fb_supplier where sup_id =  :ls_sup_id;
		if sqlca.sqlcode = -1 then 
			messagebox('Sql Error','Error During Getting Supplier State Code : '+sqlca.sqlerrtext)
			return 1
		end if;			
...
IF MessageBox("Save  Alert", 'Do You Want To Save ....?' ,Exclamation!, YesNo!, 1) = 1 THEN
	...
	if lb_neworder = true then
		ll_last =0
		if ll_last=0 then
			select nvl(MAX(substr(lpi_id,4,10)),0) into :ll_last from fb_servicepurchaseinvoice;
		end if

			ll_last = ll_last + 1
			ls_tmp_id = 'SPI'+string(ll_last,'0000000000') 
	...
	next
	if lb_neworder = true then
		Messagebox('Information!','Service Invoice No Generated Is ('+ls_tmp_id+')')
	end if
...
event itemchanged;if dwo.name = 'lpi_desc'  and lb_query = false then
	...
	select distinct 'x' into :ls_temp from fb_servicepurchaseinvoice where  trunc(LPI_DATE) < trunc(:ld_lpi_date) and nvl(LPI_VOU_NO,'N')='N' ;
	
	if sqlca.sqlcode = -1 then 
	      messagebox('Sql Error','Error During check Service invoice for previous date A/c posting : '+sqlca.sqlerrtext)
	      return 1
	elseif sqlca.sqlcode = 0 then 
	     messagebox('Error','Previous Date Service invoice Already Available for  A/c Posting : ')
		return 1
	end if;
...
if dwo.name = 'lpi_date'   then
	...
	select max(lpi_date) into :ld_date from fb_servicepurchaseinvoice;  		
	if sqlca.sqlcode = -1 then
		messagebox('Error : While Getting Maximum Service Invoice Date',sqlca.sqlerrtext)
		rollback using sqlca;
		return 1
	end if
	
	if ld_date > datetime(data) then
		messagebox('Warning!','Service Invoice Date Should Be >= '+string(ld_date,'dd/mm/yyyy')+', Please Check !!!')
		return 1
	end if
end if;	
...
if dwo.name = 'sup_id' then
	ls_sup_id = data
	select distinct sup_gstn_stcd, sup_gstnno into :ls_iss_locn,:ls_iss_gstnno from fb_supplier where sup_id = :ls_sup_id ;
	if sqlca.sqlcode = -1 then
		messagebox('Sql Error','Error During Getting Supplier GST No.  : '+sqlca.sqlerrtext)
		rollback using sqlca;
		return 1
	elseif sqlca.sqlcode = 100 then
		messagebox('Error','Supplier not found  : '+sqlca.sqlerrtext)
		rollback using sqlca;
		return 1
	end if 
...
if dwo.name = 'lpo_id'  then
	...
	select distinct LPO_DATE,b.SUP_ID,c.SUP_NAME, c.SUP_ADD, sup_gstn_stcd, SUP_GSTNNO  
	   ...
	   from fb_servicepurchaseorder b,fb_supplier c
      where b.SUP_ID=c.SUP_ID(+) and  LPO_ID=:ls_lpo_id;
...
//		
 ...
 if (dwo.name = 'lpi_freight' or dwo.name = 'lpi_insurance' or dwo.name = 'lpi_otheramo') then
