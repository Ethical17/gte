$PBExportHeader$w_gtedsf011.srw
forward
global type w_gtedsf011 from window
end type
type cb_4 from commandbutton within w_gtedsf011
end type
type cb_3 from commandbutton within w_gtedsf011
end type
type cb_2 from commandbutton within w_gtedsf011
end type
type cb_1 from commandbutton within w_gtedsf011
end type
type dw_1 from datawindow within w_gtedsf011
end type
end forward
...
string title = "(w_gtedsf011) Parameter Master"
...
global w_gtedsf011 w_gtedsf011
...
public function integer wf_check_duplicate_rec (string fs_type, string fs_fp, datetime fd_dt)
end prototypes
...
public function integer wf_check_duplicate_rec (string fs_type, string fs_fp, datetime fd_dt);long fl_row
...
if dw_1.rowcount() > 1 then
	for fl_row = 1 to (dw_1.rowcount() - 1)
		...
		if ls_type1 = fs_type and fs_fp = ls_fp1 and ld_dt1 = fd_dt then
	...
	if dw_1.rowcount() > 0 then
		...
		for ll_ctr = 1 to dw_1.rowcount( ) 
			...
			if dw_1.getitemstatus(ll_ctr,0,primary!) = NewModified! or dw_1.getitemstatus(ll_ctr,0,primary!) = New! then		
				...
				where PD_DOC_TYPE = :ls_type and PD_VALUETYPE = :ls_fp  and pd_period_from >= :ld_frdt and pd_period_to is null;
				...
				where PD_DOC_TYPE = :ls_type and PD_VALUETYPE = :ls_fp  and pd_period_to is null and pd_period_from < :ld_frdt;
				if sqlca.sqlcode = -1 then
...
integer height = 2052
integer taborder = 30
string dataobject = "dw_gtedsf011"
...
event itemchanged;if lb_query = false then
	...
	if dwo.name = 'pd_valuetype' then
		...
		where PD_DOC_TYPE = :ls_type and PD_VALUETYPE = :ls_fp and pd_period_to is null;
	...
	if dwo.name = 'pd_period_from'  then
		...
		if  wf_check_duplicate_rec(ls_type,ls_fp,ld_frdt) = -1 then return 1
